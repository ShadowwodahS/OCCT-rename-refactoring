...
more fix: 
1. IGESType，原本声明定义了（但又没人调用）一个同名的函数IGESData_IGESEntity::IGESType()，改成IGESData_IGESEntity::GetIGESType()了。
2. ShapeExplorer，这个命名和原本的LoadSeamEdge()函数里的局部变量名相同。把局部变量改成小写开头了。