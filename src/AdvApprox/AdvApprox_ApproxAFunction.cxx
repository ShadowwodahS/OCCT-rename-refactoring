...

{
  ...
  Standard_Integer icurve, idim, iordre, ii, Dimension = Num1DSS + 2 * Num2DSS + 3 * Num3DSS,
  ...
  Standard_Integer&        ErrorCode)
{
  ...
  for (Standard_Integer I = 1; I <= TotalNumSS; I++)
  {
                                        ...
                                        const CuttingTool& CutTool)
{
  ...
  Standard_Integer TotalNumSS = Num1DSS + Num2DSS + Num3DSS, ii, jj, kk, index, dim_index,
                   local_index;
  ...
  Standard_Integer ErrorCode = 0, NumCurves, size = myMaxSegments * NumMaxCoeffs * TotalDimension;
  Handle(TColStd_HArray1OfInteger) NumCoeffPerCurvePtr =
    new TColStd_HArray1OfInteger(1, myMaxSegments);

  Handle(TColStd_HArray1OfReal) LocalCoefficientsPtr = new TColStd_HArray1OfReal(1, size);

  Handle(TColStd_HArray1OfReal) IntervalsPtr = new TColStd_HArray1OfReal(1, myMaxSegments + 1);
  ...
  if (ErrorCode == 0 || ErrorCode == -1)
  {
    ...
    if (AConverter.IsDone())
    {
      ...
      {
        my1DPoles    = new TColStd_HArray2OfReal(1, PolesPtr->ColLength(), 1, myNumSubSpaces[0]);
...
void AdvApprox_ApproxAFunction::Poles(const Standard_Integer Index, TColgp_Array1OfPnt& P) const
{
  Standard_Integer ii;
...
void AdvApprox_ApproxAFunction::Poles2d(const Standard_Integer Index, TColgp_Array1OfPnt2d& P) const
{
  Standard_Integer ii;
  for (ii = P.Lower(); ii <= P.Upper(); ii++)
  {
...
void AdvApprox_ApproxAFunction::Poles1d(const Standard_Integer Index, TColStd_Array1OfReal& P) const
{
  Standard_Integer ii;
  for (ii = P.Lower(); ii <= P.Upper(); ii++)
  {
...
Handle(TColStd_HArray1OfReal) AdvApprox_ApproxAFunction::MaxError(const Standard_Integer D) const
...
{
  Handle(TColStd_HArray1OfReal) EPtr = MaxError(D);
...
Handle(TColStd_HArray1OfReal) AdvApprox_ApproxAFunction::AverageError(
  const Standard_Integer D) const
...
Standard_Real AdvApprox_ApproxAFunction::AverageError(const Standard_Integer D,
                                                      const Standard_Integer Index) const

{
  Handle(TColStd_HArray1OfReal) EPtr = AverageError(D);
  return EPtr->Value(Index);
}
