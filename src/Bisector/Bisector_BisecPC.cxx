                               ...
                               const Standard_Real         DistMax)
{
  ...
  if (!isConvex)
  {
    if (point.IsEqual(curve->Value(curve->FirstParameter()), Precision1::Confusion()))
    {
    ...
    else if (point.IsEqual(curve->Value(curve->LastParameter()), Precision1::Confusion()))
    {
                                 ...
                                 gp_Vec2d&           V3) const
{
  ...
  {
    if (pointStartBis.IsEqual(point, Precision1::PConfusion()))
      P = pointStartBis;
    else
    {
  ...
  {
    if (pointEndBis.IsEqual(point, Precision1::PConfusion()))
      P = pointEndBis;
    else
    {
...
Standard_Real Bisector_BisecPC::Distance(const Standard_Real U) const
{
  ...
  if (point.IsEqual(PC, Precision1::Confusion()))
  {
    if (isConvex)
    {
      return 0.;
  ...
  if (Abs(Prosca) < Precision1::Confusion() || (Prosca * sign) > 0.)
  {
    return Precision1::Infinite();
  }
  else
  {
    ...
    if (!isConvex)
    {
      Standard_Real K = Curvature(curve, UOnCurve, Precision1::Confusion());
      if (K != 0.)
      {
        if (Dist > 1 / (K * K))
        {
...
void Bisector_BisecPC::D0(const Standard_Real U, gp_Pnt2d& P) const
{
  P = point;
  gp_Vec2d V1, V2, V3;
...
void Bisector_BisecPC::D1(const Standard_Real U, gp_Pnt2d& P, gp_Vec2d& V) const
{
  P = point;
  V.SetCoord(0., 0.);
  gp_Vec2d V2, V3;
...
void Bisector_BisecPC::D2(const Standard_Real U, gp_Pnt2d& P, gp_Vec2d& V1, gp_Vec2d& V2) const
{
  P = point;
  V1.SetCoord(0., 0.);
  V2.SetCoord(0., 0.);
...
gp_Vec2d Bisector_BisecPC::DN(const Standard_Real U, const Standard_Integer N) const
{
  gp_Pnt2d P = point;
  gp_Vec2d V1(0., 0.);
  gp_Vec2d V2(0., 0.);
...
void Bisector_BisecPC::Dump(const Standard_Integer, const Standard_Integer Offset) const
{
  Indent(Offset);
