...
static Standard_Boolean IsInside(const TopoFace&, const TopoWire&, const TopoWire&);

static Standard_Boolean IsInside(const TopoFace&, const TopoWire&);

static void GetDirection(const TopoEdge&,
                         const TopoFace&,
                         gp_Pnt2d&,
                         gp_Vec2d&,
                         Standard_Boolean isFirstEnd);

static Standard_Boolean ChoixUV(const TopoEdge&,
                                const TopoFace&,
...
static TopoShape ChooseDirection(const TopoShape&,
                                    const TopoVertex&,
                                    const TopoFace&,
                                    const ShapeList&);

inline Standard_Boolean SameUV(const gp_Pnt2d&            P1,
                               const gp_Pnt2d&            P2,
...
void LocOpe_SplitShape::Init(const TopoShape& S)
{
...
Standard_Boolean LocOpe_SplitShape::CanSplit(const TopoEdge& E) const
{
  if (myDone)
  {
...
void LocOpe_SplitShape::Add(const TopoVertex& V, const Standard_Real P, const TopoEdge& E)
{
  if (!CanSplit(E))
  {
    return;
  ...
  TopoEdge edg = TopoDS::Edge(itl.Value());
  le.Remove(itl);
  if (V.Orientation() == TopAbs_FORWARD || V.Orientation() == TopAbs_REVERSED)
  {

    edg.Orientation(TopAbs_FORWARD);
    TopoVertex aCurV1, aCurV2;
    TopExp1::Vertices(edg, aCurV1, aCurV2);
    Standard_Real aPar1 = BRepInspector::Parameter(aCurV1, edg);

    Standard_Real aPar2 = BRepInspector::Parameter(aCurV2, edg);

    TopoShape aLocalShape = edg.EmptyCopied();
    TopoEdge  E1          = TopoDS::Edge(aLocalShape);
    aLocalShape              = edg.EmptyCopied();
    TopoEdge E2           = TopoDS::Edge(aLocalShape);
    //    TopoEdge E1 = TopoDS::Edge(edg.EmptyCopied());
    //    TopoEdge E2 = TopoDS::Edge(edg.EmptyCopied());
    E1.Orientation(TopAbs_FORWARD);
    E2.Orientation(TopAbs_FORWARD);
    TopoVertex newVtx     = V;
    Standard_Real aTolSplitV = BRepInspector::Tolerance(V);
    ...
    Standard_Real aTolV1 =
      (BRepInspector::Degenerated(edg) ? Max(BRepInspector::Tolerance(aCurV1), aTolSplitV)
                                   : BRepInspector::Tolerance(aCurV1));

    B.UpdateVertex(aCurV1, aPar1, E1, aTolV1);
    newVtx.Orientation(TopAbs_REVERSED);
    B.Add(E1, newVtx);
    B.UpdateVertex(newVtx, P, E1, BRepInspector::Tolerance(V));
    newVtx.Orientation(TopAbs_FORWARD);
    B.Add(E2, newVtx);
    B.UpdateVertex(newVtx, P, E2, BRepInspector::Tolerance(V));
    ...
    // for degenerated edges tolerance of vertices should be set to maximal value
    // from tolerance of the vertex of the edge and tolerance of splitting vertex
    Standard_Real aTolV2 = (BRepInspector::Degenerated(edg) ? aTolV1 : BRepInspector::Tolerance(aCurV2));
    ...
    for (; itl.More(); itl.Next())
    {
      const TopoEdge& edg1 = TopoDS::Edge(itl.Value());
  ...
  else
  {
    ...
    for (exp.Init(edg, TopAbs_VERTEX); exp.More(); exp.Next())
    {
      ...
      f                        = BRepInspector::Parameter(vtx, edg);
      B.Add(E1, vtx);
      B.UpdateVertex(vtx, f, E1, BRepInspector::Tolerance(vtx));
    }
    B.Add(E1, V);
    B.UpdateVertex(V, P, E1, BRepInspector::Tolerance(V));
    le.Append(E1);
  }
}
...
Standard_Boolean LocOpe_SplitShape::Add(const ShapeList& Lwires, const TopoFace& F)
{
  ...
  for (; itl.More(); itl.Next())
  {
    ...
    {
      const TopoWire& aWire = TopoDS::Wire(itwires.Value());
  ...
  TopoFace FaceRef = TopoDS::Face(itl.Value());
  ...
  ShapeList BreakVertices;
  ShapeList BreakOnWires;

  TopTools_DataMapOfShapeShape VerWireMap;
  Standard_Integer             i;
  ShapeExplorer              ExploF, ExploW;
  for (itl.Initialize(aLInside); itl.More(); itl.Next())
  {
    const TopoWire& aSection = TopoDS::Wire(itl.Value());
    TopoVertex      Ver[2];
    TopExp1::Vertices(aSection, Ver[0], Ver[1]);
    for (i = 0; i < 2; i++)
    {
      ...
      {
        const TopoShape& aWire = ExploF.Current();
        TopoShape        aVer;
  ...
  for (itl.Initialize(aLInside); itl.More(); itl.Next())
  {
    const TopoWire& aWire = TopoDS::Wire(itl.Value());
    TopoVertex      V1, V2;
    TopExp1::Vertices(aWire, V1, V2);
    ShapeList LW1, LW2;
  ...
  TopoWire       outerW  = BRepTools1::OuterWire(FaceRef);
  TopoWire       CurWire = outerW;
  BRepLib_MakeWire* MW;
  MW = new BRepLib_MakeWire();
  BRepTools_WireExplorer wexp(CurWire, FaceRef);
  for (;;)
  {
    TopoVertex theStartVertex = wexp.CurrentVertex(), CurVertex;
    TopoEdge   CurEdge        = wexp.Current();
    TopoEdge   LastEdge       = CurEdge;
    MW->Add(CurEdge);
    TopoWire   aSectionWire;
    TopoVertex aBreakVertex;
    wexp.Next();
    if (!wexp.More())
    ...
    for (;;)
    {
      ...
      {
        TopoShape aLocalWire =
          ChooseDirection(LastEdge, CurVertex, FaceRef, VerSecMap(CurVertex));
    ...
    for (;;)
    {
      ...
      {
        TopoVertex V1, V2, aStartVertex;
        TopExp1::Vertices(aSectionWire, V1, V2);
        ...
        const ShapeList& Lsections = VerSecMap(aStartVertex);
        if (Lsections.Extent() == 1)
          break;

        // else: choose the way
        TopoWire NextSectionWire = TopoDS::Wire(
          (aSectionWire.IsSame(Lsections.First())) ? Lsections.Last() : Lsections.First());
        ...
        else
        {
          ...
          const TopoEdge&   aStartEdge = wexp.Current();
          ShapeList Ldirs;
          Ldirs.Append(aStartEdge);
          Ldirs.Append(NextSectionWire);
          TopoShape theDirection = ChooseDirection(aSectionWire, aStartVertex, FaceRef, Ldirs);
  ...
  {
    TopoShape aLocalFace = FaceRef.EmptyCopied();
    TopoFace  aNewFace   = TopoDS::Face(aLocalFace);
  ...
  for (ExploF.Init(FaceRef, TopAbs_WIRE); ExploF.More(); ExploF.Next())
  {
    ...
    TopoShape     anEdge = ExploW.Current();
    Standard_Boolean found  = Standard_False;
    for (itl.Initialize(NewWires); itl.More(); itl.Next())
    {
      const TopoShape& aNewWire = itl.Value();
  ...
  {
    const TopoWire& aHole = TopoDS::Wire(itl.Value());
    for (itlNewF.Initialize(NewFaces); itlNewF.More(); itlNewF.Next())
    {
      TopoFace& aNewFace = TopoDS::Face(itlNewF.ChangeValue());
  ...
  {
    ShapeList& ls = myMap(ExploF.Current());
    ls.Clear();
  }
  ///////////////////
  ...
  for (itl.Initialize(aLInside); itl.More(); itl.Next())
    ...
    {
      const TopoEdge& edg = TopoDS::Edge(ExploW.Current());
      if (!BRepInspector::HasContinuity(edg, F, F))
      {
...
Standard_Boolean LocOpe_SplitShape::Add(const TopoWire& W, const TopoFace& F)
{
...
Standard_Boolean LocOpe_SplitShape::AddClosedWire(const TopoWire& W, const TopoFace& F)
{
  ShapeExplorer exp;
  ...
  TopoShape       aLocalFace = F.EmptyCopied();
  TopoFace        newFace    = TopoDS::Face(aLocalFace);
  ...
  TopoFace newRef = TopoDS::Face(aLocalFace);
  //  TopoFace newRef = TopoDS::Face(FaceRef.EmptyCopied());
  ...
  B.Add(newRef, W.Oriented(TopAbs1::Reverse(orWire)));
  lf.Append(newRef);
  lf.Append(newFace);
  return Standard_True;
}
...
static Standard_Boolean checkOverlapping(const TopoEdge& theEdge1,
                                         ...
                                         const TopoFace& theFace)
{
  ...
  Standard_Real MaxTol = (BRepInspector::Tolerance(theEdge1) + BRepInspector::Tolerance(theEdge2));
  ...
  Handle(GeomCurve2d) aCrv1 = BRepInspector::CurveOnSurface(theEdge1, theFace, aF1, aL1);
  Handle(GeomCurve2d) aCrv2 = BRepInspector::CurveOnSurface(theEdge2, theFace, aF2, aL2);
...
Standard_Boolean LocOpe_SplitShape::AddOpenWire(const TopoWire& W, const TopoFace& F)
{
  // On cherche la face descendante de F qui continent le wire
  ShapeList&              lf = myMap(F);
  TopTools_ListIteratorOfListOfShape itl(lf);
  ...
  TopoShape aLocalShape = W.Oriented(TopAbs_FORWARD);
  TopExp1::Vertices(TopoDS::Wire(aLocalShape), Vfirst, Vlast);

  tolf = BRepInspector::Tolerance(Vfirst);
  toll = BRepInspector::Tolerance(Vlast);
  tol1 = Max(tolf, toll);

  ShapeExplorer exp, exp2;

  TopoWire wfirst, wlast;
  for (; itl.More(); itl.Next())
  {
    TopoFace fac = TopoDS::Face(itl.Value());
    if (!IsInside(fac, W))
    {
      continue;
    }
  ...
  if (wfirst.IsSame(wlast))
  {
    ...
    // Essai JAG
    ShapeList WiresFirst;
    for (exp.Init(wfirst, TopAbs_EDGE); exp.More(); exp.Next())
    {
      if (BRepInspector::IsClosed(TopoDS::Edge(exp.Current()), FaceRef))
      {
    ...
    for (exp.Init(W.Oriented(TopAbs_FORWARD), TopAbs_EDGE); exp.More(); exp.Next())
    {
      ...
      WiresFirst.Append(edg.Oriented(TopAbs1::Reverse(orient)));
      myDblE.Add(edg);
    }
    ...
    TopoShape aLocalFace = FaceRef.Oriented(wfirst.Orientation());
    C2d                     = BRepInspector::CurveOnSurface(LastEdge, TopoDS::Face(aLocalFace), f, l);
    ...
    while (cond)
    {
      ...
      {
        const TopoEdge& edg = TopoDS::Edge(lexp.Value());

        orient = edg.Orientation();
        TopExp1::Vertices(edg, vdeb, vfin);
      ...
      if (nbPoss == 1)
      {
        ...
        TopoShape aLocalFaceTemp = FaceRef.Oriented(wfirst.Orientation());
      ...
      if (nbPoss >= 1)
      {
        ...
        if (LastEdge.Orientation() == TopAbs_FORWARD)
        {
          Vlast = TopExp1::LastVertex(LastEdge);
        }
        else
        {
          Vlast = TopExp1::FirstVertex(LastEdge);
        }
    ...
    {
      ShapeList& ls = myMap(exp.Current());
      itl.Initialize(ls);
      for (; itl.More(); itl.Next())
      {
  ...
  else
  {
    ...
    // Edges de wfirst
    for (exp.Init(wfirst.Oriented(TopAbs_FORWARD), TopAbs_EDGE); exp.More(); exp.Next())
    {
      B.Add(newWire, TopoDS::Edge(exp.Current()));
    }

    // Edges de wlast
    for (exp.Init(wlast.Oriented(TopAbs_FORWARD), TopAbs_EDGE); exp.More(); exp.Next())
    {
      const TopoEdge& edg = TopoDS::Edge(exp.Current());
      orient                 = TopAbs1::Compose(edg.Orientation(), orRelat);
    ...
    for (exp.Init(W.Oriented(TopAbs_FORWARD), TopAbs_EDGE); exp.More(); exp.Next())
    {
      ...
      orient                 = edg.Orientation();
      B.Add(newWire, edg.Oriented(orient));
      B.Add(newWire, edg.Oriented(TopAbs1::Reverse(orient)));
...
const ShapeList& LocOpe_SplitShape::LeftOf(const TopoWire& W, const TopoFace& F)
{
  if (myShape.IsNull())
  {
  ...
  {
    const TopoEdge& edg = TopoDS::Edge(expw.Current());
    for (itl.Initialize(myMap(theFace)); itl.More(); itl.Next())
    {
      ...
      {
        const TopoEdge& edgbis = TopoDS::Edge(expf.Current());
        if (edgbis.IsSame(edg) && edgbis.Orientation() == edg.Orientation())
        {
          ...
          if (!itl2.More())
          { // la face n`est pas deja presente
            myLeft.Append(fac);
          }
          break;
        }
...
const ShapeList& LocOpe_SplitShape::DescendantShapes(const TopoShape& S)
{
  if (!myDone)
  {
    Rebuild(myShape);
...
void LocOpe_SplitShape::Put(const TopoShape& S)
{
  if (!myMap.IsBound(S))
  {
    ShapeList thelist;
...
static void updateToleraces(const TopoFace&                        theFace,
                            const TopTools_DataMapOfShapeListOfShape& theMap)
{
  ShapeExplorer aExpE(theFace, TopAbs_EDGE);
  for (; aExpE.More(); aExpE.Next())
  {
    if (!theMap.IsBound(aExpE.Current()))
      continue;
    const ShapeList& lEdges = theMap(aExpE.Current());
    if (lEdges.Extent() <= 1)
      continue;
    ...
    {
      TopoEdge   aCurE   = TopoDS::Edge(itrE.Value());
...
Standard_Boolean LocOpe_SplitShape::Rebuild(const TopoShape& S)

{
  ...
  {
    ShapeBuilder       B;
    TopoShape       result = S.EmptyCopied();
    TopAbs_Orientation orient;
    for (it.Initialize(S); it.More(); it.Next())
    {
...
static Standard_Boolean IsInside(const TopoFace& F, const TopoWire& W1, const TopoWire& W2)
{
  ...
  //  Standard_Real U,V;
  ShapeExplorer      exp(W1, TopAbs_EDGE);
  const TopoEdge&   edg = TopoDS::Edge(exp.Current());
  ShapeExplorer      exp2(edg, TopAbs_VERTEX);
...
static Standard_Boolean IsInside(const TopoFace& F, const TopoWire& W)
{
  // Attention, c`est tres boeuf !!!!
...
static void GetDirection(const TopoEdge& theEdge,
                         const TopoFace& theFace,
                         gp_Pnt2d&          thePnt,
                         gp_Vec2d&          theDir,
                         Standard_Boolean   isFirstEnd)
...
Standard_Boolean ChoixUV(const TopoEdge&                Last,
                         const TopoFace&                F,
                         ...
                         gp_Vec2d&                         dlst)
{
  ...
  {
    TopoEdge anEdge = TopoDS::Edge(Poss.FindKey(index));
...
static TopoShape ChooseDirection(const TopoShape&         RefDir,
                                    const TopoVertex&        RefVertex,
                                    const TopoFace&          theFace,
                                    const ShapeList& Ldirs)
{
  ShapeExplorer    Explo(RefDir, TopAbs_EDGE);
  TopoEdge        RefEdge;
  TopoVertex      V1, V2;
  ...
  for (; Explo.More(); Explo.Next())
  {
    ...
    TopExp1::Vertices(RefEdge, V1, V2);
  ...
  Handle(GeomCurve2d) RefCurve = BRepInspector::CurveOnSurface(RefEdge, theFace, RefFirst, RefLast);
  gp_Pnt2d             RefPnt;
  gp_Vec2d             RefVec;
  ...
  for (; itl.More(); itl.Next())
  {
    const TopoShape& aShape = itl.Value();
    TopoEdge         anEdge;
    ...
    aCurve = BRepInspector::CurveOnSurface(anEdge, theFace, aFirst, aLast);
