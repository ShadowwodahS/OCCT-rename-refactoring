...
class LineBuilder2d : public Root6
{
public:
  DEFINE_STANDARD_ALLOC

  //! Creates a line located with A.
  Standard_EXPORT LineBuilder2d(const gp_Ax2d& A);

  //! <P> is the location point (origin) of the line and
  //! <V> is the direction of the line.
  Standard_EXPORT LineBuilder2d(const gp_Pnt2d& P, const gp_Dir2d& V);

  //! Creates the line from the equation A*X + B*Y + C = 0.0
  //! the status is "NullAxis"if Sqrt(A*A + B*B) <= Resolution from gp1.
  Standard_EXPORT LineBuilder2d(const Standard_Real A,
  ...
  Standard_EXPORT LineBuilder2d(const gp_Lin2d& Lin, const Standard_Real Dist);

  //! Make a Lin2d from gp1 <TheLin> parallel to another
  //! Lin2d <Lin> and passing through a Pnt2d <Point>.
  Standard_EXPORT LineBuilder2d(const gp_Lin2d& Lin, const gp_Pnt2d& Point);
  ...
  Standard_EXPORT LineBuilder2d(const gp_Pnt2d& P1, const gp_Pnt2d& P2);
