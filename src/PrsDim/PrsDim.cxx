...
const Standard_Real SquareTolerance = Precision1::SquareConfusion();

//=================================================================================================

Point3d PrsDim1::Nearest(const TopoShape& ashape, const Point3d& apoint)
{
...
//=======================================================================
Point3d PrsDim1::Nearest(const gp_Lin& theLine, const Point3d& thePoint)
{
  Handle(GeomLine) aLine = new GeomLine(theLine);
...
//              and FALSE otherwise.
//=======================================================================
Standard_Boolean PrsDim1::Nearest(const Handle(GeomCurve3d)& theCurve,
...
Point3d PrsDim1::Farest(const TopoShape& aShape, const Point3d& aPoint)
{
...
//=======================================================================
Standard_Boolean PrsDim1::ComputeGeometry(const TopoEdge&  theEdge,
                                         ...
                                         Point3d&             theLastPnt)
{
  ...
  theCurve = BRepInspector::Curve(theEdge, anEdgeLoc, aFirst, aLast);
  if (theCurve.IsNull())
  {
    return Standard_False;
  }
                                         ...
                                         Standard_Boolean&   theIsInfinite)
{
  ...
  theCurve = Handle(GeomCurve3d)::DownCast(anAdaptor.Curve().Curve()->Transformed(anAdaptor.Trsf()));
  if (theCurve.IsNull())
  {
    return Standard_False;
  }
  ...
  theIsInfinite              = (Precision1::IsInfinite(aFirst) || Precision1::IsInfinite(aLast));
...
Standard_Boolean PrsDim1::ComputeGeometry(const TopoEdge&        theEdge,
                                         ...
                                         Point3d&                   theFirstPnt,
                                         Point3d&                   theLastPnt,
                                         Handle(GeomCurve3d)&       theExtCurve,
                                         Standard_Boolean&         theIsInfinite,
                                         ...
                                         const Handle(GeomPlane)& thePlane)
{
  ...
  theCurve =
    Handle(GeomCurve3d)::DownCast(aCurveAdaptor.Curve().Curve()->Transformed(aCurveAdaptor.Trsf()));
  if (theCurve.IsNull())
  {
    return Standard_False;
  }
  ...
  if (Handle(GeomLine) aLine = Handle(GeomLine)::DownCast(theExtCurve))
  {
    theIsOnPlane =
      thePlane->Pln().Contains(aLine->Lin(), Precision1::Confusion(), Precision1::Angular());
  }
  else if (Handle(GeomCircle) aCircle = Handle(GeomCircle)::DownCast(theExtCurve))
  {
    Ax3 aCircPos(aCircle->Position1());
    theIsOnPlane =
      aCircPos.IsCoplanar(thePlane->Pln().Position1(), Precision1::Confusion(), Precision1::Angular());
  }
  ...
  if (Handle(GeomLine) aLine = Handle(GeomLine)::DownCast(theCurve))
  {
    if (!theIsInfinite)
    {
      theFirstPnt = ElCLib1::Value(aFirst, aLine->Lin());
...
Standard_Boolean PrsDim1::ComputeGeometry(const TopoEdge&        theFirstEdge,
                                         const TopoEdge&        theSecondEdge,
                                         Handle(GeomCurve3d)&       theFirstCurve,
                                         Handle(GeomCurve3d)&       theSecondCurve,
                                         ...
                                         const Handle(GeomPlane)& thePlane)
{
  ...
  theFirstCurve  = BRepInspector::Curve(theFirstEdge, aFirstEdgeLoc, aFirst1, aLast1);
  theSecondCurve = BRepInspector::Curve(theSecondEdge, aSecondEdgeLoc, aFirst2, aLast2);
                                         ...
                                         Standard_Boolean&   theIsInfinite2)
{
  ...
  if (!PrsDim1::ComputeGeometry(theSecondEdge,
                               theSecondCurve,
                               theFirstPnt2,
                               theLastPnt2,
                               theIsInfinite2))
  ...
  if (theIsInfinite1 || theIsInfinite2)
  {
    if (theFirstCurve->DynamicType() == theSecondCurve->DynamicType()
        && theFirstCurve->IsInstance(STANDARD_TYPE(GeomLine)))
    {
      gp_Lin aLin1 = Handle(GeomLine)::DownCast(theFirstCurve)->Lin();
                                         ...
                                         const Handle(GeomPlane)& thePlane)
{
  ...
  theFirstCurve =
    Handle(GeomCurve3d)::DownCast(aFirstAdaptor.Curve().Curve()->Transformed(aFirstAdaptor.Trsf()));
  theSecondCurve = Handle(GeomCurve3d)::DownCast(
  ...
  Handle(GeomCurve3d) aFirstSaved  = theFirstCurve;
  Handle(GeomCurve3d) aSecondSaved = theSecondCurve;
  ...
  if (Precision1::IsInfinite(aFirst2) || Precision1::IsInfinite(aLast2))
  {
...
//           Return TRUE if ok
//=======================================================================
Standard_Boolean PrsDim1::ComputeGeomCurve(Handle(GeomCurve3d)&       aCurve,
                                          ...
                                          Standard_Boolean&         isOnPlane)
{
  ...
  if (Delta <= Precision1::PConfusion())
  {
    Delta = last1 - first1;
  }
  ...
  if (!Precision1::IsInfinite(first1) && !Precision1::IsInfinite(last1))
  {
  ...
  if (!isOnPlane)
  {
    ...
    if (!Precision1::IsInfinite(first1) && !Precision1::IsInfinite(last1))
    {
      FirstPnt1 = PrsDim1::ProjectPointOnPlane(FirstPnt1, aPlane->Pln());
      LastPnt1  = PrsDim1::ProjectPointOnPlane(LastPnt1, aPlane->Pln());
    }
  }
  return Standard_True;
}
...
//=======================================================================
Standard_Boolean PrsDim1::ComputeGeometry(const TopoVertex&      aVertex,
                                         Point3d&                   point,
                                         const Handle(GeomPlane)& aPlane,
                                         Standard_Boolean&         isOnPlane)
{
  point     = BRepInspector::Pnt(aVertex);
  isOnPlane = aPlane->Pln().Contains(point, Precision1::Confusion());
  if (!isOnPlane)
  {
    point = PrsDim1::ProjectPointOnPlane(point, aPlane->Pln());
  }
  return Standard_True;
}
...
//=======================================================================
Standard_Boolean PrsDim1::GetPlaneFromFace(const TopoFace&    aFace,
                                          gp_Pln&               aPlane,
...

{
  ...
  aSurf = Handle(GeomSurface)::DownCast(aSurf->Transformed(surf1.Trsf()));
  ...
  if (Result == Standard_True && isOffset)
  {
    ...
    aPlane = (Handle(GeomPlane)::DownCast(aSurf))->Pln();
  }
...
Point3d PrsDim1::ProjectPointOnPlane(const Point3d& aPoint, const gp_Pln& aPlane)
{
...
Point3d PrsDim1::ProjectPointOnLine(const Point3d& aPoint, const gp_Lin& aLine)
{
...
void PrsDim1::InitFaceLength(const TopoFace&    theFace,
                            gp_Pln&               thePlane,
                            Handle(GeomSurface)& theSurface,
                            PrsDim_KindOfSurface& theSurfaceType,
                            Standard_Real&        theOffset)
{
  if (PrsDim1::GetPlaneFromFace(theFace, thePlane, theSurface, theSurfaceType, theOffset)
      && Abs(theOffset) > Precision1::Confusion())
  {
...
Standard_Boolean PrsDim1::InitAngleBetweenPlanarFaces(const TopoFace&     theFirstFace,
...
{
  Handle(GeomPlane) aFirstPlane  = Handle(GeomPlane)::DownCast(BRepInspector::Surface(theFirstFace));
  Handle(GeomPlane) aSecondPlane = Handle(GeomPlane)::DownCast(BRepInspector::Surface(theSecondFace));

  SurfaceIntersector aPlaneIntersector(aFirstPlane, aSecondPlane, Precision1::Confusion());
  ...
  // Get intersect line.
  const Handle(GeomCurve3d)& anIntersectCurve = aPlaneIntersector.Line(1);

  Handle(GeomLine) anIntersectLine = Handle(GeomLine)::DownCast(anIntersectCurve);
  ...
  BRepTools1::UVBounds(theFirstFace, anU1Min, anU1Max, aV1Min, aV1Max);
  BRepTools1::UVBounds(theSecondFace, anU2Min, anU2Max, aV2Min, aV2Max);

  // Get first and second attach.
  if (theIsFirstPointSet)
  {
    PointOnSurfProjector aProjector(theFirstAttach, aFirstPlane);
...
Standard_Boolean PrsDim1::InitAngleBetweenCurvilinearFaces(
  ...
  const Standard_Boolean     theIsFirstPointSet)
{
  Handle(GeomSurface) aFirstSurf  = BRepInspector::Surface(theFirstFace);
  Handle(GeomSurface) aSecondSurf = BRepInspector::Surface(theSecondFace);

  // Find intersection curve between two surfaces.
  SurfaceIntersector aSurfaceIntersector(aFirstSurf, aSecondSurf, Precision1::Confusion());
  ...
  // Get intersect line.
  Handle(GeomLine) aFirstLine, aSecondLine;
  Standard_Real     aFirstU = 0.0;
  Standard_Real     aFirstV = 0.0;

  if (theIsFirstPointSet)
  {
    PointOnSurfProjector aProjector(theFirstAttach, aFirstSurf);
    if (!aProjector.IsDone())
    {
      return Standard_False;
    }
  ...
  aFirstLine = Handle(GeomLine)::DownCast(aFirstSurf->UIso(aFirstU));
  ...
  else if (theSecondSurfType == PrsDim_KOS_Cone)
  {
    ...
    // Check sign
    if (!anXdirection.IsEqual(aToFirstAttach, Precision1::Angular())
        && !anXdirection.IsOpposite(aToFirstAttach, Precision1::Angular())
        && (anXdirection ^ aToFirstAttach) * aCone->Cone().Axis().Direction() < 0.0)
    {
      aSecondU = 2 * M_PI - aSecondU;
    }
    aSecondLine = Handle(GeomLine)::DownCast(aCone->UIso(aSecondU));
  }
  else
  {
    return Standard_False;
  ...
  if (!(aFirstLine->Lin().Direction().IsEqual(aSecondLine->Lin().Direction(), Precision1::Angular()))
                                                    ...
                                                    Precision1::Angular())))
  {
    ...
    PointOnSurfProjector aProjector(theFirstAttach, aSecondSurf);
    if (!aProjector.IsDone())
    {
      return Standard_False;
    }
...
void PrsDim1::InitLengthBetweenCurvilinearFaces(const TopoFace&    theFirstFace,
                                               const TopoFace&    theSecondFace,
                                               Handle(GeomSurface)& theFirstSurf,
                                               Handle(GeomSurface)& theSecondSurf,
                                               Point3d&               theFirstAttach,
                                               Point3d&               theSecondAttach,
                                               Dir3d&               theDirOnPlane)
{
  ...
  ShapeExplorer anExplorer(theFirstFace, TopAbs_VERTEX);

  theFirstAttach = BRepInspector::Pnt(TopoDS::Vertex(anExplorer.Current()));
  ...
  {
    theFirstAttach = PrsDim1::Farest(theFirstFace, theFirstAttach);
    aProjector.Init(theFirstAttach, theFirstSurf);
    aProjector.LowerDistanceParameters(aPU, aPV);
    theFirstSurf->D1(aPU, aPV, theFirstAttach, aD1U, aD1V);
  }
  ...
  for (Standard_Integer aPointIt = 1; aPointIt <= aProjector.NbPoints(); aPointIt++)
  {
    ...
    if (aFirstSurfN.IsParallel(aLocalDir, Precision1::Angular())
  ...
  else
  {
    ...
    BRepTopAdaptor_FClass2d aClassifier(theSecondFace, Precision1::Confusion());
    ...
    {
      theSecondAttach = PrsDim1::Nearest(theSecondFace, theSecondAttach);
    }
  }
}

Point3d PrsDim1::TranslatePointToBound(const Point3d&  aPoint,
                                     const Dir3d&  aDir,
                                     const Box2& aBndBox)
{
  ...
  else
  {
    ...
    EnlargedBox.Enlarge(aBndBox.GetGap() + Precision1::Confusion());
...
Standard_Boolean PrsDim1::InDomain(const Standard_Real fpar,
...
Standard_Real PrsDim1::DistanceFromApex(const gp_Elips&     elips,
                                       const Point3d&       Apex,
...
Point3d PrsDim1::NearestApex(const gp_Elips&     elips,
                           const Point3d&       pApex,
                           const Point3d&       nApex,
...
void PrsDim1::ComputeProjEdgePresentation(const Handle(Prs3d_Presentation)& aPresentation,
                                         ...
                                         const Aspect_TypeOfLine           aCallTOL)
{
  ...
  if (ProjCurve->IsInstance(STANDARD_TYPE(GeomLine)))
  {
    // CLE
    // Handle(GeomLine) gl (Handle(GeomLine)::DownCast (ProjCurve));
    Handle(GeomLine) gl = Handle(GeomLine)::DownCast(ProjCurve);
    // ENDCLE
    if (!isInfinite)
    {
  ...
  else if (ProjCurve->IsInstance(STANDARD_TYPE(GeomCircle)))
  {
    // CLE
...
void PrsDim1::ComputeProjVertexPresentation(const Handle(Prs3d_Presentation)& aPresentation,
                                           const Handle(StyleDrawer)&       aDrawer,
                                           ...
                                           const Aspect_TypeOfLine           aCallTOL)
{
  ...
  if (!ProjPoint.IsEqual(BRepInspector::Pnt(aVertex), Precision1::Confusion()))
  {
    ...
    EdgeMaker MakEd(ProjPoint, BRepInspector::Pnt(aVertex));
