...
public:
  ...
  //! The distance is the length of a projection from the
  //! shape to the plane.
  Standard_EXPORT PrsDim_EqualDistanceRelation(const TopoShape&       aShape1,
  ...
  void SetShape3(const TopoShape& aShape) { myShape3 = aShape; }

  //! Returns the shape aShape3 from the framework
  //! created at construction time.
  const TopoShape& Shape3() const { return myShape3; }

  //! Sets the shape aShape to be used as the shape
  //! aShape4 in the framework created at construction time.
  void SetShape4(const TopoShape& aShape) { myShape4 = aShape; }

  //! Returns the shape aShape4 from the framework
  //! created at construction time.
  const TopoShape& Shape4() const { return myShape4; }
