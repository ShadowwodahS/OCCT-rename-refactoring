...
public:
  ...
  Standard_EXPORT void Init(const Handle(StepBasic_Action)& aActionAssignment_AssignedAction,
                            const Handle(HArray1OfWorkItem)& aItems);

  //! Returns field Items
  Standard_EXPORT Handle(HArray1OfWorkItem) Items() const;

  //! Set field Items
  Standard_EXPORT void SetItems(const Handle(HArray1OfWorkItem)& Items);

  DEFINE_STANDARD_RTTIEXT(StepAP203_StartWork, StepBasic_ActionAssignment)

protected:
private:
  Handle(HArray1OfWorkItem) theItems;
};
