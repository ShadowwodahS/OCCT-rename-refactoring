...
NumShapeIterator::NumShapeIterator()
...
void NumShapeIterator::Init(const SweepNumShape& aShape)
{
  myNumShape = aShape;
  if (myNumShape.Type() == TopAbs_EDGE)
  {
    ...
    if (myMore)
    {
      myCurrentRange = 1;
      myCurrentNumShape =
        SweepNumShape(1, TopAbs_VERTEX, myNumShape.Closed(), Standard_False, Standard_False);
      if (nbvert == 1)
      {
...
void NumShapeIterator::Next()
{
  ...
  if (myMore)
  {
    if (myNumShape.Type() == TopAbs_EDGE)
    {
      myCurrentNumShape    = SweepNumShape(myCurrentRange,
                                         TopAbs_VERTEX,
                                         myNumShape.Closed(),
                                         Standard_False,
                                         Standard_False);
