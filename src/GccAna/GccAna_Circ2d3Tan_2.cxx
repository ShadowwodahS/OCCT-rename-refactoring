  ...
  if (Bis1.IsDone() && Bis2.IsDone())
  {
    ...
    for (Standard_Integer i = 1; i <= nbsolution1; i++)
    {
      ...
      for (Standard_Integer k = 1; k <= nbsolution2; k++)
      {
        ...
        {
          Intp.Perform(Bis2.ThisSolution(k), Conic2d(Sol1->Parabola()));
        }
        if (Intp.IsDone())
        {
            ...
            for (Standard_Integer j = 1; j <= Intp.NbPoints(); j++)
            {
              ...
              if (ok)
              {
                for (Standard_Integer m = 1; m <= nbsol3; m++)
                {
                  ...
                  if (Center.Distance(center1) <= Tolerance && Abs(Radius(m) - R1) <= Tolerance)
                  {
                    TheSame1(NbrSol) = 1;
                  }
                  else
