        ...
        if (!Intp.IsEmpty())
        {
          for (Standard_Integer j = 1; j <= Intp.NbPoints(); j++)
          {
            ...
            if (Qualified1.IsEnclosed())
            {
              ...
              {
                if (Abs(Abs(R1 - dist1) - dist2) < Tolerance)
                {
                  ok = Standard_True;
                }
              }
            ...
            if (ok)
            {
              ...
              if (Center.Distance(center1) <= Tolerance && Abs(Radius - C1.Radius()) <= Tolerance)
              {
                TheSame1(NbrSol) = 1;
              }
              else
