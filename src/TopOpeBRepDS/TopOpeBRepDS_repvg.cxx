  ...
  while (it1.More())
  {
    ...
    while (it2.More())
    {
      ...
      if (!isComplex && memeS)
      {
        ...
        Standard_Boolean ok = FDS_stateEwithF2d(BDS, E, pE, GT1, G1, F1, TrmemeS);
        if (!ok)
        {
          it2.Next();
          continue;
...
//------------------------------------------------------
{
  ...
  for (tki.Init(); tki.More(); tki.Next()) {
    ...
    TopOpeBRepDS_ListOfInterference& loi = tki.ChangeValue(K,G); TopOpeBRepDS_ListOfInterference
  Rloi; Standard_Integer nloi = loi.Extent(); if      (nloi == 0) continue; else if (nloi == 1)
