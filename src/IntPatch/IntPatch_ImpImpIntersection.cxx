...
static Standard_Boolean FindLine(Point3d&                          Psurf,
                                 ...
                                 Point3d&                          thepointonarc,
                                 const Quadric1&           QuadSurf1,
                                 const Quadric1&           QuadSurf2,
...
Standard_Boolean IntersectionWithAnArc(Point3d&                          PSurf,
                                       ...
                                       const Standard_Real              u1alin)
{
  ...
  //-- Distance initiale
  {
    Point3d        pp0 = alin->Value(thetamin);
  ...
  do
  {
    ...
    Point3d pp0 = alin->Value(theta);
    Point3d pp1 = alin->Value(theta + du);
    QuadSurf.Parameters(pp0, ua0, va0);
                               ...
                               const Standard_Real                               theToler)
{
  ...
  Point3d                              Point = currentpointonrst.Value();
...
Standard_Boolean FindLine(Point3d&                          Psurf,
                          const IntPatch_SequenceOfLine&   slin,
                          const Standard_Real              Tol,
                          TColStd_ListOfReal&              theLParams,
                          Vector3d&                          Vtgtint,
                          ...
                          Standard_Real&                   theOutputToler)
{
  ...
  for (i = 1; i <= nblin; i++)
  {
    ...
    {
      case IntPatch_Lin: {
        para = ElCLib1::Parameter(Handle(IntPatch_GLine)::DownCast(lin)->Line(), Psurf);
        if (para <= upper && para >= lower)
        {
          pt      = ElCLib1::Value(para, Handle(IntPatch_GLine)::DownCast(lin)->Line());
      ...
      case IntPatch_Circle: {
        para = ElCLib1::Parameter(Handle(IntPatch_GLine)::DownCast(lin)->Circle(), Psurf);
        if ((para <= upper && para >= lower)
            || (para + 2. * M_PI <= upper && para + 2. * M_PI >= lower)
            || (para - 2. * M_PI <= upper && para - 2. * M_PI >= lower))
        {
          pt      = ElCLib1::Value(para, Handle(IntPatch_GLine)::DownCast(lin)->Circle());
          aSqDist = Psurf.SquareDistance(pt);
          if ((aSqDist < aSqTol) && (aSqDist < aSqDistMin))
          {
            aSqDistMin = aSqDist;
      ...
      case IntPatch_Ellipse: {
        para = ElCLib1::Parameter(Handle(IntPatch_GLine)::DownCast(lin)->Ellipse(), Psurf);
        if ((para <= upper && para >= lower)
            || (para + 2. * M_PI <= upper && para + 2. * M_PI >= lower)
            || (para - 2. * M_PI <= upper && para - 2. * M_PI >= lower))
        {
          pt      = ElCLib1::Value(para, Handle(IntPatch_GLine)::DownCast(lin)->Ellipse());
          aSqDist = Psurf.SquareDistance(pt);
          if ((aSqDist < aSqTol) && (aSqDist < aSqDistMin))
          {
            aSqDistMin = aSqDist;
...
#if 0 	
	para = ElCLib1::Parameter(Handle(IntPatch_GLine)::DownCast (lin)->Parabola(),Psurf);
	if (para <= upper && para >= lower) {
	  pt = ElCLib1::Value(para,Handle(IntPatch_GLine)::DownCast (lin)->Parabola());
	  ...
	  if (dist< distmin) {
...
#else
        //-- Le calcul du parametre sur une parabole est mal fait ds ElCLib1. Il ne tient pas compte
        ...
        //-- On ne peut pas modifier faciolement ds ElCLib1 car on ne passe pas la focale. ...
        const gp_Parab& Parab = Handle(IntPatch_GLine)::DownCast(lin)->Parabola();
        para                  = ElCLib1::Parameter(Parab, Psurf);
        if (para <= upper && para >= lower)
        {
          ...
          do
          {
            ...
            pt      = ElCLib1::Value(para, Parab);
            aSqDist = Psurf.SquareDistance(pt);

            const Point3d        ptbis   = ElCLib1::Value(parabis, Parab);
      ...
      case IntPatch_Hyperbola: {
        para = ElCLib1::Parameter(Handle(IntPatch_GLine)::DownCast(lin)->Hyperbola(), Psurf);
        if (para <= upper && para >= lower)
        {
          pt      = ElCLib1::Value(para, Handle(IntPatch_GLine)::DownCast(lin)->Hyperbola());
          aSqDist = Psurf.SquareDistance(pt);
          if ((aSqDist < aSqTol) && (aSqDist < aSqDistMin))
          {
            aSqDistMin = aSqDist;
  ...
  switch (typarc)
  {
    ...
    case IntPatch_Circle:
      theLParams.Append(aParaInt);
      Vtgtint = ElCLib1::DN(aParaInt, (*((Handle(IntPatch_GLine)*)&slin(theLineIdx)))->Circle(), 1);
      break;
    case IntPatch_Ellipse:
      theLParams.Append(aParaInt);
      Vtgtint = ElCLib1::DN(aParaInt, (*((Handle(IntPatch_GLine)*)&slin(theLineIdx)))->Ellipse(), 1);
      break;
    case IntPatch_Parabola:
      theLParams.Append(aParaInt);
      Vtgtint =
        ElCLib1::DN(aParaInt, (*((Handle(IntPatch_GLine)*)&slin(theLineIdx)))->Parabola(), 1);
      break;
    case IntPatch_Hyperbola:
      theLParams.Append(aParaInt);
      Vtgtint =
        ElCLib1::DN(aParaInt, (*((Handle(IntPatch_GLine)*)&slin(theLineIdx)))->Hyperbola(), 1);
      break;
    ...
    case IntPatch_Analytic: {
      if (!Handle(IntPatch_ALine)::DownCast(slin(theLineIdx))->D1(theLParams.Last(), pt, Vtgtint))
      {
        ...
        // must be processed by high-level algorithms (SpecialPoints class).
                            ...
                            Vector3d&                      Vtgtint)
{
  ...
  {
    case IntPatch_Lin:
      parproj = ElCLib1::Parameter(Handle(IntPatch_GLine)::DownCast(lin)->Line(), Psurf);
      ElCLib1::D1(parproj, Handle(IntPatch_GLine)::DownCast(lin)->Line(), ptproj, tgint);
      break;
    case IntPatch_Circle:
      parproj = ElCLib1::Parameter(Handle(IntPatch_GLine)::DownCast(lin)->Circle(), Psurf);
      ElCLib1::D1(parproj, Handle(IntPatch_GLine)::DownCast(lin)->Circle(), ptproj, tgint);
      break;
    case IntPatch_Ellipse:
      parproj = ElCLib1::Parameter(Handle(IntPatch_GLine)::DownCast(lin)->Ellipse(), Psurf);
      ElCLib1::D1(parproj, Handle(IntPatch_GLine)::DownCast(lin)->Ellipse(), ptproj, tgint);
      break;
    case IntPatch_Parabola:
      parproj = ElCLib1::Parameter(Handle(IntPatch_GLine)::DownCast(lin)->Parabola(), Psurf);
      ElCLib1::D1(parproj, Handle(IntPatch_GLine)::DownCast(lin)->Parabola(), ptproj, tgint);
      break;
    case IntPatch_Hyperbola:
      parproj = ElCLib1::Parameter(Handle(IntPatch_GLine)::DownCast(lin)->Hyperbola(), Psurf);
      ElCLib1::D1(parproj, Handle(IntPatch_GLine)::DownCast(lin)->Hyperbola(), ptproj, tgint);
      break;
    case IntPatch_Analytic: {
      Handle(IntPatch_ALine) alin(Handle(IntPatch_ALine)::DownCast(lin));
      TColStd_ListOfReal     aLParams;
                     ...
                     const Standard_Real                             TolArc)
{
  ...
  for (i = 1; i <= nbedg; i++)
  {
          ...
          if (EdgeDegenere == Standard_False && dofirst)
          {
            if (ptvtx.Value().Distance(PStartf.Value()) <= TolArc)
            {
              ...
              if (ptvtx.IsOnDomS1())
              {
                ...
                {
                  IntSurf1::MakeTransition(tgline, tgarc, norm1, TRest, TArc);
                }
              ...
              if (ptvtx.IsOnDomS2())
              {
                ...
                {
                  IntSurf1::MakeTransition(tgline, tgarc, norm2, TRest, TArc);
                }
    ...
    if (dofirst)
    {
      ...
      if (!PStartf.IsNew())
      {
        Transition2 Transline;
        Transition2 Transarc;
...
inline const Point3d& PointValue(const Handle(IntPatch_RLine) theRLine,
                                const Standard_Integer       theIndex)
{
  return theRLine->Point(theIndex).Value();
}

inline const Point3d& VertexValue(const Handle(IntPatch_RLine) theRLine,
                                 const Standard_Integer       theIndex)
{
...
static Standard_Boolean IsRLineGood(const Quadric1&       Quad1,
                                    const Quadric1&       Quad2,
                                    ...
                                    const Standard_Real          theTol)
{
  ...
  const Point3d& (*Value)(const Handle(IntPatch_RLine), const Standard_Integer);
  ...
  if (aNbPntsM1 == 1)
  {
    Point3d aP1(Value(theRLine, 1)), aP2(Value(theRLine, 2));
  ...
  {
    const Point3d        aP(Value(theRLine, i));
          ...
          for (k = 1; EdgeDegenere == Standard_False && k <= Nbvtx; k++)
          {
            ...
            if ((OnFirst && !Ptvtx.IsOnDomS1()) || (!OnFirst && !Ptvtx.IsOnDomS2()))
            {
              ...
              { //-- il faut projeter pour trouver le point sur la rline.
                ...
                project = HInterTool::Project(arcref, gp_Pnt2d(U, V), paramproj, p2d);
              ...
              if (keeppoint)
              {
                ...
                if (Ptvtx.IsOnDomS1() || Ptvtx.IsOnDomS2())
                {
                  ...
                  { // donc OnDomS2
                    ...
                    {
                      IntSurf1::MakeTransition(tgrest, tgarc, norm, TRest, TArc);
                    }
                    newptvtx.SetArc(Standard_False, thearc, Ptvtx.ParameterOnArc2(), TRest, TArc);
                  }
                  else
...
static Standard_Boolean IntPP(const Quadric1&,
                              const Quadric1&,
                              const Standard_Real,
                              const Standard_Real,
                              Standard_Boolean&,
...
static Standard_Boolean IntPCy(const Quadric1&,
                               const Quadric1&,
                               const Standard_Real,
                               const Standard_Real,
                               const Standard_Boolean,
...
static Standard_Boolean IntPSp(const Quadric1&,
                               const Quadric1&,
...
static Standard_Boolean IntPCo(const Quadric1&,
                               const Quadric1&,
                               const Standard_Real,
                               const Standard_Real,
                               const Standard_Boolean,
...
static Standard_Boolean IntCySp(const Quadric1&,
                                const Quadric1&,
                                const Standard_Real,
                                const Standard_Boolean,
                                Standard_Boolean&,
...
static Standard_Boolean IntCyCo(const Quadric1&,
                                const Quadric1&,
                                const Standard_Real,
                                const Standard_Boolean,
                                Standard_Boolean&,
...
static Standard_Boolean IntSpSp(const Quadric1&,
                                const Quadric1&,
                                const Standard_Real,
                                Standard_Boolean&,
                                Standard_Boolean&,
...
static Standard_Boolean IntCoSp(const Quadric1&,
                                const Quadric1&,
                                const Standard_Real,
                                const Standard_Boolean,
                                Standard_Boolean&,
...
static Standard_Boolean IntCoCo(const Quadric1&,
                                const Quadric1&,
                                const Standard_Real,
                                Standard_Boolean&,
                                Standard_Boolean&,
...
// torus
static Standard_Boolean IntPTo(const Quadric1&,
                               const Quadric1&,
                               const Standard_Real,
                               const Standard_Boolean,
                               Standard_Boolean&,
...
static Standard_Boolean IntCyTo(const Quadric1&,
                                const Quadric1&,
                                const Standard_Real,
                                const Standard_Boolean,
                                Standard_Boolean&,
...
static Standard_Boolean IntCoTo(const Quadric1&,
                                const Quadric1&,
                                const Standard_Real,
                                const Standard_Boolean,
                                Standard_Boolean&,
...
static Standard_Boolean IntSpTo(const Quadric1&,
                                const Quadric1&,
                                const Standard_Real,
                                const Standard_Boolean,
                                Standard_Boolean&,
...
static Standard_Boolean IntToTo(const Quadric1&,
                                const Quadric1&,
                                const Standard_Real,
                                Standard_Boolean&,
                                Standard_Boolean&,
  ...
  if (isPostProcessingRequired)
  {
    ...
    if (SameSurf || (all1 && all2))
    {
      ...
      switch (typs1)
      {
        ...
        break;
        case GeomAbs_Cylinder: {
          Ptreference = ElSLib1::Value(0., 0., S1->Cylinder());
        }
        break;
        case GeomAbs_Sphere: {
          Ptreference = ElSLib1::Value(M_PI / 4., M_PI / 4., S1->Sphere());
        }
        break;
        case GeomAbs_Cone: {
          Ptreference = ElSLib1::Value(0., 10., S1->Cone());
        }
        break;
        case GeomAbs_Torus: {
          Ptreference = ElSLib1::Value(0., 0., S1->Torus());
        }
        break;
        default:
          break;
...
//=======================================================================
Standard_Boolean IntPP(const Quadric1&   Quad1,
                       const Quadric1&   Quad2,
...
//=======================================================================
Standard_Boolean IntPCy(const Quadric1&   Quad1,
                        const Quadric1&   Quad2,
                        const Standard_Real      Tolang,
                        const Standard_Real      TolTang,
...

{
  ...
  switch (typint)
  {
    ...
    case IntAna_Ellipse: {
      ...
      Point3d   ptref;
      Vector3d   Tgt;
      ElCLib1::D1(0., elipsol, ptref, Tgt);
...
//=======================================================================
Standard_Boolean IntPSp(const Quadric1& Quad1,
                        const Quadric1& Quad2,
...
//=======================================================================
Standard_Boolean IntPCo(const Quadric1&    Quad1,
                        const Quadric1&    Quad2,
                        const Standard_Real       Tolang,
                        const Standard_Real       TolTang,
                        const Standard_Boolean    Reversed,
...

{
  ...
  switch (typint)
  {
    ...
    case IntAna_Line: {
      ...
      Standard_Real para = ElCLib1::Parameter(linsol, apex);
      Point3d        ptbid(ElCLib1::Value(para + 5., linsol));
      Standard_Real U1, V1, U2, V2;
      ...
      else
      {
        ...
        linsol.SetDirection(linsol.Direction().Reversed());
        glig = new IntPatch_GLine(linsol, Standard_False, trans1, trans2);
        para = ElCLib1::Parameter(linsol, apex);
        ptsol.SetParameter(para);
        glig->AddVertex(ptsol);
        glig->SetFirstPoint(1);
        slin.Append(glig);
        ...
        para  = ElCLib1::Parameter(linsol, apex);
        ptbid = ElCLib1::Value(para + 5., linsol);
        if (linsol.Direction().DotCross(Quad2.Normale(ptbid), Quad1.Normale(ptbid)) > 0.)
        {
          trans1 = IntSurf_Out;
          trans2 = IntSurf_In;
    ...
    case IntAna_Hyperbola: {
      ...
      for (Standard_Integer i = 1; i <= 2; i++)
      {
        ...
        tophypr           = ElCLib1::Value(hyprsol.MajorRadius(), hyprsol.XAxis());
...
Standard_Boolean IntPTo(const Quadric1&   theQuad1,
                        const Quadric1&   theQuad2,
...
public:
  ...
  WorkWithBoundaries(const Quadric1&                   theQuad1,
                     const Quadric1&                   theQuad2,
  ...
  const Quadric1& GetQSurface(const Standard_Integer theIdx) const
  {
    if (theIdx <= 1)
      return myQuad1;
...
private:
  ...
  const Quadric1&                   myQuad1;
  const Quadric1&                   myQuad2;
...
//          on correspond curve (see correspond method for ExtElC class).
//=======================================================================
...
void ProcessBounds(const Handle(IntPatch_ALine)&  alig, //-- ligne courante
                   ...
                   const Point3d&                  ptf,   //-- Debut Ligne Courante
                   const Standard_Real            first, //-- Paramf
                   Standard_Boolean&              procl,
                   const Point3d&                  ptl,  //-- Fin Ligne courante
...
//=======================================================================
Standard_Boolean CyCyAnalyticalIntersect(const Quadric1&    Quad1,
                                         const Quadric1&    Quad2,
                                         const QuadQuadGeoIntersection& theInter,
                                         const Standard_Real       Tol,
                                         Standard_Boolean&         Empty,
                                         Standard_Boolean&         Same,
                                         Standard_Boolean&         Multpoint,
                                         ...
                                         IntPatch_SequenceOfPoint& spnt)
{
  ...
  switch (typint)
  {
    ...
    case IntAna_Ellipse: {
      ...
      Point3d pttang1(ElCLib1::Value(0.5 * M_PI, elipsol));
      Point3d pttang2(ElCLib1::Value(1.5 * M_PI, elipsol));
      ...
      //
      {
        Standard_Real  aU1, aV1, aU2, aV2;
        IntPatch_Point aIP;
        Point3d         aP(ElCLib1::Value(0., elipsol));
        //
      ...
      Standard_Real param1              = ElCLib1::Parameter(elipsol, pttang1);
      Standard_Real param2              = ElCLib1::Parameter(elipsol, pttang2);
      ...
      ElCLib1::D1(parampourtransition, elipsol, ptref, Tgt);
                               ...
                               const Standard_Boolean theFlForce)
{
  ...
  theUGiven = ElCLib1::InPeriod(theUGiven, aUf, aUl);
...
//=======================================================================
static Standard_Boolean AddPointIntoWL(const Quadric1&                   theQuad1,
                                       const Quadric1&                   theQuad2,
                                       const ComputationMethods::stCoeffsValue& theCoeffs,
                                       const Standard_Boolean                   isTheReverse,
                                       ...
                                       const Standard_Boolean theOnlyCheck = Standard_False)
{
  ...
  Point3d aPt1(theQuad1.Value(thePntOnSurf1.X(), thePntOnSurf1.Y())),
  ...
  if (aNbPnts > 0)
  {
    ...
    const PointOn2Surfaces aPlast = theLine->Value(aNbPnts);
    if (isTheReverse)
                                 ...
                                 const Standard_Boolean                   isTheReverse)
{
  ...
  do
  {
    ...
    for (Standard_Integer fp = theStartPointOnLine, lp = 0; fp < aLastPointIndex; fp = lp + 1)
    {
      ...
      const Point3d aP1(theQuad1.Value(U1prec, V1prec)), aP2(theQuad2.Value(U2prec, V2prec));
                                            ...
                                            Range1&         theOutBoxS2) const
{
  ...
  theOutBoxS1.Common(Range1(aV1, aV2));

  myUVSurf2.Get(aU1, aV1, aU2, aV2);
  theOutBoxS2.Common(Range1(aV1, aV2));
}
  ...
  IntPatch_SequenceOfPoint& theSPnt)
{
  ...
  const Quadric1&                   aQuad1           = theBW.GetQSurface(1);
  const Quadric1&                   aQuad2           = theBW.GetQSurface(2);
  ...
  for (Standard_Integer aNbPnt = 1; aNbPnt <= theSPnt.Length(); aNbPnt++)
  {
    for (Standard_Integer aNbLin = 1; aNbLin <= theSlin.Length(); aNbLin++)
    {
      ...
      const PointOn2Surfaces& aPntFWL1 = aWLine1->Point(1);
      const PointOn2Surfaces& aPntLWL1 = aWLine1->Point(aWLine1->NbPnts());

      const PointOn2Surfaces aPntCur = theSPnt.Value(aNbPnt).PntOn2S();
                                               ...
                                               IntPatch_SequenceOfPoint& theSPnt)
{
  ...
  {
    anAlloc->Reset(false);
    NCollection_List<Range1> aListOfRng(anAlloc);
    ...
    NCollection_List<Range1>::Iterator anITrRng;
    for (Standard_Integer aSInd = 0; aSInd < 3; aSInd++)
    {
      NCollection_List<Range1> aLstTemp(aListOfRng);
      aListOfRng.Clear();
      for (anITrRng.Init(aLstTemp); anITrRng.More(); anITrRng.Next())
      {
        Range1& aRng = anITrRng.ChangeValue();
    ...
    {
      Range1& aCurrRange = anITrRng.ChangeValue();
...
Standard_Boolean IntCySp(const Quadric1&    Quad1,
                         const Quadric1&    Quad2,
                         const Standard_Real       Tol,
                         const Standard_Boolean    Reversed,
                         Standard_Boolean&         Empty,
...

{
  ...
  QuadQuadGeoIntersection inter(Cy, Sp, Tol);
  ...
  switch (typint)
  {
    ...
    case IntAna_NoGeometricSolution: {
      Point3d             psol;
      Standard_Real      U1, V1, U2, V2;
      QuadQuadIntersection anaint(Cy, Sp, Tol);
      if (!anaint.IsDone())
      {
        return Standard_False;
      }
...
Standard_Boolean IntCyCo(const Quadric1&    Quad1,
                         const Quadric1&    Quad2,
                         const Standard_Real       Tol,
                         const Standard_Boolean    Reversed,
                         Standard_Boolean&         Empty,
...

{
  ...
  QuadQuadGeoIntersection inter(Cy, Co, Tol);
  ...
  switch (typint)
  {
    ...
    case IntAna_NoGeometricSolution: {
      Point3d             psol;
      Standard_Real      U1, V1, U2, V2;
      QuadQuadIntersection anaint(Cy, Co, Tol);
      if (!anaint.IsDone())
      {
        return Standard_False;
      }
      ...
      else
      {
        ...
        for (i = 1; i <= NbSol; ++i)
        {
          ...
          {
            Curve1& curvsol = aIt.ChangeValue();
            //
            curvsol.Domain(first, last);
...
{
  const Standard_Real aSqTol = theTol * theTol;
  const Point3d        aPapx(theCo.Apex());
  ...
  for (TColStd_ListIteratorOfListOfReal anItr(aLParams); anItr.More(); anItr.Next())
  {
    ...
    const Point3d        aP   = theCrv.Value(aPrm);
...
Standard_Boolean IntCoCo(const Quadric1&    Quad1,
                         const Quadric1&    Quad2,
                         const Standard_Real       Tol,
                         Standard_Boolean&         Empty,
                         Standard_Boolean&         Same,
...

{
  ...
  switch (typint)
  {
    ...
    case IntAna_Line: {
      ...
      if (NbSol == 1)
      {
        ...
        //
        linsol = inter.Line(1);
        para   = ElCLib1::Parameter(linsol, aPApex1);
        ptbid  = ElCLib1::Value(para + 5., linsol);
    ...
    case IntAna_Point: {
      Point3d        ptcontact;
      Point3d        apex1(Co1.Apex());
      Point3d        apex2(Co2.Apex());
      Standard_Real param1, param2;
      Standard_Real paramapex1 = ElCLib1::LineParameter(Co1.Axis(), apex1);
      Standard_Real paramapex2 = ElCLib1::LineParameter(Co2.Axis(), apex2);
      for (i = 1; i <= NbSol; i++)
      {
        ptcontact = inter.Point(i);
        param1    = ElCLib1::LineParameter(Co1.Axis(), ptcontact);
        param2    = ElCLib1::LineParameter(Co2.Axis(), ptcontact);
    ...
    case IntAna_Circle: {
      ...
      for (i = 1; i <= NbSol; i++)
      {
        ...
        {
          const Point3d& aPChar = inter.PChar();
    ...
    case IntAna_Hyperbola: {
      ...
      {
        gp_Hypr hyprsol = inter.Hyperbola(i);
        ElCLib1::D1(0., hyprsol, ptref, Tgt);
        Standard_Real qwe = Tgt.DotCross(Quad2.Normale(ptref), Quad1.Normale(ptref));
        if (qwe > 0.00000001)
        {
          trans1 = IntSurf_Out;
    ...
    case IntAna_NoGeometricSolution: {
      Point3d             psol;
      QuadQuadIntersection anaint(Co1, Co2, Tol);
      if (!anaint.IsDone())
      {
        return Standard_False;
      }
  ...
  if (inter.HasCommonGen())
  {
    ...
    aPtsol.SetValue(aPChar, Tol, Standard_False);
    aPtsol.SetParameters(U1, V1, U2, V2);
    para = ElCLib1::Parameter(linsol, aPChar);
    aPtsol.SetParameter(para);
    glig->AddVertex(aPtsol);
...
Standard_Boolean IntCoSp(const Quadric1&    Quad1,
                         const Quadric1&    Quad2,
                         const Standard_Real       Tol,
                         const Standard_Boolean    Reversed,
                         Standard_Boolean&         Empty,
...

{
  ...
  QuadQuadGeoIntersection inter(Sp, Co, Tol);
  ...
  switch (typint)
  {
    ...
    case IntAna_Point: {
      ...
      Standard_Real param;
      Standard_Real paramapex = ElCLib1::LineParameter(Co.Axis(), apex);
      for (i = 1; i <= NbSol; i++)
      {
        ptcontact = inter.Point(i);
        param     = ElCLib1::LineParameter(Co.Axis(), ptcontact);
        Quad1.Parameters(ptcontact, U1, V1);
        Quad2.Parameters(ptcontact, U2, V2);
    ...
    case IntAna_Circle: {
      ...
      {
        gp_Circ cirsol = inter.Circle(i);
        //-- param = ElCLib1::LineParameter(Co.Axis(),
        //-- 			      cirsol.Location());
        //-- if (param >= paramapex) {
    ...
    case IntAna_PointAndCircle: {
      ...
      gp_Circ cirsol = inter.Circle(1);
      param          = ElCLib1::LineParameter(Co.Axis(), cirsol.Location());
      ElCLib1::D1(0., cirsol, ptref, Tgt);
      Standard_Real qwe = Tgt.DotCross(Quad2.Normale(ptref), Quad1.Normale(ptref));
    ...
    case IntAna_NoGeometricSolution: {
      Point3d             psol;
      QuadQuadIntersection anaint(Co, Sp, Tol);
      if (!anaint.IsDone())
      {
        return Standard_False;
      }
      ...
      else
      {
        ...
        for (i = 1; i <= NbSol; i++)
        {
          ...
          if (kount <= 5)
          {
            para              = ElCLib1::LineParameter(Co.Axis(), ptvalid);
            Standard_Real qwe = tgvalid.DotCross(Quad2.Normale(ptvalid), Quad1.Normale(ptvalid));
            if (qwe > 0.000000001)
            {
              trans1 = IntSurf_Out;
...
Standard_Boolean IntSpSp(const Quadric1&    Quad1,
                         const Quadric1&    Quad2,
                         const Standard_Real       Tol,
                         Standard_Boolean&         Empty,
                         Standard_Boolean&         Same,
...
static Standard_Boolean TreatResultTorus(const Quadric1&    theQuad1,
                                         const Quadric1&    theQuad2,
                                         const QuadQuadGeoIntersection& anInt,
                                         Standard_Boolean&         bEmpty,
                                         IntPatch_SequenceOfLine&  theSeqLin);

//=================================================================================================

Standard_Boolean IntCyTo(const Quadric1&   theQuad1,
                         const Quadric1&   theQuad2,
                         const Standard_Real      theTolTang,
                         const Standard_Boolean   bReversed,
                         Standard_Boolean&        bEmpty,
...
Standard_Boolean IntCoTo(const Quadric1&   theQuad1,
                         const Quadric1&   theQuad2,
                         const Standard_Real      theTolTang,
                         const Standard_Boolean   bReversed,
                         Standard_Boolean&        bEmpty,
...
Standard_Boolean IntSpTo(const Quadric1&   theQuad1,
                         const Quadric1&   theQuad2,
                         const Standard_Real      theTolTang,
                         const Standard_Boolean   bReversed,
                         Standard_Boolean&        bEmpty,
...
Standard_Boolean IntToTo(const Quadric1&   theQuad1,
                         const Quadric1&   theQuad2,
                         const Standard_Real      theTolTang,
                         ...
                         IntPatch_SequenceOfLine& theSeqLin)
{
  ...
  //
  QuadQuadGeoIntersection anInt(aTorus1, aTorus2, theTolTang);
