...
Standard_Boolean Express_Entity::GenerateClass() const
{
  ...
  AsciiString1 anInheritName;
  {
    ...
    for (Standard_Integer i = 2; i <= myInherit->Length(); i++)
    {
      ...
      anOS << "  //! Returns data for supertype " << aName
           ...
           << anEntityCPPName << ") " << aName
           << "() const;\n"
              "  //! Sets data for supertype "
           << aName
           << "\n"
              "  Standard_EXPORT void Set"
           << aName << " (const Handle(" << anEntityCPPName << ")& the" << aName
           << ");\n"
              "\n";
    }
    for (Standard_Integer i = 1; i <= myFields->Length(); i++)
    {
      ...
      {
        anOS << "  Standard_EXPORT Handle(" << aFieldCPPName << ") " << aField->Name()
             << "() const;\n";
      }
      else
      {
      ...
      if (aFieldType->IsHandle())
      {
        anOS << "  Standard_EXPORT void Set" << aField->Name() << " (const Handle(" << aFieldCPPName
             << ")& the" << aField->Name() << ");\n";
      }
      else
      {
        anOS << "  Standard_EXPORT void Set" << aField->Name() << " (const " << aFieldCPPName
             << (aFieldType->IsSimple() ? "" : "&") << " the" << aField->Name() << ");\n";
      }
      if (aField->IsOptional())
      {
        anOS << "\n"
                "  //! Returns True if optional field "
             << aField->Name()
             << " is defined\n"
                "  Standard_EXPORT Standard_Boolean Has"
             << aField->Name() << "() const;\n";
      }
      anOS << "\n";

      // prepare additional methods for HArray1 field (NbField() and <field_elem> FieldValue(int
      // num))
      ...
      if (aFindPos > -1)
      {
        ...
        anOS << "  //! Returns number of " << aField->Name()
             << "\n"
                "  Standard_EXPORT Standard_Integer Nb"
             << aField->Name()
             << "() const;\n"
                "\n"
                "  //! Returns value of "
  ...
  AsciiString1 aRWCPPName;
  {
    ...
    // share
    anOS << "  Standard_EXPORT void Share (const Handle(" << aCPPName
         << ")& theEnt,\n"
  ...
  // Step 4: generating CXX for Read/Write tool
  {
    ...
    if (aRWCPPName.Length() < 40)
    {
      anOS << "void " << aRWCPPName << "::Share (const Handle(" << aCPPName << ")&"
           << ((aNnFileds2 > 1) ? "theEnt," : ",") << "\n";
    ...
    if (CheckFlag())
    {
      ...
      anOS << "  Handle(" << aCPPName << ") ent = Handle(" << aCPPName
           << ")::DownCast(entt);\n"
              "}\n";
    }
    if (FillSharedFlag())
    ...
    if (aFindPos > -1)
    {
      ...
      // create hxx files for Array1
      {
        ...
        anOS << "\n"
             ...
             << aPack << "_" << aNameClass
             << ".hxx>\n"
                "#include <NCollection_"
             << aNameHA.SubString(aFindPos + 3, aFindPos + 8)
             << ".hxx>\n"
                "\n"
                "typedef NCollection_"
             << aNameHA.SubString(aFindPos + 3, aFindPos + 8) << "<Handle(" << aPack << "_"
             << aNameClass << ")> " << aNameA
             << ";\n"
                "\n"
                "#endif\n";
      ...
      // create hxx files for HArray1
      {
        ...
        anOS << "\n"
             ...
             << ".hxx>\n"
                "#include <NCollection_"
             << aNameHA.SubString(aFindPos + 2, aFindPos + 8)
             << ".hxx>\n"
                "\n"
                "DEFINE_HARRAY"
             << aNameHA.SubString(aFindPos + 8, aFindPos + 8) << "(" << aNameHA << ", " << aNameA
             << ");\n"
                "\n"
                "#endif\n";
        aStreamPtr.reset();
                             ...
                             const Standard_Boolean         isOptional)
{
  ...
  if (aType->IsKind(STANDARD_TYPE(Express_NamedType)))
  {
    ...
    {
      theOS << anIdent << "theData->ReadEntity (" << aParam << ", " << theIndex << ", \""
            << theSTEPName << "\", theCheck,\n"
    ...
    {
      theOS << anIdent << "theData->ReadEntity (" << aParam << ", " << theIndex << ", \""
            << theSTEPName << "\", theCheck, a" << theVarName << ");\n";
    }
    else if (aNamedType->Item()->IsKind(STANDARD_TYPE(Express_Enum)))
    {
      theOS << anIdent << "if (theData->ParamType(" << aParam << ", " << theIndex
            << ") == Interface_ParamEnum)\n"
            << anIdent << "{\n"
      ...
      for (Standard_Integer i = 1; i <= aNames->Length(); i++)
      {
        ...
        theOS << anIdent << (i == 1 ? "  if     " : "  else if") << " (strcmp (text, \"."
              ...
              << aNames->Value(i)->String() << ";\n";
      }
      theOS << anIdent << "  else\n"
            << anIdent << "  {\n"
            << anIdent << "    theCheck->AddFail (\"Parameter #" << theIndex << " (" << theSTEPName
  ...
  else if (aType->IsKind(STANDARD_TYPE(Express_ComplexType)))
  {
    ...
    theOS << anIdent << "Standard_Integer aSub" << theIndex << " = 0;\n"
          << anIdent << "if (theData->ReadSubList (" << aParam << ", " << theIndex << ", \""
          << theSTEPName << "\", theCheck, aSub" << theIndex << "))\n"
          << anIdent << "{\n"
    ...
    if (aComplex->Type()->IsKind(STANDARD_TYPE(Express_ComplexType)))
    {
      ...
      theOS << anIdent << "  Standard_Integer nbj" << theLevel
            ...
            << anIdent << "  a" << theVarName << " = new " << aTypeCPPName << "(1, aNb" << theLevel
            << ", 1, nbj" << theLevel << ");\n"
            << anIdent << "  for (Standard_Integer " << anIterI << " = 1; " << anIterI << " <= aNb"
            << theLevel << "; " << anIterI << "++)\n"
            << anIdent << "  {\n"
            << anIdent << "    Standard_Integer subj" << theIndex << " = 0;\n"
            << anIdent << "    if ( theData->ReadSubList (aSub" << theIndex << ", " << anIterI
            << ", \"sub-part(" << theSTEPName << ")\", theCheck, subj" << theIndex << ") ) {\n"
      ...
      writeRWReadField(theOS,
                       ...
                       aLevel + 2,
                       Standard_False);
      theOS << anIdent << "        a" << theVarName << "->SetValue(" << anIterI << "," << anIterJ
            << ", a" << aVar << ");\n"
            << anIdent << "      }\n"
            << anIdent << "    }\n";
    }
  ...
  else if (aType->IsKind(STANDARD_TYPE(Express_String)))
  {
    ...
    {
      theOS << anIdent << "theData->ReadString (" << aParam << ", " << theIndex << ", \""
            << theSTEPName << "\", theCheck, a" << theVarName << ");\n";
    }
    else
    {
  ...
  {
    theOS << anIdent << "theData->ReadLogical (" << aParam << ", " << theIndex << ", \""
          << theSTEPName << "\", theCheck, a" << theVarName << ");\n";
  }
  else if (aType->IsKind(STANDARD_TYPE(Express_Boolean)))
  {
    theOS << anIdent << "theData->ReadBoolean (" << aParam << ", " << theIndex << ", \""
          << theSTEPName << "\", theCheck, a" << theVarName << ");\n";
  }
  else if (aType->IsKind(STANDARD_TYPE(Express_Number))
           || aType->IsKind(STANDARD_TYPE(Express_Integer)))
  {
    theOS << anIdent << "theData->ReadInteger (" << aParam << ", " << theIndex << ", \""
          << theSTEPName << "\", theCheck, a" << theVarName << ");\n";
  }
  else if (aType->IsKind(STANDARD_TYPE(Express_Real)))
  {
    theOS << anIdent << "theData->ReadReal (" << aParam << ", " << theIndex << ", \"" << theSTEPName
  ...
  if (aType->IsKind(STANDARD_TYPE(Express_ComplexType)))
  {
    ...
    if (aType->IsKind(STANDARD_TYPE(Express_ComplexType)))
    {
      ...
      theOS << theVarName << "->RowLength(); i" << theIndex << "++)\n"
            ...
            << anIdent << "  for (Standard_Integer j" << theIndex << " = 1; j" << theIndex
            << " <= " << theVarName << "->ColLength(); j" << theIndex << "++)\n"
            << anIdent << "  {\n"
            << anIdent << "  " << (aType->IsHandle() ? "  Handle(" : "  ") << aType->CPPName()
            << (aType->IsHandle() ? ") " : " ") << aVar << " = " << theVarName << "->Value (i"
            << theIndex << ", j" << theIndex << ");\n";
                                          ...
                                          const Standard_Integer         theLevel)
{
  ...
  if (aType->IsKind(STANDARD_TYPE(Express_ComplexType)))
  {
    ...
    theOS << anIdent << "for (Standard_Integer i" << theIndex << " = 1; i" << theIndex
          << " <= " << theVarName << "->Length(); i" << theIndex << "++)\n"
          << anIdent << "{\n"
          << anIdent << (aType->IsHandle() ? "  Handle(" : "  ") << aType->CPPName()
          << (aType->IsHandle() ? ") " : " ") << aVar << " = " << theVarName << "->Value (i"
          << theIndex << ");\n";
                                                  ...
                                                  const Standard_Integer theOwn) const
{
  ...
  for (Standard_Integer i = 1; i <= myFields->Length(); i++)
  {
    ...
    std::ostringstream aStringOS;
    if (!writeRWShareField(aStringOS,
                           aVarName,
                           aField->Type(),
                           aNum,
                           (aField->IsOptional() ? 1 : 0)))
    {
      continue;
    }
    aNum++;
                                          ...
                                          const Standard_Integer theMode) const
{
  ...
  for (Standard_Integer i = 1; i <= myFields->Length(); i++)
  {
    ...
    {
      theOS << "const " << (aFieldType->IsHandle() ? "Handle(" : "") << aFieldType->CPPName()
            << (aFieldType->IsHandle() ? ")" : "") << (aFieldType->IsSimple() ? " the" : "& the")
            << aVarName;
    }
    else if (theMode == 2)
    {
