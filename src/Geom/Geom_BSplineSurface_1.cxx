  ...
  switch (Usmooth)
  {
    ...
    case GeomAbs_C3:
      return N <= 3 ? Standard_True
                    : N <= udeg
                             - BSplCLib1::MaxKnotMult(umults->Array1(),
                                                     umults->Lower() + 1,
                                                     umults->Upper() - 1);
    default:
...
Standard_Boolean Geom_BSplineSurface::IsCNv(const Standard_Integer N) const
{
  ...
  switch (Vsmooth)
  {
    ...
    case GeomAbs_C3:
      return N <= 3 ? Standard_True
                    : N <= vdeg
                             - BSplCLib1::MaxKnotMult(vmults->Array1(),
                                                     vmults->Lower() + 1,
                                                     vmults->Upper() - 1);
    default:
      return Standard_False;
                             ...
                             Vector3d&             D1V) const
{
  ...
  BSplCLib1::LocateParameter(udeg, uknots->Array1(), &umults->Array1(), U, uperiodic, uindex, aNewU);
  uindex = BSplCLib1::FlatIndex(udeg, uindex, umults->Array1(), uperiodic);

  BSplCLib1::LocateParameter(vdeg, vknots->Array1(), &vmults->Array1(), V, vperiodic, vindex, aNewV);
  vindex = BSplCLib1::FlatIndex(vdeg, vindex, vmults->Array1(), vperiodic);
...
Vector3d Geom_BSplineSurface::DN(const Standard_Real    U,
                               const Standard_Real    V,
                                  ...
                                  Point3d&                P) const
{
  ...
  BSplCLib1::LocateParameter(udeg, UFKNOTS, U, uperiodic, FromUK1, ToUK2, uindex, u);
  uindex = BSplCLib1::FlatIndex(udeg, uindex, umults->Array1(), uperiodic);

  BSplCLib1::LocateParameter(vdeg, VFKNOTS, V, vperiodic, FromVK1, ToVK2, vindex, v);
  vindex = BSplCLib1::FlatIndex(vdeg, vindex, vmults->Array1(), vperiodic);

  //  BSplSLib1::D0(U,V,uindex,vindex,POLES,WEIGHTS,UFKNOTS,VFKNOTS,FMULTS,FMULTS,
  BSplSLib1::D0(u,
               v,
               uindex,
               vindex,
...
Vector3d Geom_BSplineSurface::LocalDN(const Standard_Real    U,
                                    const Standard_Real    V,
                                    const Standard_Integer FromUK1,
                                    const Standard_Integer ToUK2,
                                    const Standard_Integer FromVK1,
...
void Geom_BSplineSurface::Transform(const Transform3d& T)
{
...
void Geom_BSplineSurface::SetUPeriodic()
{
  ...
  Standard_Integer nbp = BSplCLib1::NbPoles(udeg, Standard_True, cmults);
  ...
  weights = new TColStd_HArray2OfReal(1, nbp, cweights.LowerCol(), cweights.UpperCol());
...
void Geom_BSplineSurface::SetVPeriodic()
{
  ...
  // compute new number of poles;
  Standard_Integer nbp = BSplCLib1::NbPoles(vdeg, Standard_True, cmults);
  ...
  if (urational || vrational)
  {
    TColStd_Array2OfReal cweights(weights->LowerRow(), weights->UpperRow(), 1, nbp);
    ...
    weights = new TColStd_HArray2OfReal(cweights.LowerRow(), cweights.UpperRow(), 1, nbp);
    weights->ChangeArray2() = cweights;
  }
...
void Geom_BSplineSurface::SetUNotPeriodic()
{
  if (uperiodic)
  {
    ...
    Handle(TColStd_HArray2OfReal) nweights =
      new TColStd_HArray2OfReal(1, NbPoles, 1, poles->RowLength(), 0);

    if (urational || vrational)
    {

      BSplSLib1::Unperiodize(Standard_True,
                            udeg,
                            umults->Array1(),
                            uknots->Array1(),
                            poles->Array2(),
...
void Geom_BSplineSurface::SetVNotPeriodic()
{
  if (vperiodic)
  {
    ...
    Handle(TColStd_HArray2OfReal) nweights =
      new TColStd_HArray2OfReal(1, poles->ColLength(), 1, NbPoles, 0);

    if (urational || vrational)
    {

      BSplSLib1::Unperiodize(Standard_False,
                            vdeg,
                            vmults->Array1(),
                            vknots->Array1(),
                            poles->Array2(),
...
Standard_Integer Geom_BSplineSurface::FirstUKnotIndex() const
{
  if (uperiodic)
    ...
    return BSplCLib1::FirstUKnotIndex(udeg, umults->Array1());
}
...
Standard_Integer Geom_BSplineSurface::FirstVKnotIndex() const
{
  if (vperiodic)
    ...
    return BSplCLib1::FirstUKnotIndex(vdeg, vmults->Array1());
}
...
Standard_Integer Geom_BSplineSurface::LastUKnotIndex() const
{
  if (uperiodic)
    ...
    return BSplCLib1::LastUKnotIndex(udeg, umults->Array1());
}
...
Standard_Integer Geom_BSplineSurface::LastVKnotIndex() const
{
  if (vperiodic)
    ...
    return BSplCLib1::LastUKnotIndex(vdeg, vmults->Array1());
}
...
{
  BSplCLib1::Reverse(umults->ChangeArray1());
  BSplCLib1::Reverse(uknots->ChangeArray1());
  Standard_Integer last;
  if (uperiodic)
    last = ufknots->Upper() - udeg - 1;
  else
    last = poles->UpperRow();
  BSplSLib1::Reverse(poles->ChangeArray2(), last, Standard_True);
  if (urational || vrational)
    BSplSLib1::Reverse(weights->ChangeArray2(), last, Standard_True);
  UpdateUKnots();
}

//=================================================================================================

Standard_Real Geom_BSplineSurface::UReversedParameter(const Standard_Real U) const
{
...
{
  BSplCLib1::Reverse(vmults->ChangeArray1());
  BSplCLib1::Reverse(vknots->ChangeArray1());
  Standard_Integer last;
  if (vperiodic)
    last = vfknots->Upper() - vdeg - 1;
  else
    last = poles->UpperCol();
  BSplSLib1::Reverse(poles->ChangeArray2(), last, Standard_False);
  if (urational || vrational)
    BSplSLib1::Reverse(weights->ChangeArray2(), last, Standard_False);
  UpdateVKnots();
}

//=================================================================================================

Standard_Real Geom_BSplineSurface::VReversedParameter(const Standard_Real V) const
{
                                       ...
                                       const Standard_Boolean         Add)
{
  ...
  if (!BSplCLib1::PrepareInsertKnots(udeg,
                                    uperiodic,
                                    uknots->Array1(),
                                    umults->Array1(),
                                    Knots,
  ...
  if (urational || vrational)
  {
    BSplSLib1::InsertKnots(Standard_True,
                          udeg,
                          uperiodic,
                          poles->Array2(),
                          &weights->Array2(),
                                       ...
                                       const Standard_Boolean         Add)
{
  ...
  if (!BSplCLib1::PrepareInsertKnots(vdeg,
                                    vperiodic,
                                    vknots->Array1(),
                                    vmults->Array1(),
                                    Knots,
  ...
  if (urational || vrational)
  {
    BSplSLib1::InsertKnots(Standard_False,
                          vdeg,
                          vperiodic,
                          poles->Array2(),
                          &weights->Array2(),
                                                  ...
                                                  const Standard_Real    Tolerance)
{
  ...
  if (urational || vrational)
  {
    nweights = new TColStd_HArray2OfReal(1, npoles->ColLength(), 1, npoles->RowLength());
    if (!BSplSLib1::RemoveKnot(Standard_True,
                              Index,
                              M,
                              udeg,
                              uperiodic,
  ...
  else
  {
    ...
    //
    nweights = new TColStd_HArray2OfReal(1, npoles->ColLength(), 1, npoles->RowLength(), 1.0e0);
    if (!BSplSLib1::RemoveKnot(Standard_True,
                              Index,
                              M,
                              udeg,
                                                  ...
                                                  const Standard_Real    Tolerance)
{
  ...
  if (urational || vrational)
  {
    ...
    if (!BSplSLib1::RemoveKnot(Standard_False,
                              Index,
                              M,
                              vdeg,
                              vperiodic,
                                     ...
                                     Standard_Real&      VTolerance)
{
  if (!maxderivinvok)
  {
    BSplSLib1::Resolution(poles->Array2(),
                         &weights->Array2(),
                         uknots->Array1(),
                         vknots->Array1(),
                         umults->Array1(),
