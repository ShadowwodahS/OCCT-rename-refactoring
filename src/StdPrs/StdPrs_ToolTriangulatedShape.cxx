                                                          ...
                                                          const Handle(StyleDrawer)& theDrawer)
{
  ...
  Handle(BRepMesh_DiscretRoot) aMeshAlgo =
    DiscretizationFactory::Get().Discret(theShape, aDeflection, theDrawer->DeviationAngle());
  if (!aMeshAlgo.IsNull())
  {
    aMeshAlgo->Perform();
  ...
  const Standard_Boolean      theToResetCoeff)
{
  ...
  if ((!isOwnDeviationAngle || Abs(anAngleNew - anAnglePrev) <= Precision1::Angular())
      && (!isOwnDeviationCoefficient || Abs(aCoeffNew - aCoeffPrev) <= Precision1::Confusion()))
  {
    return;
  }
