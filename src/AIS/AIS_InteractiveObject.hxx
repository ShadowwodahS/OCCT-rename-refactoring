...
class VisualEntity : public SelectMgr_SelectableObject
{
  friend class VisualContext;
  DEFINE_STANDARD_RTTIEXT(VisualEntity, SelectMgr_SelectableObject)
public:
  ...
  //! corresponding VisualContext instance for cases when it
  ...
  VisualContext* InteractiveContext() const { return myCTXPtr; }
...
protected:
  // clang-format off
  VisualContext*    myCTXPtr; //!< pointer to Interactive Context, where object is currently displayed; @sa SetContext()
  // clang-format on
  Handle(RefObject) myOwner; //!< application-specific owner object
};

DEFINE_STANDARD_HANDLE(VisualEntity, SelectMgr_SelectableObject)
