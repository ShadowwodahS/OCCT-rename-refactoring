...
static Standard_Integer nbDirectionComponents(const Dir3d& theDir)
{
...
{
  const bool isNewSize = Abs(mySize - theValue) > Precision1::Confusion();
  mySize               = theValue;
...
void AIS_ViewCube::SetRoundRadius(const Standard_Real theValue)
{
  ...
  if (Abs(myRoundRadius - theValue) > Precision1::Confusion())
  {
                                            ...
                                            V3d_TypeOfOrientation                     theDir) const
{
  ...
  else
  {
    ...
    theTris->AddVertex(aHSizeDir + myBoxFacetExtension * Dir3d(aDir.X(), 0.0, 0.0).XYZ());
                           ...
                           const Standard_Integer            theMode)
{
  ...
  // Display box sides
  {
         ...
         ++aPartIter)
    {
      ...
      const Frame3d        aPosition(aPos, aDir, anUp.Crossed(aDir));
                              ...
                              const Handle(CameraOn3d)& theCamera)
{
  ...
  if (!aBndBox.IsVoid())
  {
    theView->FitMinMax(theCamera, aBndBox, 0.01, 10.0 * Precision1::Confusion());
  }
}
...
void AIS_ViewCube::StartAnimation(const Handle(AIS_ViewCubeOwner)& theOwner)
{
...

  {
    ...
    const Dir3d aNewDir = myEndState->Direction();
    if (!myToResetCameraUp && !aNewDir.IsEqual(myStartState->Direction(), Precision1::Angular()))
    {
      ...
      Standard_Real aBestAngle = Precision1::Infinite();
      Dir3d        anUpBest;
