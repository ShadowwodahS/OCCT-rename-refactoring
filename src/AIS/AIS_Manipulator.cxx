...
IMPLEMENT_STANDARD_HANDLE(AIS_Manipulator, VisualEntity)
IMPLEMENT_STANDARD_RTTIEXT(AIS_Manipulator, VisualEntity)
...
void AIS_Manipulator::init()
{
  ...
  // transformation.
  myAxes[0] = Axis(gp1::OX(), Quantity_NOC_RED);
  myAxes[1] = Axis(gp1::OY(), Quantity_NOC_GREEN);
  myAxes[2] = Axis(gp1::OZ(), Quantity_NOC_BLUE1);
...
void AIS_Manipulator::EnableMode(const AIS_ManipulatorMode theMode)
{
  ...
  const Handle(VisualContext)& aContext = GetContext();
  if (aContext.IsNull())
  {
    return;
  }
                             ...
                             const OptionsForAttach&                      theOptions)
{
  ...
  const Handle(VisualContext)& aContext = Object()->GetContext();
  if (!aContext.IsNull())
  {
...
void AIS_Manipulator::Detach()
{
  ...
  Handle(VisualEntity)         anObject = Object();
  const Handle(VisualContext)& aContext = anObject->GetContext();
  if (!aContext.IsNull())
  {
...
Handle(VisualEntity) AIS_Manipulator::Object(const Standard_Integer theIndex) const
{
...
Handle(VisualEntity) AIS_Manipulator::Object() const
{
  return Object(1);
}
                                                       ...
                                                       Transform3d&                theTrsf)
{
  ...
  switch (myCurrentMode)
  {
    ...
    case AIS_MM_Rotation: {
      ...
      // Draw1 a sector indicating the rotation angle
...
Standard_Boolean AIS_Manipulator::ProcessDragging(const Handle(VisualContext)& aCtx,
                                                  const Handle(ViewWindow)&               theView,
...
void AIS_Manipulator::RecomputeTransformation(const Handle(CameraOn3d)& theCamera)
{
  ...
  for (Standard_Integer anIt = 0; anIt < 3; ++anIt)
  {
    ...
    const Dir3d aCameraProj = Abs(Abs(anAxisDir.Dot(aCameraDir)) - 1.0) <= gp1::Resolution()
                                 ? aCameraDir
  ...
  {
    const Dir3d aXDir  = gp1::DX();
    const Dir3d anYDir = gp1::DY();
    const Dir3d aZDir  = gp1::DZ();

    const Dir3d aCameraProjection =
      Abs(aXDir.Dot(aCameraDir)) <= gp1::Resolution()
          || Abs(anYDir.Dot(aCameraDir)) <= gp1::Resolution()
        ? aCameraDir
    ...
    Dir3d              aRotAxis = Abs(Abs(aCameraProjection.Dot(aZDir)) - 1.0) <= gp1::Resolution()
                                     ? aZDir
    ...
    aRotationTrsf.SetRotation(Axis3d(gp1::Origin(), aRotAxis), anAngle);

    Ax3 aToSystem(gp1::Origin(),
                     myPosition.XDirection().Crossed(myPosition.YDirection()),
                     myPosition.XDirection());
    Ax3 aFromSystem(gp1::XOY());
...
void AIS_Manipulator::updateTransformation()
{
  ...
  {
    aTrsf.SetTransformation(myPosition, gp1::XOY());
  }
  else
  {
  ...
  VisualEntity::setLocalTransformation(aGeomTrsf);
  for (Standard_Integer anIt = 0; anIt < 3; ++anIt)
  {
                              ...
                              const Standard_Integer                    theMode)
{
  ...
  // Display center
  myCenter.Init(myAxes[0].AxisRadius() * 2.0f, gp1::Origin(), mySkinMode);
                                            ...
                                            const Handle(SelectMgr_EntityOwner)&      theOwner)
{
  ...
  if (anOwner->Mode() == AIS_MM_TranslationPlane && mySkinMode == ManipulatorSkin_Shaded)
  {
    Handle(StyleDrawer) aStyle = new StyleDrawer();
...
void AIS_Manipulator::RecomputeSelection(const AIS_ManipulatorMode theMode)
{
  ...
  const Handle(SelectionContainer)& aSelection = Object()->Selection(theMode);
...
void AIS_Manipulator::ComputeSelection(const Handle(SelectionContainer)& theSelection,
                                       const Standard_Integer             theMode)
{
  // Check mode
      ...
      for (Standard_Integer anIt = 0; anIt < 3; ++anIt)
      {
        ...
        if (mySkinMode == ManipulatorSkin_Shaded)
        {
          ...
          Handle(Select3D_SensitiveSegment) aLine =
            new Select3D_SensitiveSegment(anOwner, gp1::Origin(), anAxis.TranslatorTipPosition());
    ...
    case AIS_MM_Rotation: {
      for (Standard_Integer anIt = 0; anIt < 3; ++anIt)
      {
        ...
        if (mySkinMode == ManipulatorSkin_Shaded)
        {
          ...
          const gp_Circ aGeomCircle(Frame3d(gp1::Origin(), anAxis.ReferenceAxis().Direction()),
                                 ...
                                 const Standard_Integer   theStacksNb)
{
  ...
  Ax3  aSystem(myPosition.Location(), myPosition.Direction());
  Transform3d aTrsf;
                                   ...
                                   const Standard_Integer   theStacksNb)
{
  ...
  Transform3d aTrsf;
  aTrsf.SetTranslation(Vector3d(gp1::Origin(), thePosition));
                                 ...
                                 const ManipulatorSkin    theSkinMode)
{
  if (theSkinMode == ManipulatorSkin_Flat)
  {
    ...
    Ax3  aSystem(aLocation, aXDirection, thePosition.Direction());
    Transform3d aTrsf;
    aTrsf.SetTransformation(aSystem, Ax3());
  ...
  else
  {
    ...
    myTriangulation = new MeshTriangulation(aPoints, aPolyTriangles);
                                        ...
                                        const Dir3d&          theNormal)
{
  ...
  myTriangulation->SetTriangle(theIndex + 1,
                               Triangle2(theIndex * 3 + 1, theIndex * 3 + 2, theIndex * 3 + 3));
...
void AIS_Manipulator::Sector::Init(const Standard_ShortReal theRadius,
                                   ...
                                   const Standard_Integer   theStacksNb)
{
  Ax3  aSystem(thePosition.Location(), thePosition.Direction(), theXDirection);
  Transform3d aTrsf;
  aTrsf.SetTransformation(aSystem, Ax3());
  ...
  if (theSkinMode == ManipulatorSkin_Flat)
  {
    ...
    myTriangulation = new MeshTriangulation(4, 2, Standard_False);
    ...
    myTriangulation->SetTriangle(1, Triangle2(3, 1, 2));
    myTriangulation->SetTriangle(2, Triangle2(1, 3, 4));
  }
  else
  {
                                    ...
                                    const ManipulatorSkin                     theSkinMode)
{
  if (myHasTranslation)
  {
    ...
    if (theSkinMode == ManipulatorSkin_Flat)
    {
      ...
      Ax3  aSystem(gp1::Origin(), myReferenceAxis.Direction());
      Transform3d aTrsf;
      aTrsf.SetTransformation(aSystem, Ax3());
  ...
  if (myHasRotation)
  {
    ...
    myCircle.Init(myInnerRadius + myIndent * 2.0f,
                  Size(),
                  Axis3d(gp1::Origin(), myReferenceAxis.Direction()),
                  anAngle,
                  myFacettesNumber * 2);
