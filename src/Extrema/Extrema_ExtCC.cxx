      ...
      myIsParallel(Standard_False)
{
  myC[0]   = 0;
  myC[1]   = 0;
  myInf[0] = myInf[1] = -Precision1::Infinite();
  mySup[0] = mySup[1] = Precision1::Infinite();
  myTol[0]            = TolC1;
  myTol[1]            = TolC2;
...
void Extrema_ExtCC::Perform()
{
  ...
  if (!Precision1::IsInfinite(U11))
    myP1f = CurveTool4::Value(*myC[0], U11);
  if (!Precision1::IsInfinite(U12))
    myP1l = CurveTool4::Value(*myC[0], U12);
  if (!Precision1::IsInfinite(U21))
    myP2f = CurveTool4::Value(*myC[1], U21);
  if (!Precision1::IsInfinite(U22))
    myP2l = CurveTool4::Value(*myC[1], U22);

  if (Precision1::IsInfinite(U11) || Precision1::IsInfinite(U21))
    mydist11 = RealLast();
  else
    mydist11 = myP1f.SquareDistance(myP2f);
  if (Precision1::IsInfinite(U11) || Precision1::IsInfinite(U22))
    mydist12 = RealLast();
  else
    mydist12 = myP1f.SquareDistance(myP2l);
  if (Precision1::IsInfinite(U12) || Precision1::IsInfinite(U21))
    mydist21 = RealLast();
  else
    mydist21 = myP1l.SquareDistance(myP2f);
  if (Precision1::IsInfinite(U12) || Precision1::IsInfinite(U22))
    mydist22 = RealLast();
  else
      ...
      || (type2 == GeomAbs_Line && type1 <= GeomAbs_Parabola))
  {
    ...
    {
      case GeomAbs_Line: {
        ExtElC Xtrem(myC[anInd1]->Line(), myC[anInd2]->Line(), Tol);
        PrepareResults(Xtrem, isInverse, U11, U12, U21, U22);
        break;
      }
      case GeomAbs_Circle: {
        ExtElC Xtrem(myC[anInd1]->Line(), myC[anInd2]->Circle(), Tol);
        PrepareResults(Xtrem, isInverse, U11, U12, U21, U22);
        break;
      }
      case GeomAbs_Ellipse: {
        ExtElC Xtrem(myC[anInd1]->Line(), myC[anInd2]->Ellipse());
        PrepareResults(Xtrem, isInverse, U11, U12, U21, U22);
        break;
      }
      case GeomAbs_Hyperbola: {
        ExtElC Xtrem(myC[anInd1]->Line(), myC[anInd2]->Hyperbola());
        PrepareResults(Xtrem, isInverse, U11, U12, U21, U22);
        break;
      }
      case GeomAbs_Parabola: {
        ExtElC Xtrem(myC[anInd1]->Line(), myC[anInd2]->Parabola());
        PrepareResults(Xtrem, isInverse, U11, U12, U21, U22);
        break;
      }
      default:
  ...
  else if (type1 == GeomAbs_Circle && type2 == GeomAbs_Circle)
  {
    ...
    ExtElC   CCXtrem(myC[0]->Circle(), myC[1]->Circle());
                                          ...
                                          const Standard_Real theSqDist)
{
  ...
  const GeomAbs_CurveType aType1 = CurveTool4::GetType(*myC[0]);
  const GeomAbs_CurveType aType2 = CurveTool4::GetType(*myC[1]);
  ...
  if (aType1 != aType2)
  {
    ...
    const Point3d           aPonC      = !isReversed ? CurveTool4::Value(*myC[0], theUt11)
                                                    : CurveTool4::Value(*myC[1], theUt21);

    const gp_Lin          aL = myC[!isReversed ? 1 : 0]->Line();
    const PointElCCurveExtrema ExtPLin(aPonC,
                                  aL,
  ...
  if (aType1 == GeomAbs_Line)
  {
    ...
    const Standard_Real isFirstInfinite =
      (Precision1::IsInfinite(theUt11) && Precision1::IsInfinite(theUt12));
    const Standard_Real isLastInfinite =
      (Precision1::IsInfinite(theUt21) && Precision1::IsInfinite(theUt22));
    ...
    else
    {
      ...
      Range1 aRange2(theUt21, theUt22);
      Range1 aProjRng12;

      if (Precision1::IsInfinite(theUt11))
      {
        if (isOpposite)
          aProjRng12.Add(Precision1::Infinite());
        else
          aProjRng12.Add(-Precision1::Infinite());
      }
      else
      {
        const Point3d        aPonC1 = ElCLib1::Value(theUt11, aLin1);
        const Standard_Real aPar   = ElCLib1::Parameter(aLin2, aPonC1);
        aProjRng12.Add(aPar);
      }

      if (Precision1::IsInfinite(theUt12))
      {
        if (isOpposite)
          aProjRng12.Add(-Precision1::Infinite());
        else
      ...
      {
        const Point3d        aPonC1 = ElCLib1::Value(theUt12, aLin1);
        const Standard_Real aPar   = ElCLib1::Parameter(aLin2, aPonC1);
        aProjRng12.Add(aPar);
      }

      aRange2.Common(aProjRng12);
      if (aRange2.Delta() > Precision1::Confusion())
      {
        ClearSolutions();
        mySqDist.Append(theSqDist);
        myIsParallel = Standard_True;
      ...
      else if (!aRange2.IsVoid())
      {
        ...
        Point3d                aP = ElCLib1::Value(aPar2, aLin2);
        const PointOnCurve1 aP2(aPar2, aP);
        aPar1 = ElCLib1::Parameter(aLin1, aP);
        aP    = ElCLib1::Value(aPar1, aLin1);
        const PointOnCurve1 aP1(aPar1, aP);
        mypoints.Append(aP1);
        mypoints.Append(aP2);
        mySqDist.Append(theSqDist);
      }
  ...
  else
  {
    ...
    Vector3d              aVTg1;
    Point3d              aP11;
    const Point3d        aP12 = CurveTool4::Value(*myC[0], theUt12);
    CurveTool4::D1(*myC[0], theUt11, aP11, aVTg1);

    const Range1 aRange(theUt21, theUt22);
    Range1       aProjRng1;

    // Precision1 of the calculation depends on circles radii
    ...
    Standard_Real aPar1 =
      ElCLib1::InPeriod(ElCLib1::Parameter(aWorkCirc, aP11), theUt21, theUt21 + aPeriod);
    const Vector3d aVTg2 = CurveTool4::DN(*myC[1], aPar1, 1);

    // Check if circles have same/opposite directions
    const Standard_Boolean isOpposite(aVTg1.Dot(aVTg2) < 0.0);

    Standard_Real aPar2 =
      ElCLib1::InPeriod(ElCLib1::Parameter(aWorkCirc, aP12), theUt21, theUt21 + aPeriod);

    if (isOpposite)
    {
      // Must be aPar2 < aPar1
      if ((aRange.Delta() > Precision1::Angular()) && ((aPar1 - aPar2) < Precision1::Angular()))
      {
        aPar2 -= aPeriod;
      }
    }
    ...
    {
      // Must be aPar2 > aPar1
      if ((aRange.Delta() > Precision1::Angular()) && ((aPar2 - aPar1) < Precision1::Angular()))
      {
        aPar2 += aPeriod;
      }
    }
    ...
    for (Standard_Integer i = 0; i < 3; i++)
    {
      ...
      if (aRng.Delta() > Precision1::Confusion())
      {
        Standard_Real aPar = 0.0;
        aRng.GetIntermediatePoint(0.5, aPar);
        const Point3d    aPCirc2 = ElCLib1::Value(aPar, aWorkCirc);
        PointElCCurveExtrema ExtPCir(aPCirc2,
                                CurveTool4::Circle(*myC[0]),
                                Precision1::Confusion(),
                                theUt11,
                                theUt12);
        if (ExtPCir.NbExt() < 1)
        ...
        if (aMinSqD <= aMinSquareDist + (1. + aMinSqD) * aPrecision)
        {
          ...
          if (aDeltaSqDist * aDeltaSqDist < 4.0 * aSqD * Precision1::SquareConfusion())
          {
      ...
      else if (!aRng.IsVoid())
      {
        ...
        aRng.GetIntermediatePoint(0.5, aPar);
        const Point3d          aPCirc2 = ElCLib1::Value(aPar, aWorkCirc);
        const PointOnCurve1 aP2(aPar, aPCirc2);
  ...
  if (myDone)
  {
    ...
    else
    {
      ...
      for (i = 1; i <= NbExt; i++)
      {
        ...
        if (CurveTool4::IsPeriodic(*myC[0]))
        {
          U = ElCLib1::InPeriod(U, Ut11, Ut11 + CurveTool4::Period(*myC[0]));
        }
        if (CurveTool4::IsPeriodic(*myC[1]))
        {
          U2 = ElCLib1::InPeriod(U2, Ut21, Ut21 + CurveTool4::Period(*myC[1]));
        }
