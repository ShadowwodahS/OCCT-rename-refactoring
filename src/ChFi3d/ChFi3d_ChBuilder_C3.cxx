                                            ...
                                            Standard_Real&                 tolreached)
{
  ...
  return ChFi3d_ComputeCurves(HS1, HS2, Pardeb, Parfin, gc, pc1, pc2, tol3d, tol2d, tolreached);
}
...
void ChFi3d_ChBuilder::PerformThreeCorner(const Standard_Integer Jndex)
{
  ...
  if (oksea[pivot] && (i[deb][pivot] != i[deb][fin] || i[fin][pivot] != i[fin][deb]))
  {
  ...
  ChFi3d_ComputesIntPC(fdpiv->Interference(jf[pivot][deb]),
                       ...
                       p[deb][pivot],
                       p3d[fin]);
  ChFi3d_ComputesIntPC(fdpiv->Interference(jf[pivot][fin]),
                       fdfin->Interference(jf[fin][pivot]),
                       HSpiv,
                       HSfin,
                       p[pivot][fin],
                       p[fin][pivot],
                       p3d[deb]);
  ChFi3d_ComputesIntPC(fddeb->Interference(jf[deb][fin]),
                       fdfin->Interference(jf[fin][deb]),
                       HSdeb,
                       HSfin,
                       p[deb][fin],
                       p[fin][deb],
  ...
  else
  {
    ...
    else
    {
      ...
      //			    p2d[3],tolesp,2.e-4);
      Bfac = ChFi3d_mkbound(Fac, PCurveOnFace, p2d[pivot], p2d[3], tolapp3d, 2.e-4);
      Bpiv = ChFi3d_mkbound(Surf, PCurveOnPiv, p2d[fin], p2d[deb], tolapp3d, 2.e-4);
    }
