...

{
  ...
  if ((fac.GetType() != GeomAbs_Plane) || (s1.GetType() != GeomAbs_Plane)
    ...
    return Standard_False;
  Dir3d df  = fac.Plane1().Position1().Direction();
  Dir3d ds1 = s1.Plane1().Position1().Direction();
  Dir3d ds2 = s2.Plane1().Position1().Direction();
...
void ChFi3d_FilBuilder::PerformTwoCorner(const Standard_Integer Index)
{
  ...
  if (!c1biseau && !done)
  {
	...
	fil.CheckResult(ib);
      ...
      stsam->SetParameters(isfirstsam, intcoin1.FirstParameter(), intcoin1.LastParameter());
