...
public:
  Standard_EXPORT StdSelect_Shape(const TopoShape&         theShape,
                                  const Handle(StyleDrawer)& theDrawer = Handle(StyleDrawer)());

  Standard_EXPORT virtual void Compute(const Handle(PrsMgr_PresentationManager)& thePrsMgr,
                                       const Handle(Prs3d_Presentation)&         thePrs,
                                       const Standard_Integer theMode) Standard_OVERRIDE;

  const TopoShape& Shape() const { return mysh; }

  void Shape(const TopoShape& theShape) { mysh = theShape; }
