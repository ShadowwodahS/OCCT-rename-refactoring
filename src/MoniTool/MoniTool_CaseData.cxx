...
MoniTool_CaseData::MoniTool_CaseData(const Standard_CString caseid, const Standard_CString name)
    : thesubst(0),
      thecase(caseid),
      thename(name)
{
...
void MoniTool_CaseData::AddReal(const Standard_Real val, const Standard_CString name)
{
...
void MoniTool_CaseData::AddText(const Standard_CString text, const Standard_CString name)
{
  AddData(new TCollection_HAsciiString(text), 10, name);
}

void MoniTool_CaseData::AddInteger(const Standard_Integer val, const Standard_CString name)
{
...
Standard_Boolean MoniTool_CaseData::Real(const Standard_Integer nd, Standard_Real& val) const
{
  Handle(Geom2d_CartesianPoint) p = Handle(Geom2d_CartesianPoint)::DownCast(Data(nd));
  if (p.IsNull())
    return Standard_False;
...
Standard_Boolean MoniTool_CaseData::Text(const Standard_Integer nd, Standard_CString& text) const
{
...
void MoniTool_CaseData::SetDefMsg(const Standard_CString casecode, const Standard_CString mesdef)
{
