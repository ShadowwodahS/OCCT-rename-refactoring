...
Standard_Boolean MoniTool_AttrList::RemoveAttribute(const Standard_CString name)
{
...
void MoniTool_AttrList::SetIntegerAttribute(const Standard_CString name, const Standard_Integer val)
{
...
Standard_Boolean MoniTool_AttrList::GetIntegerAttribute(const Standard_CString name,
                                                        Standard_Integer&      val) const
{
  Handle(MoniTool_IntVal) ival = Handle(MoniTool_IntVal)::DownCast(Attribute(name));
  if (ival.IsNull())
  {
    val = 0;
    return Standard_False;
...
Standard_Integer MoniTool_AttrList::IntegerAttribute(const Standard_CString name) const
{
  Handle(MoniTool_IntVal) ival = Handle(MoniTool_IntVal)::DownCast(Attribute(name));
  if (ival.IsNull())
    return 0;
...
void MoniTool_AttrList::SetRealAttribute(const Standard_CString name, const Standard_Real val)
{
...
Standard_Boolean MoniTool_AttrList::GetRealAttribute(const Standard_CString name,
                                                     Standard_Real&         val) const
{
  Handle(MoniTool_RealVal) rval = Handle(MoniTool_RealVal)::DownCast(Attribute(name));
  if (rval.IsNull())
  {
    val = 0.0;
    return Standard_False;
...
void MoniTool_AttrList::SetStringAttribute(const Standard_CString name, const Standard_CString val)
{
  Handle(TCollection_HAsciiString) hval = new TCollection_HAsciiString(val);
  SetAttribute(name, hval);
}

Standard_Boolean MoniTool_AttrList::GetStringAttribute(const Standard_CString name,
