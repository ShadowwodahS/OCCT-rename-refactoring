...
TCollection_AsciiString IGESSelect_SelectSubordinate::ExtractLabel() const
{
  ...
  if (thestatus == 1)
    lab.AssignCat("Physically only Dependant (1)");
  if (thestatus == 2)
    lab.AssignCat("Logically only Dependant (2) ");
  if (thestatus == 3)
    lab.AssignCat("Both Phys. and Log. Dependant (3)");
  if (thestatus == 4)
    lab.AssignCat("Physically Dependant (1 or 3)");
  if (thestatus == 5)
    lab.AssignCat("Logically Dependant (2 or 3)");
  if (thestatus == 6)
    lab.AssignCat("Dependant in any way (1 or 2 or 3)");
  return lab;
}
