                                             ...
                                             const Handle(RefObject)& item) const
{
  ...
  {
    Standard_Boolean tryc2 = GetCasted(IGESSelect_SplineToBSpline, item)->OptionTryC2();
                                            ...
                                            Handle(RefObject)&    item) const
{
  ...
  if (type.IsEqual("IGESSelect_SelectIGESTypeForm"))
  {
    ...
    if (exname.Value(1) == 'e')
    {
    } // szv#4:S4163:12Mar99 `exact = Standard_True` not needed
    else if (exname.Value(1) == 'c')
    {
    } // szv#4:S4163:12Mar99 `exact = Standard_False` not needed
    else
      return Standard_False;
  ...
  if (type.IsEqual("IGESSelect_SetGlobalParameter"))
  {
    ...
    Handle(IGESSelect_SetGlobalParameter) sp = new IGESSelect_SetGlobalParameter(np);
    sp->SetValue(val);
    item = sp;
    return Standard_True;
  }
