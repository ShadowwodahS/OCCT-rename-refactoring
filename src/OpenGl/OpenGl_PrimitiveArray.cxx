...
public:
  ...
  virtual void BindPositionAttribute(const Handle(OpenGl_Context)& theGlCtx) const
  {
    ...
    for (Standard_Integer anAttribIter = 0; anAttribIter < NbAttributes; ++anAttribIter)
    {
      ...
      if (anAttrib.Id == Graphic3d_TOA_POS && aDataType != GL_NONE)
      {
        TheBaseClass::bindAttribute(theGlCtx,
                                    Graphic3d_TOA_POS,
                                    aNbComp,
                                    aDataType,
                                    Stride,
...
Standard_Boolean OpenGl_PrimitiveArray::initNormalVbo(const Handle(OpenGl_Context)& theCtx) const
{
  ...
  {
    case 1:
      myVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBuffer, 1>(*myAttribs);
      break;
    case 2:
      myVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBuffer, 2>(*myAttribs);
      break;
    case 3:
      myVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBuffer, 3>(*myAttribs);
      break;
    case 4:
      myVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBuffer, 4>(*myAttribs);
      break;
    case 5:
      myVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBuffer, 5>(*myAttribs);
      break;
    case 6:
      myVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBuffer, 6>(*myAttribs);
      break;
    case 7:
      myVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBuffer, 7>(*myAttribs);
      break;
    case 8:
      myVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBuffer, 8>(*myAttribs);
      break;
    case 9:
      myVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBuffer, 9>(*myAttribs);
      break;
    case 10:
      myVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBuffer, 10>(*myAttribs);
      break;
  }
  ...
  if (myAttribs->NbElements != myAttribs->NbMaxElements() && myIndices.IsNull()
      && (!isAttribInterleaved || isAttribMutable))
  {
    throw Standard_ProgramError("OpenGl_PrimitiveArray::buildVBO() - vertex attribute data with "
  ...
  const Standard_Integer aNbVertexes =
  ...
  {
    UtfString aMsg =
      UtfString("VBO creation for Primitive Array has failed for ") + aNbVertexes
      + " vertices. Out of memory?";
  ...
  const Standard_Integer aNbIndexes =
    !myIndices->IsMutable() ? myIndices->NbElements : myIndices->NbMaxElements();
  ...
  switch (myIndices->Stride)
  {
    case 2: {
      isOk = myVboIndices->Init(theCtx,
                                1,
                                aNbIndexes,
                                reinterpret_cast<const GLushort*>(myIndices->Data()));
    ...
    case 4: {
      isOk = myVboIndices->Init(theCtx,
                                1,
                                aNbIndexes,
                                reinterpret_cast<const GLuint*>(myIndices->Data()));
      myVboIndices->SetElemsNb(myIndices->NbElements);
      myIndices->Validate();
      break;
    }
  ...
  {
    UtfString aMsg =
      UtfString("VBO creation for Primitive Array has failed for ") + aNbIndexes
      + " indices. Out of memory?";
    theCtx->PushMessage(GL_DEBUG_SOURCE_APPLICATION,
                        GL_DEBUG_TYPE_PERFORMANCE,
                        0,
                        GL_DEBUG_SEVERITY_LOW,
                                                 ...
                                                 const Standard_Boolean        theToKeepData) const
{
  ...
  if (isNormalMode && initNormalVbo(theCtx))
  {
    if (!theCtx->caps->keepArrayData && !theToKeepData && !myAttribs->IsMutable())
    {
      myIndices.Nullify();
      myAttribs.Nullify();
    }
  ...
  {
    case 1:
      aVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBufferCompat, 1>(*myAttribs);
      break;
    case 2:
      aVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBufferCompat, 2>(*myAttribs);
      break;
    case 3:
      aVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBufferCompat, 3>(*myAttribs);
      break;
    case 4:
      aVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBufferCompat, 4>(*myAttribs);
      break;
    case 5:
      aVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBufferCompat, 5>(*myAttribs);
      break;
    case 6:
      aVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBufferCompat, 6>(*myAttribs);
      break;
    case 7:
      aVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBufferCompat, 7>(*myAttribs);
      break;
    case 8:
      aVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBufferCompat, 8>(*myAttribs);
      break;
    case 9:
      aVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBufferCompat, 9>(*myAttribs);
      break;
    case 10:
      aVboAttribs = new OpenGl_VertexBufferT<OpenGl_VertexBufferCompat, 10>(*myAttribs);
      break;
  }
...
void OpenGl_PrimitiveArray::drawEdges(const Handle(OpenGl_Workspace)& theWorkspace) const
{
  ...
  aGlContext->SetSampleAlphaToCoverage(
    aGlContext->ShaderManager()->MaterialState().HasAlphaCutoff());
  const GLenum aDrawMode =
    !aGlContext->ActiveProgram().IsNull() && aGlContext->ActiveProgram()->HasTessellationStage()
      ? GL_PATCHES
      : myDrawMode;
  ...
  {
    aGlContext->core11fwd->glDrawArrays(aDrawMode,
                                        0,
                                        !myVboAttribs.IsNull() ? myVboAttribs->GetElemsNb()
...
void OpenGl_PrimitiveArray::drawMarkers(const Handle(OpenGl_Workspace)& theWorkspace) const
{
  ...
  else if (const Handle(OpenGl_PointSprite)& aSprite =
                                            ...
                                            bool                         theIsDetailed) const
{
  ...
  const Standard_Integer aNbIndices =
    !myVboIndices.IsNull() ? myVboIndices->GetElemsNb() : myVboAttribs->GetElemsNb();
...
void OpenGl_PrimitiveArray::Render(const Handle(OpenGl_Workspace)& theWorkspace) const
{
  ...
  else if ((!myAttribs.IsNull() && myAttribs->IsMutable())
           || (!myIndices.IsNull() && myIndices->IsMutable()))
  {
    updateVBO(aCtx);
  }
  ...
  if (toDrawArray)
  {
    const bool hasColorAttrib = !myVboAttribs.IsNull() && myVboAttribs->HasColorAttribute();
    const bool toHilight      = theWorkspace->ToHighlight();
    ...
    const Graphic3d_Vec4* aFaceColors =
        ...
        && aCtx->ShaderManager()->BindOutlineProgram())
    {
      ...
      anOutlineProgram->SetUniform(
        aCtx,
        anOutlineProgram->GetStateLocation(OpenGl_OCCT_SILHOUETTE_THICKNESS),
        anEdgeWidth);
      anOutlineProgram->SetUniform(aCtx,
                                   anOutlineProgram->GetStateLocation(OpenGl_OCCT_ORTHO_SCALE),
                                   anOrthoScale);
