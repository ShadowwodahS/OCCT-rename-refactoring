                         ...
                         const Handle(OpenGl_Context)&       theShareCtx)
{
  ...
  if ((EGLContext)theGContext == EGL_NO_CONTEXT)
  {
    ...
    {
      throw Aspect_GraphicDeviceDefinitionError(
        "OpenGl_Window, EGL is unable to create surface for window!");
    }
  ...
  {
    throw Aspect_GraphicDeviceDefinitionError(
      "OpenGl_Window, EGL is used in unsupported combination!");
  }
  else
  {
  ...
  myGlContext->Init((Aspect_Drawable)anEglSurf,
                    (Aspect_Display)anEglDisplay,
                    (Aspect_RenderingContext)anEglContext,
                    isCoreProfile);
  #elif defined(_WIN32)
  (void)theDriver;
  ...
  if (aGContext == NULL)
  {
    ...
    if (aChoosePixProc != NULL)
    {
      ...
      {
        /// TODO
        Message1::SendFail() << "Error: unable to find RGB10_A2 window buffer format!";
      }
    }
    ...
    if (aCreateCtxProc != NULL)
    {
      ...
      {
        int aCoreCtxAttribs[] = {WGL_CONTEXT_MAJOR_VERSION_ARB,
                                 3,
  ...
  myGlContext->Init((Aspect_Handle)aWindow,
                    (Aspect_Handle)aWindowDC,
  ...
  myGlContext->Init((Aspect_Drawable)aWindow,
                    (Aspect_Display)aDisp,
                    (Aspect_RenderingContext)aGContext,
                    isCoreProfile);
  #else
  // not implemented
...
OpenGl_Window::~OpenGl_Window()
{
  ...
  {
    eglDestroySurface((EGLDisplay)myGlContext->myDisplay, (EGLSurface)myGlContext->myWindow);
  }
  #elif defined(_WIN32)
