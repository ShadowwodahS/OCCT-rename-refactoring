                                                     ...
                                                     const Handle(OpenGl_Context)& theGlContext)
{
         ...
         ++aPriorityIter)
    {
           ...
           aStructIt.Next())
      {
        ...
        else if (theMode == OpenGl_GUM_PREPARE)
        {
          ...
          for (OpenGl_Structure::GroupIterator aGroupIter(aStructure->Groups()); aGroupIter.More();
               aGroupIter.Next())
          {
            // Extract OpenGL elements from the group (primitives arrays)
            for (const ElementNode* aNode = aGroupIter.Value()->FirstNode(); aNode != NULL;
                 aNode                           = aNode->next)
            {
  ...
  else if (theMode == OpenGl_GUM_REBUILD)
  {
    ...
    std::map<const OpenGl_Structure*, StructState1>::iterator anIter = myStructureStates.begin();
...
Standard_Boolean OpenGl_View::toUpdateStructure(const OpenGl_Structure* theStructure)
{
  ...
  std::map<const OpenGl_Structure*, StructState1>::iterator aStructState =
...
// =======================================================================
void buildTextureTransform(const Handle(TextureParams)& theParams, BVH_Mat4f& theMatrix)
{
...
// =======================================================================
RaytraceMaterial OpenGl_View::convertMaterial(const OpenGl_Aspects*         theAspect,
                                                     const Handle(OpenGl_Context)& theGlContext)
{
  RaytraceMaterial aResMat;
  ...
  const Handle(TextureSet2)& aTextureSet = theAspect->TextureSet(theGlContext);
  ...
  if (theGlContext->HasRayTracingTextures())
  {
    ...
    for (TextureSet2::Iterator aTexIter(aTextureSet); aTexIter.More(); aTexIter.Next())
    {
                                                   ...
                                                   const Handle(OpenGl_Context)& theGlContext)
{
  ...
  {
    myStructureStates[theStructure] = StructState1(theStructure);

    return Standard_True;
  }

  // Get structure material
  RaytraceMaterial aDefaultMaterial;
                                                ...
                                                const Handle(OpenGl_Context)&  theGlContext)
{
  ...
  {
    // Get group material
    RaytraceMaterial aGroupMaterial;
         ...
         aNode                           = aNode->next)
    {
      ...
      if (anAspect != NULL)
      {
        ...
        RaytraceMaterial aMaterial = convertMaterial(anAspect, theGlContext);
      ...
      else
      {
        ...
        if (aPrimArray != NULL)
        {
          std::map<Standard_Size, TriangleSet2*>::iterator aSetIter =
            myArrayToTrianglesMap.find(aPrimArray->GetUID());

          if (aSetIter != myArrayToTrianglesMap.end())
          {
            TriangleSet2*                                       aSet = aSetIter->second;
            ...
            if (aSet->MaterialIndex() != TriangleSet2::INVALID_MATERIAL
                && aSet->MaterialIndex() != aMatID)
            {
          ...
          {
            if (Handle(TriangleSet2) aSet = addRaytracePrimitiveArray(aPrimArray, aMatID, 0))
            {
              opencascade::handle<BVH_Transform<Standard_ShortReal, 4>> aTransform =
                new BVH_Transform<Standard_ShortReal, 4>();
              if (!theTrsf.IsNull())
...
// =======================================================================
Handle(TriangleSet2) OpenGl_View::addRaytracePrimitiveArray(
  ...
  const OpenGl_Mat4*           theTransform)
{
  ...
  Handle(TriangleSet2) aSet =
    new TriangleSet2(theArray->GetUID(), myRaytraceBVHBuilder);
  {
...
// =======================================================================
Standard_Boolean OpenGl_View::addRaytraceVertexIndices(TriangleSet2&          theSet,
                                                       ...
                                                       const OpenGl_PrimitiveArray& theArray)
{
  ...
  {
    case GL_TRIANGLES:
      return addRaytraceTriangleArray(theSet, theMatID, theCount, theOffset, theArray.Indices());
    case GL_TRIANGLE_FAN:
      return addRaytraceTriangleFanArray(theSet, theMatID, theCount, theOffset, theArray.Indices());
    case GL_TRIANGLE_STRIP:
      return addRaytraceTriangleStripArray(theSet,
                                           theMatID,
                                           theCount,
                                           theOffset,
                                           theArray.Indices());
    case GL_QUAD_STRIP:
      return addRaytraceQuadrangleStripArray(theSet,
                                             theMatID,
                                             theCount,
                                             theOffset,
                                             theArray.Indices());
    case GL_QUADS:
      return addRaytraceQuadrangleArray(theSet, theMatID, theCount, theOffset, theArray.Indices());
    case GL_POLYGON:
...
const AsciiString1 OpenGl_View::ShaderSource1::EMPTY_PREFIX;

// =======================================================================
// function : Source
// purpose  : Returns shader source combined with prefix
// =======================================================================
AsciiString1 OpenGl_View::ShaderSource1::Source(const Handle(OpenGl_Context)& theCtx,
...
// =======================================================================
Standard_Boolean OpenGl_View::ShaderSource1::LoadFromFiles(
...
Standard_Boolean OpenGl_View::ShaderSource1::LoadFromStrings(
  ...
  const AsciiString1&     theName)
{
  ...
  Handle(OpenGl_ShaderProgram)  aProgram =
    new OpenGl_ShaderProgram(Handle(ShaderProgram2)(), anId);
                                                    ...
                                                    const Handle(OpenGl_Context)& theGlContext)
{
  ...
  if (myRaytraceInitStatus == OpenGl_RT_INIT)
  {
    ...
    const bool isZeroToOneDepth =
      myCaps->useZeroToOneDepth && myWorkspace->GetGlContext()->arbClipControl;
    ...
    if (myRenderParams.AdaptiveScreenSampling != myRaytraceParameters.AdaptiveScreenSampling
        || myRenderParams.AdaptiveScreenSamplingAtomic
    ...
    if (aToRebuildShaders)
    {
...
#ifdef RAY_TRACE_PRINT_INFO
      Message1::SendTrace() << "GLSL prefix string:" << std::endl << aPrefixString;
#endif
      ...
      if (!myRaytraceShader->LoadAndCompile(
            theGlContext,
            myRaytraceProgram->ResourceId(),
            myRaytraceShaderSource.Source(theGlContext, GL_FRAGMENT_SHADER))
          || !myPostFSAAShader->LoadAndCompile(
            theGlContext,
            myPostFSAAProgram->ResourceId(),
            myPostFSAAShaderSource.Source(theGlContext, GL_FRAGMENT_SHADER))
          || !myOutImageShader->LoadAndCompile(
            theGlContext,
            myOutImageProgram->ResourceId(),
            myOutImageShaderSource.Source(theGlContext, GL_FRAGMENT_SHADER)))
      {
  ...
  if (myRaytraceInitStatus == OpenGl_RT_NONE)
  {
    ...
    const AsciiString1 aShaderFolder = ShaderProgram2::ShadersFolder();
    if (myIsRaytraceDataValid)
    {
    ...
    ShaderSource1 aBasicVertShaderSrc;
    {
  ...
  if (myRaytraceInitStatus == OpenGl_RT_NONE || aToRebuildShaders)
  {
    for (Standard_Integer anIndex = 0; anIndex < 2; ++anIndex)
    {
      ...
      aShaderProgram->SetSampler(theGlContext,
                                 "uSceneMinPointTexture",
                                 OpenGl_RT_SceneMinPointTexture);
      aShaderProgram->SetSampler(theGlContext,
                                 "uSceneMaxPointTexture",
                                 OpenGl_RT_SceneMaxPointTexture);
      aShaderProgram->SetSampler(theGlContext,
                                 "uSceneNodeInfoTexture",
                                 OpenGl_RT_SceneNodeInfoTexture);
      aShaderProgram->SetSampler(theGlContext,
                                 "uGeometryVertexTexture",
                                 OpenGl_RT_GeometryVertexTexture);
      aShaderProgram->SetSampler(theGlContext,
                                 "uGeometryNormalTexture",
                                 OpenGl_RT_GeometryNormalTexture);
      aShaderProgram->SetSampler(theGlContext,
                                 "uGeometryTexCrdTexture",
                                 OpenGl_RT_GeometryTexCrdTexture);
      aShaderProgram->SetSampler(theGlContext,
                                 "uGeometryTriangTexture",
                                 OpenGl_RT_GeometryTriangTexture);
      aShaderProgram->SetSampler(theGlContext,
                                 "uSceneTransformTexture",
                                 OpenGl_RT_SceneTransformTexture);
      aShaderProgram->SetSampler(theGlContext, "uEnvMapTexture", OpenGl_RT_EnvMapTexture);
      aShaderProgram->SetSampler(theGlContext,
                                 "uRaytraceMaterialTexture",
                                 OpenGl_RT_RaytraceMaterialTexture);
      aShaderProgram->SetSampler(theGlContext,
                                 "uRaytraceLightSrcTexture",
                                 OpenGl_RT_RaytraceLightSrcTexture);

      if (anIndex == 1)
      {
        aShaderProgram->SetSampler(theGlContext, "uFSAAInputTexture", OpenGl_RT_FsaaInputTexture);
      }
      else
      {
        aShaderProgram->SetSampler(theGlContext, "uAccumTexture", OpenGl_RT_PrevAccumTexture);
      }
    ...
    myOutImageProgram->SetSampler(theGlContext, "uInputTexture", OpenGl_RT_PrevAccumTexture);
  ...
  for (int aViewIter = 0; aViewIter < 2; ++aViewIter)
  {
    ...
    if (myRaytraceParameters.AdaptiveScreenSampling)
    {
      ...
      myRaytraceOutputTexture[aViewIter]->InitRectangle(theGlContext,
                                                        theSizeX * 3,
                                                        theSizeY * 2,
                                                        TextureFormat::Create<GLfloat, 1>());
      ...
      myRaytraceVisualErrorTexture[aViewIter]->Init(
        theGlContext,
        TextureFormat::FindSizedFormat(theGlContext, GL_R32I),
...
Standard_Boolean OpenGl_View::uploadRaytraceData(const Handle(OpenGl_Context)& theGlContext)
{
  if (theGlContext->GraphicsLibrary() == Aspect_GraphicsLibrary_OpenGLES)
  {
    if (!theGlContext->IsGlGreaterEqual(3, 2))
    {
      Message1::SendFail() << "Error: OpenGL ES version is less than 3.2";
      return Standard_False;
    }
  }
  else
  {
    if (!theGlContext->IsGlGreaterEqual(3, 1))
    {
      Message1::SendFail() << "Error: OpenGL version is less than 3.1";
      return Standard_False;
    }
  }
  ...
  if (theGlContext->arbTexBindless != NULL)
  {
    ...
    {
      Message1::SendTrace() << "Error: Failed to get OpenGL texture handles";
      return Standard_False;
    }
  }
  ...
  if (mySceneNodeInfoTexture.IsNull()) // create scene BVH buffers
  {
    ...
    {
      Message1::SendTrace() << "Error: Failed to create scene BVH buffers";
      return Standard_False;
    }
  }
  ...
  if (myRaytraceMaterialTexture.IsNull()) // create material buffer
  {
    ...
    {
      Message1::SendTrace() << "Error: Failed to create buffers for material data";
      return Standard_False;
    }
  }
  ...
  for (Standard_Integer anElemIndex = 0; anElemIndex < myRaytraceGeometry.Size(); ++anElemIndex)
  {
    ...
    Standard_ASSERT_RETURN(aTransform != NULL,
                           "TriangleSet2 does not contain transform",
                           Standard_False);

    aNodeTransforms[anElemIndex] = aTransform->Inversed();
  }

  aResult &= mySceneTransformTexture->Init(theGlContext,
                                           4,
                                           myRaytraceGeometry.Size() * 4,
                                           reinterpret_cast<const GLfloat*>(aNodeTransforms));
  ...
  if (!aResult)
  {
    Message1::SendTrace() << "Error: Failed to upload buffers for bottom-level scene BVH";
    return Standard_False;
  }
  ...
  if (!aResult)
  {
    Message1::SendTrace() << "Error: Failed to upload buffers for scene geometry";
    return Standard_False;
  }
  ...
  if (aBVH->Length() > 0)
  {
    aResult &= mySceneNodeInfoTexture->SubData(
      theGlContext,
      0,
      aBVH->Length(),
  ...
  for (Standard_Integer aNodeIdx = 0; aNodeIdx < aBVH->Length(); ++aNodeIdx)
  {
    ...
    TriangleSet2* aTriangleSet = myRaytraceGeometry.TriangleSet(aNodeIdx);
    ...
    if (aBvhBuffersSize != 0)
    {
      ...
      if (!aResult)
      {
        Message1::SendTrace() << "Error: Failed to upload buffers for bottom-level scene BVHs";
        return Standard_False;
      }
    }
    ...
    if (!aResult)
    {
      Message1::SendTrace() << "Error: Failed to upload triangulation buffers for OpenGL element";
      return Standard_False;
    }
  }
  ...
  if (myRaytraceGeometry.Materials.size() != 0)
  {
    ...
    if (!aResult)
    {
      Message1::SendTrace() << "Error: Failed to upload material buffer";
      return Standard_False;
    }
  }
  ...
  for (Standard_Integer anElemIdx = 0; anElemIdx < myRaytraceGeometry.Size(); ++anElemIdx)
  {
    TriangleSet2* aTriangleSet =
      dynamic_cast<TriangleSet2*>(myRaytraceGeometry.Objects()(anElemIdx).get());
                                                         ...
                                                         const Handle(OpenGl_Context)& theGlContext)
{
  ...
  for (size_t aLightIdx = 0, aRealIdx = 0; aLightIdx < aLightSources.size(); ++aLightIdx)
  {
    ...
    if (aLight.Type() != Graphic3d_TypeOfLightSource_Directional)
    {
      aPosition = BVH_Vec4f(static_cast<float>(aLight.Position1().X()),
                            static_cast<float>(aLight.Position1().Y()),
                            static_cast<float>(aLight.Position1().Z()),
                            1.0f);
    ...
    for (int aK = 0; aK < 4; ++aK)
    {
      wasUpdated |= (aEmission[aK] != myRaytraceGeometry.Sources[aRealIdx].Emission[aK])
                    || (aPosition[aK] != myRaytraceGeometry.Sources[aRealIdx].Position1[aK]);
    }

    if (wasUpdated)
    {
      myRaytraceGeometry.Sources[aRealIdx] = RaytraceLight(aEmission, aPosition);
    }
  ...
  if (myRaytraceGeometry.Sources.size() != 0 && wasUpdated)
  {
    ...
    if (!myRaytraceLightSrcTexture->Init(theGlContext,
                                         ...
                                         aDataPtr))
    {
      Message1::SendTrace() << "Error: Failed to upload light source buffer";
      return Standard_False;
    }
                                              ...
                                              const Handle(OpenGl_Context)& theGlContext)
{
  ...
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uOriginLB],
                         aOrigins[0]);
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uOriginRB],
                         aOrigins[1]);
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uOriginLT],
                         aOrigins[2]);
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uOriginRT],
                         aOrigins[3]);
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uDirectLB],
                         aDirects[0]);
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uDirectRB],
                         aDirects[1]);
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uDirectLT],
                         aDirects[2]);
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uDirectRT],
                         aDirects[3]);
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uViewPrMat],
                         aViewPrjMat);
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uUnviewMat],
                         anUnviewMat);

  // Set screen dimensions
  myRaytraceProgram->SetUniform(theGlContext,
                                myUniformLocations[theProgramId][OpenGl_RT_uWinSizeX],
                                theWinSizeX);
  myRaytraceProgram->SetUniform(theGlContext,
                                myUniformLocations[theProgramId][OpenGl_RT_uWinSizeY],
                                theWinSizeY);

  // Set 3D scene parameters
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uSceneRad],
                         myRaytraceSceneRadius);
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uSceneEps],
      ...
      && myBackgrounds[Graphic3d_TOB_GRADIENT]->IsDefined())
  {
    ...
    if (myCamera->Tile1().IsValid())
    {
      Standard_Integer aTileOffset = myCamera->Tile1().OffsetLowerLeft().y();
      Standard_Integer aTileSize   = myCamera->Tile1().TileSize.y();
      Standard_Integer aViewSize   = myCamera->Tile1().TotalSize.y();
  ...
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uBackColorTop],
                         aBackColorTop);
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uBackColorBot],
                         aBackColorBot);

  // Set environment map parameters
  const Handle(TextureSet2)& anEnvTextureSet =
  ...
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uEnvMapEnabled],
  ...
  if (myRaytraceParameters.CubemapForBack)
  {
    ...
    theProgram->SetUniform(theGlContext,
                           myUniformLocations[theProgramId][OpenGl_RT_uEnvMapForBack],
  ...
  theProgram->SetUniform(theGlContext,
                         myUniformLocations[theProgramId][OpenGl_RT_uLightAmbnt],
                         myRaytraceGeometry.Ambient);
  if (myRenderParams.IsGlobalIlluminationEnabled) // GI parameters
  {
    theProgram->SetUniform(theGlContext,
                           myUniformLocations[theProgramId][OpenGl_RT_uMaxRadiance],
                           myRenderParams.RadianceClampingValue);

    theProgram->SetUniform(theGlContext,
                           myUniformLocations[theProgramId][OpenGl_RT_uBlockedRngEnabled],
                           myRenderParams.CoherentPathTracingMode ? 1 : 0);
    ...
    if (myRenderParams.RadianceClampingValue != myRaytraceParameters.RadianceClampingValue
        || myRenderParams.UseEnvironmentMapBackground
             != myRaytraceParameters.UseEnvMapForBackground)
    {
      myAccumFrames = 0; // accumulation should be restarted
  ...
  else // RT parameters
  {
    ...
    theProgram->SetUniform(theGlContext,
                           myUniformLocations[theProgramId][OpenGl_RT_uShadowsEnabled],
                           myRenderParams.IsShadowEnabled ? 1 : 0);
    theProgram->SetUniform(theGlContext,
                           myUniformLocations[theProgramId][OpenGl_RT_uReflectEnabled],
                                       ...
                                       int                           theStereoView)
{
  if (myRaytraceParameters.AdaptiveScreenSampling && myRaytraceParameters.GlobalIllumination)
  {
    theGlContext->core42->glBindImageTexture(OpenGl_RT_OutputImage,
                                             myRaytraceOutputTexture[theStereoView]->TextureId(),
                                             0,
                                             GL_TRUE,
                                             0,
                                             GL_READ_WRITE,
                                             GL_R32F);
    theGlContext->core42->glBindImageTexture(
      OpenGl_RT_VisualErrorImage,
      myRaytraceVisualErrorTexture[theStereoView]->TextureId(),
      0,
      GL_TRUE,
      0,
      GL_READ_WRITE,
    ...
    if (myRaytraceParameters.AdaptiveScreenSamplingAtomic)
    {
      theGlContext->core42->glBindImageTexture(
        ...
        myRaytraceTileOffsetsTexture[theStereoView]->TextureId(),
        0,
        GL_TRUE,
        0,
        GL_READ_ONLY,
    ...
    else
    {
      theGlContext->core42->glBindImageTexture(
        ...
        myRaytraceTileSamplesTexture[theStereoView]->TextureId(),
        0,
        GL_TRUE,
        0,
        GL_READ_WRITE,
                                          ...
                                          const Handle(OpenGl_Context)& theGlContext)
{
  ...
  if (myRenderParams.IsAntialiasingEnabled)
  {
    ...
    aResult &= setUniformState(1 /* FSAA ID */, theSizeX, theSizeY, theProjection, theGlContext);
    ...
    for (Standard_Integer anIt = 1; anIt < 4; ++anIt)
    {
      ...
      aResult &= myPostFSAAProgram->SetUniform(theGlContext,
                                               myUniformLocations[1][OpenGl_RT_uSamples],
                                               anIt + 1);
      aResult &= myPostFSAAProgram->SetUniform(theGlContext,
    ...
    aDepthSourceFramebuffer->DepthStencilTexture()->Bind(theGlContext,
                                                         OpenGl_RT_RaytraceDepthTexture);

    // copy the output image with depth values
    theGlContext->core20fwd->glDrawArrays(GL_TRIANGLES, 0, 6);

    aDepthSourceFramebuffer->DepthStencilTexture()->Unbind(theGlContext,
                                                           OpenGl_RT_RaytraceDepthTexture);
                                           ...
                                           const Handle(OpenGl_Context)&      theGlContext)
{
  ...
  if (myRaytraceParameters.AdaptiveScreenSampling)
  {
    if (myAccumFrames == 0)
    {
      ...
      theGlContext->core44->glClearTexImage(myRaytraceOutputTexture[aFBOIdx]->TextureId(),
                                            0,
                                            GL_RED,
                                            GL_FLOAT,
                                            NULL);
    ...
    theGlContext->core44->glClearTexImage(myRaytraceVisualErrorTexture[aFBOIdx]->TextureId(),
                                          0,
                                          GL_RED_INTEGER,
                                          GL_INT,
                                          NULL);
  ...
  myRaytraceProgram->SetUniform(theGlContext,
                                myUniformLocations[0][OpenGl_RT_uAccumSamples],
                                myAccumFrames);
  ...
  {
    myRaytraceProgram->SetUniform(theGlContext,
                                  myUniformLocations[0][OpenGl_RT_uRenderImage],
                                  OpenGl_RT_OutputImage);
    myRaytraceProgram->SetUniform(theGlContext,
                                  myUniformLocations[0][OpenGl_RT_uTilesImage],
                                  OpenGl_RT_TileSamplesImage);
    myRaytraceProgram->SetUniform(theGlContext,
                                  myUniformLocations[0][OpenGl_RT_uOffsetImage],
                                  OpenGl_RT_TileOffsetsImage);
    myRaytraceProgram->SetUniform(theGlContext,
                                  myUniformLocations[0][OpenGl_RT_uTileSize],
  ...
  // Adaptive Screen Sampling computes the same overall amount of samples per frame redraw as normal
  // Path Tracing, but distributes them unequally across pixels (grouped in tiles), so that some
  // pixels do not receive new samples at all.
  //
  // Offsets map (redirecting currently rendered tile to another tile) allows performing Adaptive
  // Screen Sampling in single pass, but current implementation relies on atomic float operations
  // (AdaptiveScreenSamplingAtomic) for this. So that when atomic floats are not supported by GPU,
  // multi-pass rendering is used instead.
  //
  // Single-pass rendering is more optimal due to smaller amount of draw calls,
  // memory synchronization barriers, discarding most of the fragments and bad parallelization in
  // case of very small amount of tiles requiring more samples. However, atomic operations on float
  // values still produces different result (close, but not bit exact) making non-regression testing
  // not robust. It should be possible following single-pass rendering approach but using extra
  // accumulation buffer and resolving pass as possible improvement.
                                              ...
                                              const Handle(OpenGl_Context)&      theGlContext)
{
  ...
  if (myRaytraceParameters.AdaptiveScreenSampling)
  {
    ...
    myOutImageProgram->SetUniform(theGlContext,
                                  "uDebugAdaptive",
                                  myRenderParams.ShowSamplingTiles ? 1 : 0);
    myOutImageProgram->SetUniform(theGlContext, "uTileSize", myTileSampler.TileSize());
    myOutImageProgram->SetUniform(theGlContext,
                                  "uVarianceScaleFactor",
                                  myTileSampler.VarianceScaleFactor());
  }
  ...
  if (myRaytraceParameters.AdaptiveScreenSampling)
  {
    // Download visual error map from the GPU and build adjusted tile offsets for optimal image
    // sampling
    myTileSampler.GrabVarianceMap(theGlContext, myRaytraceVisualErrorTexture[aFBOIdx]);
    if (myRaytraceParameters.AdaptiveScreenSamplingAtomic)
    {
      myTileSampler.UploadOffsets(theGlContext,
                                  myRaytraceTileOffsetsTexture[aFBOIdx],
                                  myAccumFrames != 0);
    }
    else
    {
      myTileSampler.UploadSamples(theGlContext,
                                  myRaytraceTileSamplesTexture[aFBOIdx],
                                  myAccumFrames != 0);
    }
  }
  ...
  if (myIsRaytraceDataValid)
  {
    ...
    if (!aResult)
    {
      theGlContext->PushMessage(GL_DEBUG_SOURCE_APPLICATION,
                                ...
                                0,
                                GL_DEBUG_SEVERITY_MEDIUM,
                                "Error: Failed to execute ray-tracing shaders");
    }
