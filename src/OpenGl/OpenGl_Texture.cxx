...
static Graphic3d_Vec2i computeSmallestMipMapSize(const Graphic3d_Vec2i& theBaseSize,
                                                 Standard_Integer       theMaxLevel)
{
                          ...
                          const Image_PixMap*           theImage)
{
  ...
  if (theFormat.DataType() == GL_FLOAT && !theCtx->arbTexFloat)
  {
    theCtx->PushMessage(
  ...
  if (aDataPtr != NULL)
  {
    ...
    if (theCtx->hasUnpackRowLength)
    {
      theCtx->core11fwd->glPixelStorei(GL_UNPACK_ROW_LENGTH,
                                       (anExtraBytes >= anAligment) ? aPixelsWidth : 0);
    }
  ...
  switch (theType)
  {
    case Graphic3d_TypeOfTexture_1D: {
      ...
      if (toPatchExisting)
      {
        theCtx->core11fwd->glTexSubImage1D(GL_TEXTURE_1D,
                                           0,
                                           0,
                                           theSizeXYZ.x(),
                                           theFormat.PixelFormat(),
      ...
      theCtx->core11fwd->glTexImage1D(GL_TEXTURE_1D,
                                      0,
                                      anIntFormat,
                                      theSizeXYZ.x(),
                                      0,
    ...
    case Graphic3d_TOT_2D_MIPMAP: {
      ...
      theCtx->core11fwd->glTexImage2D(GL_TEXTURE_2D,
                                      0,
                                      anIntFormat,
                                      theSizeXYZ.x(),
                                      theSizeXYZ.y(),
    ...
    case Graphic3d_TypeOfTexture_3D: {
      ...
      theCtx->Functions()->glTexImage3D(GL_TEXTURE_3D,
                                        0,
                                        anIntFormat,
                                        theSizeXYZ.x(),
                                        theSizeXYZ.y(),
  ...
  switch (theTextureMap->Type())
  {
    case Graphic3d_TypeOfTexture_CUBEMAP: {
      return InitCubeMap(theCtx,
                         Handle(Graphic3d_CubeMap)::DownCast(theTextureMap),
                         0,
                         Image_Format_RGB,
                         false,
    ...
    default: {
      ...
      {
        if (Handle(Image_CompressedPixMap) aCompressed =
              theTextureMap->GetCompressedImage(theCtx->SupportedTextureFormats()))
        {
                                    ...
                                    const Standard_Boolean        theIsColorMap)
{
  ...
  if (theImage.SizeX() > theCtx->MaxTextureSize() || theImage.SizeY() > theCtx->MaxTextureSize())
  {
    theCtx->PushMessage(GL_DEBUG_SOURCE_APPLICATION,
                        GL_DEBUG_TYPE_ERROR,
                        0,
  ...
  if (myMaxMipLevel > 0 && !theImage.IsCompleteMipMapSet())
  {
    ...
    {
      Message1::SendTrace(AsciiString1("Warning: compressed 2D texture ") + myResourceId
                         + " " + mySize.x() + "x" + mySize.y() + " has smallest mipmap "
  ...
  for (Standard_Integer aMipIter = 0; aMipIter <= myMaxMipLevel; ++aMipIter)
  {
    ...
    theCtx->Functions()->glCompressedTexImage2D(GL_TEXTURE_2D,
                                                aMipIter,
                                                mySizedFormat,
                                                aMipSizeXY.x(),
                                                aMipSizeXY.y(),
                                       ...
                                       const Standard_Integer        theSizeY)
{
  ...
  myMaxMipLevel = 0;
  if (theSizeX > theCtx->MaxTextureSize() || theSizeY > theCtx->MaxTextureSize())
  {
    theCtx->PushMessage(GL_DEBUG_SOURCE_APPLICATION,
                        GL_DEBUG_TYPE_ERROR,
                        0,
      ...
      if (aFormat.IsValid())
      {
        ...
        if (myMaxMipLevel > 0 && !aCompImage->IsCompleteMipMapSet())
        {
          const Graphic3d_Vec2i aMipSize =
            computeSmallestMipMapSize(Graphic3d_Vec2i(aCompImage->SizeX(), aCompImage->SizeY()),
                                      myMaxMipLevel);
          if (!theCtx->HasTextureBaseLevel())
          {
            myMaxMipLevel = 0;
          ...
          {
            Message1::SendTrace(AsciiString1("Warning: Cubemap compressed texture ")
                               + theCubeMap->GetId() + " " + aCompImage->SizeX() + "x"
    ...
    if (!theCubeMap.IsNull())
    {
      ...
      if (!aCompImage.IsNull())
      {
        ...
        for (Standard_Integer aMipIter = 0; aMipIter <= myMaxMipLevel; ++aMipIter)
        {
          ...
          theCtx->Functions()->glCompressedTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i,
                                                      aMipIter,
                                                      mySizedFormat,
                                                      aMipSizeXY.x(),
                                                      aMipSizeXY.y(),
      ...
      if (!anImage.IsNull())
      {
        ...
        if (aRowLength > 0 && !theCtx->hasUnpackRowLength)
        {
          Handle(Image_PixMap) aCopyImage = new Image_PixMap();
          aCopyImage->InitTrash(theFormat, theSize, theSize);
          const Standard_Size aRowBytesPacked =
            std::min(aCopyImage->SizeRowBytes(), anImage->SizeRowBytes());
