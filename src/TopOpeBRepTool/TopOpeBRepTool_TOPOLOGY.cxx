...
{
  Standard_Real       tolF = BRepInspector::Tolerance(TopoDS::Face(F));
...
/*// ----------------------------------------------------------------------
Standard_EXPORT Standard_Boolean FUN_tool_IsUViso(const TopoShape& E,const TopoShape& F,
...
// ----------------------------------------------------------------------
Standard_EXPORT Standard_Boolean FUN_tool_bounds(const TopoShape& Sh,
                                                 Standard_Real&      u1,
                                                 Standard_Real&      u2,
                                                 Standard_Real&      v1,
                                                 //				                 Standard_Real& v2)
...
{
  Bnd_Box2d          B2d;
  const TopoFace& F = TopoDS::Face(Sh);
  ShapeExplorer    ex(F, TopAbs_WIRE);
  for (; ex.More(); ex.Next())
  {
    const TopoWire W = TopoDS::Wire(ex.Current());
    Bnd_Box2d         newB2d;
...
// ----------------------------------------------------------------------
Standard_EXPORT Standard_Boolean FUN_tool_isobounds(const TopoShape& Sh,
                                                    Standard_Real&      u1,
                                                    Standard_Real&      u2,
                                                    Standard_Real&      v1,
                                                    Standard_Real&      v2)
{
  ...
  for (; ex.More(); ex.Next())
  {
    ...
    gp_Pnt2d         o2d;
    gp_Dir2d         d2d;
    Standard_Boolean isouv = TOOL1::UVISO(PC, isou, isov, d2d, o2d);
...
{
  Handle(GeomSurface) S = ShapeTool::BASISSURFACE(TopoDS::Face(Sh));
  if (S.IsNull())
    return Standard_False;
...
// ----------------------------------------------------------------------
Standard_EXPORT Standard_Boolean FUN_tool_value(const gp_Pnt2d& UV, const TopoFace& F, Point3d& P)
{
                                                     ...
                                                     TopAbs_Orientation& oriEinF)
{
  ...
  ShapeExplorer e(F, TopAbs_EDGE);
  for (; e.More(); e.Next())
  {
    const TopoShape& EF = e.Current();
    if (EF.IsSame(E))
    {
                                                            ...
                                                            TopAbs_Orientation& oriEinF)
{
  ...
  TopoFace  FF          = TopoDS::Face(aLocalShape);
...
Standard_EXPORT Standard_Boolean FUN_tool_EboundF(const TopoEdge& E, const TopoFace& F)
{
  ...
  if (!ok)
    return Standard_False;
  Standard_Boolean closingE = BRepInspector::IsClosed(E, F);
  if (closingE)
    return Standard_True;
                                                  ...
                                                  const Standard_Real  tol)
{
  ...
  {
    Standard_Real        ttol;
    Handle(GeomCurve2d) c2d = FC2D_CurveOnSurface(E, F, f, l, ttol);
    project                  = c2d.IsNull();
    if (!project)
...
{
  Vector3d           tgE;
  Standard_Boolean ok = TOOL1::TggeomE(paronE, E, tgE);
  if (!ok)
...
{
  Vector3d           tgOOE;
  Standard_Boolean ok = TOOL1::TggeomE(paronOOE, OOE, tgOOE);
  if (!ok)
    return Standard_False; // NYIRAISE
  Vector3d tgE;
  ok = TOOL1::TggeomE(paronE, E, tgE);
  if (!ok)
    return Standard_False; // NYIRAISE
                                                ...
                                                Standard_Real&         xsup)
{
  ...
  for (; ex.More(); ex.Next())
  {
    ...
    gp_Pnt2d           o2d;
    gp_Dir2d           d2d;
    Standard_Boolean   isouv = TOOL1::UVISO(E, F, isou, isov, d2d, o2d);
    if (!isouv)
      return Standard_False;
                                                        ...
                                                        const Standard_Real parOnEi)
{
  ...
  Vector3d           tgEi;
  Standard_Boolean ok = TOOL1::TggeomE(parOnEi, Ei, tgEi);
  if (!ok)
    return Standard_False; // NYIRAISE
...
{
  //  const Handle(GeomSurface)& S = BRepInspector::Surface(TopoDS::Face(F));
  Handle(GeomSurface) S = ShapeTool::BASISSURFACE(TopoDS::Face(F));
  if (S.IsNull())
    return Standard_False;
...
// ----------------------------------------------------------------------
Standard_EXPORT void FUN_tool_mkBnd2d(const TopoShape& W, const TopoShape& FF, Bnd_Box2d& B2d)
{
  ...
  {
    //  for (ShapeExplorer ex(W,TopAbs_EDGE); ex.More(); ex.Next()) {
    const TopoEdge&   E = TopoDS::Edge(ex.Current());
...
/*// ----------------------------------------------------------------------
Standard_EXPORT Standard_Boolean FUN_tool_ClosedE(const TopoEdge& E,TopoShape& vclosing)
{
                                              ...
                                              const TopoShape& shape)
{
  ...
  TopExp1::MapShapes(shape, sstyp, M);
...
{
  ShapeList e1s;
  ShapeExplorer      ex(F1, TopAbs_EDGE);
  for (; ex.More(); ex.Next())
  {
    const TopoShape& e1   = ex.Current();
    Standard_Boolean    e1ok = Standard_False;
    ShapeExplorer     exv(e1, TopAbs_VERTEX);
                                                  ...
                                                  Standard_Real&       par)
{
  ...
  for (ex.Init(E, TopAbs_VERTEX); ex.More(); ex.Next())
  {
    ...
    if (isVofE)
    {
      par = BRepInspector::Parameter(TopoDS::Vertex(ex.Current()), E);
      break;
    }
  }
  if (!isVofE)
  {
    Point3d pt = BRepInspector::Pnt(v);
    // <v> can share same domain with a vertex of <E>
    for (ex.Init(E, TopAbs_VERTEX); ex.More(); ex.Next())
    {
      const TopoVertex& vex  = TopoDS::Vertex(ex.Current());
      Point3d               ptex = BRepInspector::Pnt(vex);
      if (ptex.IsEqual(pt, tol))
      {
        par = BRepInspector::Parameter(vex, E);
        return Standard_True;
      }
    }
...
Standard_EXPORT Standard_Boolean FUN_tool_parF(const TopoEdge&   E,
                                               ...
                                               gp_Pnt2d&            UV)
{
  Standard_Real    tol3d = BRepInspector::Tolerance(F) * 1.e2; // KK xpu
...
Standard_EXPORT Standard_Boolean FUN_tool_paronEF(const TopoEdge&   E,
                                                  ...
                                                  gp_Pnt2d&            UV)
{
  Standard_Real    tol3d = BRepInspector::Tolerance(F) * 1.e2; // KKxpu
...
// purpose : returns maxtol of <S>'s shapes of type <typ>
{
  ...
  for (; ex.More(); ex.Next())
  {
    ...
    if (face)
      tolss = BRepInspector::Tolerance(TopoDS::Face(ss));
    if (edge)
      tolss = BRepInspector::Tolerance(TopoDS::Edge(ss));
    if (vertex)
      tolss = BRepInspector::Tolerance(TopoDS::Vertex(ss));
    if (tolss > maxtol)
      maxtol = tolss;
  }
  return hasshatyp;
...
//                2- C3d<E1> contains C3d<E2>
{
  ...
  Vector3d           tmp;
  Standard_Boolean ok = TOOL1::TggeomE(mid, E2, tmp);
  if (!ok)
    return Standard_False;
  Dir3d             tgE2(tmp);
  ...
  Standard_Real tol2 = BRepInspector::Tolerance(E2);
  Standard_Real tol  = Max(tol1, tol2) * 10.;
  if (dist > tol)
    return Standard_False;
  if (!ok)
    return Standard_False;
  ok = TOOL1::TggeomE(pE1, E1, tmp);
  if (!ok)
    return Standard_False;
  Dir3d             tgE1(tmp);
...
{
  Standard_Real        f, l, tol;
  Handle(GeomCurve2d) C2d  = FC2D_CurveOnSurface(E, F, f, l, tol);
                                                    ...
                                                    TopoFace&                newf)
{
  ...
  for (; exw.More(); exw.Next())
  {
    ...
    for (; exe.More(); exe.Next())
    {
      ...
      TopoEdge newe = faultyE;
      //      TopoEdge newe; FUN_ds_CopyEdge(e,newe); newe.Orientation(TopAbs_FORWARD);
  ...
  if (hasnewf)
  {
    TopoShape aLocalShape = fF.EmptyCopied();
                                                   ...
                                                   Standard_Boolean&   so)
{
  ...
  Vector3d            tg1;
  Standard_Boolean  ok = TOOL1::TggeomE(p1, E1, tg1);
  if (!ok)
    return Standard_False; // NYIRAISE
  Vector3d tg2;
                                                   ...
                                                   Standard_Boolean&  so)
{
  ...
  TopoVertex vf1, vl1;
  TopExp1::Vertices(E1, vf1, vl1);
  Standard_Boolean closed1 = vf1.IsSame(vl1);
  TopoVertex    vf2, vl2;
  TopExp1::Vertices(E2, vf2, vl2);
                                                   ...
                                                   TopoWire&                newW)
{
  ...
  {
    const TopoEdge& E = TopoDS::Edge(itloE.Value());
    BB.Add(newW, E);
  }
  return Standard_True;
}
