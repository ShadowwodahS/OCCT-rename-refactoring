                     ...
                     Vector3d&                          D1VNormal)
{
  ...
  GeomAbs_Shape Cont = (Surf->Surface().UContinuity() < Surf->Surface().VContinuity())
                         ? (Surf->Surface().UContinuity())
                         : (Surf->Surface().VContinuity());
  if (Cont == GeomAbs_C0 || Cont == GeomAbs_C1)
  {
                     ...
                     Vector3d&                          D2UVNormal)
{
  ...
  GeomAbs_Shape Cont = (Surf->Surface().UContinuity() < Surf->Surface().VContinuity())
                         ? (Surf->Surface().UContinuity())
                         : (Surf->Surface().VContinuity());
  if (Cont == GeomAbs_C0 || Continuity == GeomAbs_C1 || Cont == GeomAbs_C2)
  {
    throw Geom_UndefinedDerivative();
  }
#endif
                                      ...
                                      Vector3d&             BiNormal)
{
  ...
  Handle(SurfaceAdaptor) mySupport =
                                      ...
                                      Vector3d&             DBiNormal)
{
  ...
  DF      = D2_2d.X() * dS_du + D2_2d.Y() * dS_dv + d2S_du * D2d.X() * D2d.X()
       + 2 * d2S_duv * D2d.X() * D2d.Y() + d2S_dv * D2d.Y() * D2d.Y();
                                      ...
                                      Vector3d&             D2BiNormal)
{
  ...
  D2F = D3_2d.X() * dS_du + D3_2d.Y() * dS_dv + D2_2d.X() * (D2d.X() * d2S_du + D2d.Y() * d2S_duv)
  ...
  DNormal = BiNormal.Crossed(DTangent) + DBiNormal.Crossed(Tangent);
  D2Normal =
    BiNormal.Crossed(D2Tangent) + 2 * DBiNormal.Crossed(DTangent) + D2BiNormal.Crossed(Tangent);
