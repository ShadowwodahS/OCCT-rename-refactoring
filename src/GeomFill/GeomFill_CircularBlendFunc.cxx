                           ...
                           TColStd_SequenceOfReal&     Seq)
{
  ...
  constexpr Standard_Real Epspar = Precision1::PConfusion() * 0.99;
  ...
  if (Samp.IsDone())
  {
    ...
    for (ii = 1; ii <= Samp.NbPoints(); ii++)
    {
      ...
      /*
            ...
            DrawTrSurf1::Set(name, P1);
            sprintf(name,"PNT_%d",NbSections++);
            DrawTrSurf1::Set(name, P2);*/
      myPath->D0(T, Center);
                                                ...
                                                TColStd_Array1OfReal& Weigths)
{
  ...
  temp.SetLinearForm(myRadius, ns1.XYZ(), myRadius, ns2.XYZ(), 1, P1.XYZ(), P2.XYZ());
...
//    new BSplineCurve3d(Poles,Weights,Knots,Mults,Degree);
...
Standard_Boolean GeomFill_CircularBlendFunc::D1(
...
Standard_Boolean GeomFill_CircularBlendFunc::D2(
  const Standard_Real Param,
  //						const Standard_Real First,
  const Standard_Real,
  //						const Standard_Real Last,
  ...
  TColStd_Array1OfReal& D2Weigths)
{
  ...
  if (!ns1.IsParallel(ns2, TolAng))
  {
    nplan  = ns1.Crossed(ns2);
    dnplan = Dns1.Crossed(ns2).Added(ns1.Crossed(Dns2));
    d2nplan.SetLinearForm(1, D2ns1.Crossed(ns2), 2, Dns1.Crossed(Dns2), ns1.Crossed(D2ns2));
  }
  else
  {
  ...
  ns1 *= invnorm1;
  sc = Dns1.Dot(ns1);
  D2ns1.SetLinearForm(3 * sc * sc * invnorm1 - D2ns1.Dot(ns1) - invnorm1 * Dns1.SquareMagnitude(),
                      ns1,
                      -2 * sc * invnorm1,
                      Dns1,
                      D2ns1);
  ...
  ns2 *= invnorm2;
  sc = Dns2.Dot(ns2);
  D2ns2.SetLinearForm(3 * sc * sc * invnorm2 - D2ns2.Dot(ns2) - invnorm2 * Dns2.SquareMagnitude(),
                      ns2,
                      -2 * sc * invnorm2,
                      Dns2,
                      D2ns2);
  ...
  d2nplan.SetLinearForm(3 * sc * sc * invnormp - d2nplan.Dot(nplan)
                          - invnormp * dnplan.SquareMagnitude(),
                        nplan,
                        -2 * sc * invnormp,
                        dnplan,
                        d2nplan);
...
Standard_Integer GeomFill_CircularBlendFunc::NbIntervals(const GeomAbs_Shape S) const
{
...
void GeomFill_CircularBlendFunc::Intervals(TColStd_Array1OfReal& T, const GeomAbs_Shape S) const
{
  Standard_Integer NbI_Center, NbI_Cb1, NbI_Cb2, ii;
  NbI_Center = myPath->NbIntervals(GeomFillNextShape(S));
  NbI_Cb1    = myCurve1->NbIntervals(S);
...
void GeomFill_CircularBlendFunc::SetInterval(const Standard_Real First, const Standard_Real Last)
{
