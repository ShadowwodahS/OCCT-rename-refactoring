...
static void BCSmoothing(Handle(BSplineCurve3d)& theC,
...
static void MakeClosedCurve(Handle(GeomCurve3d)& C,
                            const Point3d&       PF,
                            Standard_Real&      f,
                            Standard_Real&      l)
{
  Handle(BSplineCurve3d) aBC   = Handle(BSplineCurve3d)::DownCast(C);
  ...
  if (porig > l)
  {
    ...
    Extrema_LocateExtPC aPrj(PF, aGAC, pmin, Precision1::PConfusion());
    if (aPrj.IsDone())
    {
  ...
  aBC->SetOrigin(porig, Precision1::PConfusion());
  f = aBC->FirstParameter();
  l = aBC->LastParameter();
  aBC->Segment1(f, l);
  if (aCont > GeomAbs_C0 && aBC->Continuity() == GeomAbs_C0)
  {
    BCSmoothing(aBC, 1, Precision1::Confusion());
  }
  C = aBC;
  f = C->FirstParameter();
  l = C->LastParameter();
...
TopoShape& BRepLib_FuseEdges::Shape()
{
  Standard_NullObject_Raise_if(myShape.IsNull(), "FuseEdges : No Shape");
...
void BRepLib_FuseEdges::BuildListResultEdges()
{
  ...
  if (myMapLstEdg.Extent() > 0)
  {
    ...
    for (itLstEdg.Initialize(myMapLstEdg); itLstEdg.More(); itLstEdg.Next())
    {
      ...
      TopoEdge OldEdge = TopoDS::Edge(LmapEdg.First());
      ...
      if (myConcatBSpl)
      {
        ...
        if (C->DynamicType() == STANDARD_TYPE(BSplineCurve3d))
        {
          ...
          {
            Handle(GeomCurve3d) aC = BRepInspector::Curve(TopoDS::Edge(anEdgIter.Value()), f, l);
            aTC                   = new Geom_TrimmedCurve(aC, f, l);
            if (!Concat.Add(aTC, Precision1::Confusion()))
            {
              // cannot merge curves
      ...
      Standard_Boolean isBSpline = C->DynamicType() == STANDARD_TYPE(BSplineCurve3d);
...
void BRepLib_FuseEdges::BuildListConnexEdge(const TopoShape&   theEdge,
...
Standard_Boolean BRepLib_FuseEdges::NextConnexEdge(const TopoVertex& theVertex,
...
Standard_Boolean BRepLib_FuseEdges::SameSupport(const TopoEdge& E1, const TopoEdge& E2) const
{
...
Standard_Boolean BRepLib_FuseEdges::UpdatePCurve(const TopoEdge&          theOldEdge,
                                                 TopoEdge&                theNewEdge,
