...
RWMesh_FaceIterator::RWMesh_FaceIterator(const DataLabel&       theLabel,
...
RWMesh_FaceIterator::RWMesh_FaceIterator(const TopoShape&  theShape,
                                         const XCAFPrs_Style& theStyle)
...
Dir3d RWMesh_FaceIterator::normal(Standard_Integer theNode) const
{
  ...
  {
    const Coords2d anUV = myPolyTriang->UVNode(theNode).XY();
...
void RWMesh_FaceIterator::Next()
{
  for (; myIter.More(); myIter.Next())
  {
    ...
    myPolyTriang = BRepInspector::Triangulation(myFace, myLocation);
...
void RWMesh_FaceIterator::initFace()
{
  ...
  {
    TopoFace aFaceFwd = TopoDS::Face(myFace.Oriented(TopAbs_FORWARD));
    aFaceFwd.Location(TopLoc_Location());
    TopLoc_Location aLoc;
    if (!BRepInspector::Surface(aFaceFwd, aLoc).IsNull())
    {
