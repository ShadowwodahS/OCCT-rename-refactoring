          ...
          if (!start->Curve3D().IsNull())
          {
            if (IGESToBRep1::IsTopoCurve(start->Curve3D()))
            {
              ...
              if (!Sh.IsNull())
              {
                ...
                // CurveOnSurface on Composite Surface case not implemented : 3D representation
                // returned.
  ...
  const Standard_Real                uFact)
{
  ...
  {
    Handle(Geom2d_BSplineCurve) BSplineC2d = Handle(Geom2d_BSplineCurve)::DownCast(C2d);
