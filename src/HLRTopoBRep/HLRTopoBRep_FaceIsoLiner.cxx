                                       ...
                                       const Standard_Integer nbIsos)
{
  ...
  for (ExpEdges.Init(TF, TopAbs_EDGE); ExpEdges.More(); ExpEdges.Next())
  {
    ...
    {
      Handle(Geom2d_TrimmedCurve) TPC = new Geom2d_TrimmedCurve(PC, U1, U2);
    ...
    for (IIso = 1; IIso <= nbIsos; IIso++)
    {
      ...
      if (NbDom > 0)
      {
        ...
        for (Standard_Integer IDom = 1; IDom <= NbDom; IDom++)
        {
          ...
          Standard_Real U11 = Dom.HasFirstPoint() ? Dom.FirstPoint().Parameter() : VMin - Infinite;
          Standard_Real U21 =
  ...
  if (StepV > Confusion)
  {
    ...
    for (IIso = 1; IIso <= nbIsos; IIso++)
    {
      ...
      if (NbDom > 0)
      {
        ...
        {
          const HatchGen_Domain& Dom = Hatcher.Domain(IndH, IDom);
          Standard_Real U12 = Dom.HasFirstPoint() ? Dom.FirstPoint().Parameter() : VMin - Infinite;
          Standard_Real U22 =
            Dom.HasSecondPoint() ? Dom.SecondPoint().Parameter() : VMax + Infinite;
          IsoLine->D0(U12, P);
          Surface.D0(P.X(), P.Y(), P1);
