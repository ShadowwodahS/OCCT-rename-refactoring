...
void BlendFunc_EvolRadInv::GetBounds(math_Vector& InfBound, math_Vector& SupBound) const
{
...
Standard_Boolean BlendFunc_EvolRadInv::Values(const math_Vector& X, math_Vector& F, math_Matrix& D)
{
  ...
  grosterme = ncrossns1.Dot(nplan.Crossed(temp)) / norm1 / norm1;
  resul2.SetLinearForm(-sg1 * ray / norm1 * (grosterme * ndotns1 - nplan.Dot(temp)),
                       nplan,
                       sg1 * ray * grosterme / norm1,
                       ns1,
                       -sg1 * ray / norm1,
  ...
  resul2.SetLinearForm(sg2 / norm2 * (grosterme * ndotns2 - dnplan.Dot(ns2)),
                       nplan,
                       -sg2 * ndotns2 / norm2,
                       dnplan,
                       -sg2 * grosterme / norm2,
  ...
  temp.SetLinearForm(sg1 * ndotns1 / norm1 - sg2 * ndotns2 / norm2,
                     nplan,
                     -sg1 / norm1,
                     ns1,
                     sg2 / norm2,
