...
BlendFunc_EvolRad::BlendFunc_EvolRad(const Handle(SurfaceAdaptor)& S1,
                                                  ...
                                                  const Standard_Real    Param)
{
  ...
  Vector3d        ncrossns1, ncrossns2, resul, temp, n1, n2;
  ...
  resul.SetLinearForm(ray1, n1, -ray2, n2, Vector3d(pts2, pts1));
...

{
  ...
  if (Abs(E(1)) <= Tol && E(2) * E(2) + E(3) * E(3) + E(4) * E(4) <= Tol * Tol)
  {
    ...
    if (Abs(Angle * ray) < lengthmin)
    {
      lengthmin = Abs(Angle * ray);
    }
    if (Abs(Angle * ray) > lengthmax)
    {
      lengthmax = Abs(Angle * ray);
    }
...
const Vector3d& BlendFunc_EvolRad::TangentOnS1() const
{
  if (istangent)
  {
    throw Standard_DomainError();
...
const Vector3d& BlendFunc_EvolRad::TangentOnS2() const
{
  if (istangent)
  {
    throw Standard_DomainError();
                                            ...
                                            TColStd_Array1OfReal& D2Weights)
{
  ...
  */
  ...
  if (ray < 0.)
  { // to avoid Abs(dray) several lines below
    rayprim = -rayprim;
    raysecn = -raysecn;
  }
