...
GeomConvert_CurveToAnaCurve::GeomConvert_CurveToAnaCurve()
    : myGap(Precision1::Infinite()),
      myConvType(GeomConvert_MinGap),
      myTarget(GeomAbs_Line)
{
}

GeomConvert_CurveToAnaCurve::GeomConvert_CurveToAnaCurve(const Handle(GeomCurve3d)& C)
    : myGap(Precision1::Infinite()),
      myConvType(GeomConvert_MinGap),
      myTarget(GeomAbs_Line)
{
...
void GeomConvert_CurveToAnaCurve::Init(const Handle(GeomCurve3d)& C)
{
  myCurve = C;
  myGap   = Precision1::Infinite();
}
  ...
  Standard_Real&      NewL)
{
  ...
  Handle(GeomCurve3d) C = ComputeCurve(aCurve, tol, F, L, NewF, NewL, myGap, myConvType, myTarget);
                                                       ...
                                                       Standard_Real&            Deviation)
{
  ...
  if (dMax < Precision1::SquareConfusion())
    return Standard_False;
...
gp_Lin GeomConvert_CurveToAnaCurve::GetLine(const Point3d&  P1,
                                            const Point3d&  P2,
                                            Standard_Real& cf,
                                            Standard_Real& cl)
{
...
Handle(GeomLine) GeomConvert_CurveToAnaCurve::ComputeLine(const Handle(GeomCurve3d)& curve,
                                                           const Standard_Real       tolerance,
                                                           ...
                                                           Standard_Real&            Deviation)
{
  Handle(GeomLine) line;
  if (curve.IsNull())
    return line;
  line = Handle(GeomLine)::DownCast(curve); // qui sait
  if (!line.IsNull())
  {
    cf        = c1;
    cl        = c2;
  ...
  Point3d P1 = curve->Value(c1);
  Point3d P2 = curve->Value(c2);
  if (P1.SquareDistance(P2) < Precision1::SquareConfusion())
    return line;
  cf = c1;
  cl = c2;
  ...
  {
    Handle(BezierCurve3d) bzc = Handle(BezierCurve3d)::DownCast(curve);
    if (!bzc.IsNull())
    {
      nbPoles = bzc->NbPoles();
      Poles   = new TColgp_HArray1OfPnt(1, nbPoles);
                                                        ...
                                                        const Point3d& P2)
{
  ...
  Standard_Real aMaxCoord = Sqrt(Precision1::Infinite());
...
Handle(GeomCurve3d) GeomConvert_CurveToAnaCurve::ComputeCircle(const Handle(GeomCurve3d)& c3d,
                                                              const Standard_Real       tol,
                                                              const Standard_Real       c1,
                                                              const Standard_Real       c2,
                                                              Standard_Real&            cf,
                                                              ...
                                                              Standard_Real&            Deviation)
{
  if (c3d->IsKind(STANDARD_TYPE(GeomCircle)))
  {
    cf                        = c1;
    cl                        = c2;
    Deviation                 = 0.;
    Handle(GeomCircle) aCirc = Handle(GeomCircle)::DownCast(c3d);
    return aCirc;
  }
  ...
  if (Abs(cf) < Precision1::PConfusion() || Abs(PI2 - cf) < Precision1::PConfusion())
    cf = 0.;
                                         ...
                                         const Standard_Real                prec)
{
  ...
  if (size < 3)
    return Standard_False;
  Point3d        P1    = HAP->Value(1);
  Point3d        P2    = HAP->Value(2);
  Point3d        P3    = HAP->Value(3);
  ...
  for (i = 1; i <= 3; ++i)
  {
    if (Precision1::IsInfinite(NV.Coord(i)))
      return Standard_False;
  }
  ...
  if (size > 3)
  {
    for (i = 4; i <= size; i++)
    {
      Point3d PN = HAP->Value(i);
      dist1     = P1.Distance(PN);
      if (dist1 < prec || Precision1::IsInfinite(dist1))
      {
        return Standard_False;
      }
      Vector3d VN(P1, PN);
...
Handle(GeomCurve3d) GeomConvert_CurveToAnaCurve::ComputeEllipse(const Handle(GeomCurve3d)& c3d,
                                                               const Standard_Real       tol,
                                                               const Standard_Real       c1,
                                                               const Standard_Real       c2,
                                                               Standard_Real&            cf,
                                                               ...
                                                               Standard_Real&            Deviation)
{
  ...
  Point3d PStart = c3d->Value(c1);
  Point3d PEnd   = c3d->Value(c2);
  ...
  if (Q2 > 0 && Q1 * Q3 < 0)
  {
    ...
    {
      // create ellipse
      if (Rmax - Rmin < Precision1::Confusion())
      {
        return res; // really it is circle, which must be recognized in other method
      }
      aTrans *= -1;
      Center.SetZ(ZN);
      Point3d NewCenter = Center.Transformed(Tr2);
      Point3d Ptmp(Center.X() + MainAxis.X() * 10,
...
Handle(GeomCurve3d) GeomConvert_CurveToAnaCurve::ComputeCurve(const Handle(GeomCurve3d)&  theC3d,
                                                             const Standard_Real        tolerance,
                                                             const Standard_Real        c1,
                                                             const Standard_Real        c2,
                                                             Standard_Real&             cf,
                                                             ...
                                                             const GeomAbs_CurveType    theTarget)
{
  cf                                       = c1;
  cl                                       = c2;
  std::array<Handle(GeomCurve3d), 3> newc3d = {};
  Handle(GeomCurve3d)                c3d    = theC3d;
  if (c3d.IsNull())
    return c3d;
  Point3d                       P1 = c3d->Value(c1);
  Point3d                       P2 = c3d->Value(c2);
  Point3d                       P3 = c3d->Value(c1 + (c2 - c1) / 2);
  ...
  else
  {
    ...
    if (!Precision1::IsInfinite(c1) && !Precision1::IsInfinite(c2))
    {
      d[1]      = RealLast();
