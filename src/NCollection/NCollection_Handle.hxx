...
//! Handle(RefObject) in OCCT components.

template <class T>
class NCollection_Handle : public opencascade::handle<RefObject>
{
private:
  //! Internal adaptor class wrapping actual type
  //! and enhancing it by reference counter inherited from
  //! RefObject
  class Ptr : public RefObject
  {
  public:
  ...
  NCollection_Handle(Ptr* thePtr, int)
      : opencascade::handle<RefObject>(thePtr)
  {
  }
...
public:
  ...
  NCollection_Handle(T* theObject)
      : opencascade::handle<RefObject>(theObject ? new Ptr(theObject) : 0)
  {
  }

  //! Cast handle to contained type
  T* get() { return ((Ptr*)opencascade::handle<RefObject>::get())->myPtr; }

  //! Cast handle to contained type
  const T* get() const { return ((Ptr*)opencascade::handle<RefObject>::get())->myPtr; }
  ...
  static NCollection_Handle<T> DownCast(const opencascade::handle<RefObject>& theOther)
  {
