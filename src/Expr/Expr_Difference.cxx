...
Expr_Difference::Expr_Difference(const Handle(Expression1)& exp1,
...
Handle(Expression1) Expr_Difference::ShallowSimplified() const
{
  ...
  if (unfirst && unsecond)
  {
    ...
    Handle(Expression1) ssop1 = myfirst->SubExpression(1);
    Handle(Expression1) ssop2 = mysecond->SubExpression(1);
    return ssop2 - ssop1;
  }
  if (unfirst && !unsecond)
  {
...
Handle(Expression1) Expr_Difference::Copy() const
{
  return Expr1::CopyShare(FirstOperand()) - Expr1::CopyShare(SecondOperand());
}

Standard_Boolean Expr_Difference::IsIdentical(const Handle(Expression1)& Other) const
{
...
Handle(Expression1) Expr_Difference::Derivative(const Handle(Expr_NamedUnknown)& X) const
{
  if (!Contains(X))
  {
...
Handle(Expression1) Expr_Difference::NDerivative(const Handle(Expr_NamedUnknown)& X,
