...
Expr_Tangent::Expr_Tangent(const Handle(Expression1)& exp)
{
  CreateOperand(exp);
}

Handle(Expression1) Expr_Tangent::ShallowSimplified() const
{
...
Handle(Expression1) Expr_Tangent::Copy() const
{
  return new Expr_Tangent(Expr1::CopyShare(Operand()));
}

Standard_Boolean Expr_Tangent::IsIdentical(const Handle(Expression1)& Other) const
{
...
Handle(Expression1) Expr_Tangent::Derivative(const Handle(Expr_NamedUnknown)& X) const
{
  if (!Contains(X))
  {
