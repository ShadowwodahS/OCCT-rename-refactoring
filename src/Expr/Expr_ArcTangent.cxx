...
Expr_ArcTangent::Expr_ArcTangent(const Handle(Expression1)& exp)
{
  CreateOperand(exp);
}

Handle(Expression1) Expr_ArcTangent::ShallowSimplified() const
{
...
Handle(Expression1) Expr_ArcTangent::Copy() const
{
  return new Expr_ArcTangent(Expr1::CopyShare(Operand()));
}

Standard_Boolean Expr_ArcTangent::IsIdentical(const Handle(Expression1)& Other) const
{
...
Handle(Expression1) Expr_ArcTangent::Derivative(const Handle(Expr_NamedUnknown)& X) const
{
  if (!Contains(X))
  {
