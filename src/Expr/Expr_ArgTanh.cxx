...
Expr_ArgTanh::Expr_ArgTanh(const Handle(Expression1)& exp)
{
  CreateOperand(exp);
}

Handle(Expression1) Expr_ArgTanh::ShallowSimplified() const
{
...
Handle(Expression1) Expr_ArgTanh::Copy() const
{
  return new Expr_ArgTanh(Expr1::CopyShare(Operand()));
}

Standard_Boolean Expr_ArgTanh::IsIdentical(const Handle(Expression1)& Other) const
{
...
Handle(Expression1) Expr_ArgTanh::Derivative(const Handle(Expr_NamedUnknown)& X) const
{
  if (!Contains(X))
  {
