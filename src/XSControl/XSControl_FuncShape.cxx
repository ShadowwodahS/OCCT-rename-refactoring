...
static IFSelect_ReturnStatus XSControl_tpdraw(const Handle(IFSelect_SessionPilot)& pilot)
{
  ...
  if (mode == 0)
  {
    ...
    if (arg1[0] == '*')
      ...
      num = IFSelectFunctions::GiveEntityNumber(XSControl1::Session(pilot), arg1);
  }
  else
  {
    if (arg2[0] == '*')
      ...
      num = IFSelectFunctions::GiveEntityNumber(XSControl1::Session(pilot), arg2);
  }
  ...
  for (i = n1; i <= n2; i++)
  {
    ...
    sh = TransferBRep1::ShapeResult(binder);
    ...
    if (!sh.IsNull())
    {
      ...
      XSControl1::Vars(pilot)->SetShape(nomvar, sh);
      continue;
    }
    DeclareAndCast(TransferBRep_ShapeListBinder, slb, binder);
    if (!slb.IsNull())
    {
      Standard_Integer nbs = slb->NbShapes1();
      if (tout)
        sout << "[ " << i << " ]:";
      if (num == 0)
        sout << " pas dans le modele";
    ...
    if (!trb.IsNull())
    {
      ...
      char* nomv = nomvar;
      XSControl1::Vars(pilot)->Set(nomv, geom);
...
static IFSelect_ReturnStatus XSControl_tpcompound(const Handle(IFSelect_SessionPilot)& pilot)
{
  ...
  else
  {
    ...
    list = TransferBRep1::Shapes(TP, lise);
  ...
  XSControl1::Vars(pilot)->SetShape(arg1, C);
  return IFSelect_RetDone;
}
...
static IFSelect_ReturnStatus XSControl_traccess(const Handle(IFSelect_SessionPilot)& pilot)
{
  ...
  Standard_Integer num =
    (argc > 1 ? IFSelectFunctions::GiveEntityNumber(XSControl1::Session(pilot), arg1) : 0);
  ...
  if (num == 0 || cascomp)
  {
    TopoCompound C; // pour cas compound
    ShapeBuilder    B;
    B.MakeCompound(C);
    ...
    for (Standard_Integer i = 1, nb = list->Length(); i <= nb; ++i)
    {
      ...
      if (list->Value(i).IsNull())
        continue;
      if (!cascomp && !cassave)
        XSControl1::Vars(pilot)->SetShape(noms.ToCString(), list->Value(i));
      else if (!cascomp && cassave)
        BRepTools1::Write(list->Value(i), noms.ToCString());
      else if (cascomp)
        B.Add(C, list->Value(i));
    }
    sout << std::endl;
    if (cascomp && !cassave)
      XSControl1::Vars(pilot)->SetShape(nomsh.ToCString(), C);
    else if (cascomp && cassave)
      BRepTools1::Write(C, nomsh.ToCString());
  }
  else
  {
    ...
    TopoShape sh = TR->ShapeResult(mdl->Value(num));
    if (sh.IsNull())
    {
    ...
    if (argc > 2)
      ...
      nomsh = AsciiString1("TREAD_") + num;
    if (!cascomp && !cassave)
      XSControl1::Vars(pilot)->SetShape(nomsh.ToCString(), sh);
    else if (!cascomp && cassave)
      BRepTools1::Write(sh, nomsh.ToCString());
    else
...
static Standard_Boolean XSControl_IsEqualSubShape(const TopoShape& Shape,
                                                  TopoShape&       sh,
...
static IFSelect_ReturnStatus XSControl_fromshape(const Handle(IFSelect_SessionPilot)& pilot)
{
  ...
  TopoShape Shape = XSControl1::Vars(pilot)->GetShape(a1);
  if (Shape.IsNull())
  {
  ...
  else
  {
    ...
    if (ent.IsNull())
    {
      ...
      else
      {
        ...
        {
          ent             = TP->Mapped(i);
          TopoShape sh = TransferBRep1::ShapeResult(TP, ent);
          if (sh.IsNull())
          {
            ent.Nullify();
            continue;
    ...
    if (ent.IsNull())
    {
      ...
      if (Shape.ShapeType() == TopAbs_COMPOUND)
      {
        ...
        {
          const TopoShape&        subsh     = Iter.Value();
          ...
          if (!subent.IsNull())
          {
            sout << "  " << XSControl1::Session(pilot)->Model()->Number(subent);
          }
        }
      }
    }
    else
    {
      ...
      XSControl1::Session(pilot)->Model()->Print(ent, sout);
      if (silent)
        sout << " in file " << XSControl1::Session(pilot)->LoadedFile() << std::endl;
    }
    if (!silent)
      sout << std::endl;
  }
  ...
  {
    yena                           = Standard_True;
    Handle(Transfer_Finder)    fnd = TransferBRep1::ShapeMapper(FP, Shape);
    Handle(RefObject) ent;
    if (!fnd.IsNull())
    ...
    if (!ent.IsNull())
    {
      ...
      XSControl1::Session(pilot)->Model()->Print(ent, sout);
      if (silent)
        sout << " in file " << XSControl1::Session(pilot)->LoadedFile();
      sout << std::endl;
    }
    ...
    else
    {
      ...
      if (!bnd.IsNull())
      {
        ...
        if (!TransientListBinder.IsNull())
        {
          ...
          for (; i <= nb; i++)
          {
            XSControl1::Session(pilot)->Model()->Print(TransientListBinder->Transient(i), sout);
            if (i < nb)
              sout << ", ";
          }
          if (nb > 0)
        ...
        /*	else {
              ...
              if ( ! comp.IsNull() && comp.ShapeType() < Shape.ShapeType() ) {
                ...
                for ( TopoDS_Iterator it(comp); it.More(); it.Next() ) {
                  Handle(Transfer_Finder) cfnd = TransferBRep1::ShapeMapper (FP,it.Value());
                ...
                sout<<"Shape "<<arg1<<" : exported to entities ";
                  ...
                  XSControl1::Session(pilot)->Model()->Print(cent,sout);
                }
...
static IFSelect_ReturnStatus XSControl_trconnexentities(const Handle(IFSelect_SessionPilot)& pilot)
{
  ...
  const Handle(Interface_InterfaceModel)& model = XSControl1::Session(pilot)->Model();
  sout << "(";
  for (i = 1; i <= nb; i++)
  {
    if (i > 1)
...
static IFSelect_ReturnStatus XSControl_trimport(const Handle(IFSelect_SessionPilot)& pilot)
{
  ...
  Standard_Boolean modfic = ShapeFunctions::FileAndVar(WS, arg1, arg2, "IMPORT", fnom, rnom);
  if (modfic)
  ...
  AsciiString1 compart = pilot->CommandPart(3);

  //  Reading file if required

  if (modfic)
  {
    AsciiString1 comload("xload ");
  ...
  for (Standard_Integer il = 1; il <= nbl; il++)
  {
    ...
    else
    {
      ...
      XSControl1::Vars(pilot)->SetShape(nomsh, sh);
    }
  }
  if (nbs == 0)
  ...
  else if (nbs == 1)
  {
    ...
    XSControl1::Vars(pilot)->SetShape(rnom.ToCString(), sh);
  }
  else if (iscomp)
  {
    sout << "One compound made of " << nbs << " Shapes, named " << rnom.ToCString() << std::endl;
    XSControl1::Vars(pilot)->SetShape(rnom.ToCString(), C);
  }
  else
...
static IFSelect_ReturnStatus XSControl_twrite(const Handle(IFSelect_SessionPilot)& pilot)
{
  ...
  for (Standard_Integer i = 1; i < argc; i++)
  {
    ...
    TopoShape Shape = XSControl1::Vars(pilot)->GetShape(ai);
    if (Shape.IsNull())
    {
    ...
    sout << "Pour Shape : " << ai;
    Standard_Integer stat = TW->TransferWriteShape(XSControl1::Session(pilot)->Model(), Shape);
...
void ShapeFunctions::Init()
{
  ...
  IFSelect_Act::AddFunc(
    "tpdraw",
    "[mode:item or root]  num|*  [nomvar] Passes an ITEM to Shape Draw1 (Start or Result)",
    XSControl_tpdraw);
                                                 ...
                                                 const Standard_CString               name)
{
  ...
  if (n1 <= n2 && n1 > 0)
  {
    ...
    for (i = n1; i <= n2; i++)
    {
      ...
      TopoShape Shape = session->Vars()->GetShape(nomshh);
      if (Shape.IsNull())
        continue;
      list->Append(Shape);
      nbsh++;
  ...
  TopoShape Shape = session->Vars()->GetShape(a1);
  if (Shape.IsNull())
  {
...
Standard_Boolean ShapeFunctions::FileAndVar(const Handle(ExchangeSession)& session,
