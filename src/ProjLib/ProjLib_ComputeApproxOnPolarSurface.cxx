                               ...
                               Standard_Real&                   theVPeriod)
{
  ...
  if (theUPeriod != 0.0)
  {
    ...
    if (Abs(aBaseF - aTrimF) + Abs(aBaseL - aTrimL) > Precision1::PConfusion())
    {
      // Param space reduced.
      theUPeriod = 0.0;
    }
                                     ...
                                     const Standard_Real              theV)
{
  ...
  if (aSu.SquareMagnitude() > Precision1::SquareConfusion())
    aSu.Normalize();

  if (aSv.SquareMagnitude() > Precision1::SquareConfusion())
    aSv.Normalize();
...
static gp_Pnt2d Function_Value(const Standard_Real theU, const aFuncStruct& theData)
{
  ...
  if (Abs(p2d.X() - Uinf) < Precision1::PConfusion()
      || Abs(p2d.X() - Usup) < Precision1::PConfusion())
  {
    // V isoline.
    Point3d aPnt;
  ...
  if (Abs(p2d.Y() - Vinf) < Precision1::PConfusion()
      || Abs(p2d.Y() - Vsup) < Precision1::PConfusion())
  {
    // U isoline.
    Point3d aPnt;
  ...
  Standard_Real uperiod = theData.myPeriod[0], vperiod = theData.myPeriod[1], u, v;
  ...
  if (locext.IsDone())
  {
    ...
    if (Dist2Min < theData.mySqProjOrtTol && // Point is projection.
        locext.SquareDistance()
          < aSurfPntDist + Precision1::SquareConfusion()) // Point better than initial.
    {
...
public:
  ...
  Standard_Real FirstParameter() const { return myStruct.myCurve->FirstParameter(); }

  Standard_Real LastParameter() const { return myStruct.myCurve->LastParameter(); }

  gp_Pnt2d Value(const Standard_Real t) const { return Function_Value(t, myStruct); }
...
ProjLib_ComputeApproxOnPolarSurface::ProjLib_ComputeApproxOnPolarSurface()
    : myProjIsDone(Standard_False),
      myTolerance(Precision1::Approximation()),
      myTolReached(-1.0),
      myDegMin(-1),
      myDegMax(-1),
      myMaxSegments(-1),
      ...
      myDist(0.)
{
  ...
  Handle(Geom2d_BSplineCurve) bsc = Perform(theInitialCurve2d, theCurve, theSurface);
                                          ...
                                          Standard_Real               theVJump)
{
  ...
  TColStd_Array1OfReal K1(1, nk1);
  C1->Knots(K1);
  TColStd_Array1OfInteger M1(1, nk1);
  C1->Multiplicities(M1);
  TColgp_Array1OfPnt2d P1(1, np1);
  C1->Poles(P1);
  TColStd_Array1OfReal K2(1, nk2);
  C2->Knots(K2);
  TColStd_Array1OfInteger M2(1, nk2);
  C2->Multiplicities(M2);
  TColgp_Array1OfPnt2d P2(1, np2);
  C2->Poles(P2);
  ...
  Handle(Geom2d_BSplineCurve) BS = new Geom2d_BSplineCurve(P, K, M, deg);
  return BS;
}
  ...
  const Handle(SurfaceAdaptor)& Surf)
{
  ...
  else
  {
    ...
    {
      TolU = Min(TolU, Precision1::PConfusion());
      TolV = Min(TolV, Precision1::PConfusion());
                    ...
                    for (Standard_Integer uInd = 0; uInd < aSizeU; uInd++)
                    {
                      for (Standard_Integer vInd = 0; vInd < aSizeV; vInd++)
                      {
                        Standard_Real aSQdist =
                          aP.SquareDistance(gp_Pnt2d(aUpar[uInd], aVpar[vInd]));
  ...
  const Handle(Adaptor2d_Curve2d)& InitCurve2d)
{
  ...
  Standard_Real DistTol3d2 = DistTol3d * DistTol3d;
  Standard_Real TolU = Surf->UResolution(Tol3d), TolV = Surf->VResolution(Tol3d);
  Standard_Real Tol2d = Max(Sqrt(TolU * TolU + TolV * TolV), Precision1::PConfusion());
  ...
  if (TheTypeS == GeomAbs_Plane)
  {
    ...
    {
      myTolReached                  = Precision1::Confusion();
      Handle(BSplineCurve3d) BSC = Curve->BSpline();
...
//  DrawTrSurf1::Set((Standard_CString ) "initc2d",DDD->ChangeCurve2d().Curve());
  ...
  if (Fit.IsAllApproximated())
  {
    ...
    myProjIsDone                      = Standard_True;
    Handle(Geom2d_BSplineCurve) Dummy = new Geom2d_BSplineCurve(Poles, Knots, Mults, MaxDeg);
