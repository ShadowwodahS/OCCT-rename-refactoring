...
AdvApp2Var_Context::AdvApp2Var_Context(const Standard_Integer               ifav,
      ...
      myNb3DSS(nb3Dss)
{
  ...
  // myURoots, myVRoots
  Handle(TColStd_HArray1OfReal) URoots  = new TColStd_HArray1OfReal(1, myNbURoot);
  ...
  Handle(TColStd_HArray1OfReal) ITol = new TColStd_HArray1OfReal(1, nbss);
  for (i = 1; i <= nb1Dss; i++)
  {
  ...
  Handle(TColStd_HArray2OfReal) FTol = new TColStd_HArray2OfReal(1, nbss, 1, 4);
  Handle(TColStd_HArray2OfReal) CTol = new TColStd_HArray2OfReal(1, nbss, 1, 4);
  for (i = 1; i <= nb1Dss; i++)
  {
    for (j = 1; j <= 4; j++)
    {
