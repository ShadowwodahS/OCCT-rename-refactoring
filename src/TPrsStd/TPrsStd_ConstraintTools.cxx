// Copyright (c) 1999-2014 OPEN CASCADE SAS
//
// This file is part of Open CASCADE Technology software library.
//
// This library is free software; you can redistribute it and/or modify it under
// the terms of the GNU Lesser General Public License version 2.1 as published
// by the Free Software Foundation, with special exception defined in the file
// OCCT_LGPL_EXCEPTION.txt. Consult the file LICENSE_LGPL_21.txt included in OCCT
// distribution for complete text of the license and disclaimer of any warranty.
//
// Alternatively, this file may be used under the terms of Open CASCADE
// commercial license or contractual agreement.

// Language:    C++
// Version:     Euclid Designer 2.0
// Purpose:     Update AIS1 object from a TDataXtd_Constraint.
// Modified     Mon 30 10:15:43 1998 by SZY

#include <TPrsStd_ConstraintTools.hxx>

#include <AIS_InteractiveContext.hxx>
#include <AIS_InteractiveObject.hxx>
#include <PrsDim_AngleDimension.hxx>
#include <PrsDim_ConcentricRelation.hxx>
#include <PrsDim_DiameterDimension.hxx>
#include <PrsDim_EqualDistanceRelation.hxx>
#include <PrsDim_EqualRadiusRelation.hxx>
#include <PrsDim_FixRelation.hxx>
#include <PrsDim_IdenticRelation.hxx>
#include <PrsDim_LengthDimension.hxx>
#include <PrsDim_MaxRadiusDimension.hxx>
#include <PrsDim_MidPointRelation.hxx>
#include <PrsDim_MinRadiusDimension.hxx>
#include <PrsDim_OffsetDimension.hxx>
#include <PrsDim_ParallelRelation.hxx>
#include <PrsDim_PerpendicularRelation.hxx>
#include <PrsDim_RadiusDimension.hxx>
#include <PrsDim_Relation.hxx>
#include <PrsDim_SymmetricRelation.hxx>
#include <PrsDim_TangentRelation.hxx>
#include <BRep_Builder.hxx>
#include <BRep_Tool.hxx>
#include <BRepAdaptor_Curve.hxx>
#include <BRepAdaptor_Surface.hxx>
#include <BRepBuilderAPI_MakeFace.hxx>
#include <GC_MakePlane.hxx>
#include <Geom_CartesianPoint.hxx>
#include <Geom_Circle.hxx>
#include <Geom_Geometry.hxx>
#include <Geom_Line.hxx>
#include <Geom_Plane.hxx>
#include <GeomAbs_SurfaceType.hxx>
#include <gp_Ax1.hxx>
#include <gp_Cone.hxx>
#include <gp_Cylinder.hxx>
#include <gp_Dir.hxx>
#include <gp_Lin.hxx>
#include <gp_Pln.hxx>
#include <gp_Pnt.hxx>
#include <gp_Torus.hxx>
#include <IntAna_QuadQuadGeo.hxx>
#include <Precision.hxx>
#include <Prs3d_Drawer.hxx>
#include <Standard_ErrorHandler.hxx>
#include <Standard_ProgramError.hxx>
#include <TCollection_ExtendedString.hxx>
#include <TDataStd_Name.hxx>
#include <TDataStd_Real.hxx>
#include <TDataXtd_Constraint.hxx>
#include <TDataXtd_Geometry.hxx>
#include <TDF_Label.hxx>
#include <TDF_Reference.hxx>
#include <TNaming_Iterator.hxx>
#include <TNaming_Tool.hxx>
#include <TopExp.hxx>
#include <TopExp_Explorer.hxx>
#include <TopoDS.hxx>
#include <TopoDS_Edge.hxx>
#include <TopoDS_Face.hxx>
#include <TopoDS_Shape.hxx>
#include <TopoDS_Vertex.hxx>
#include <TopTools_IndexedMapOfShape.hxx>
#include <UnitsAPI.hxx>

#include <stdio.h>
static Standard_Boolean CheckShapesPair(const TopoShape&, const TopoShape&); // ota

//=================================================================================================

static void NullifyAIS(Handle(VisualEntity)& anais)
{
  if (anais.IsNull())
    return;
  anais.Nullify();
}

//=================================================================================================

static void FindExternalShape(const Handle(TDataXtd_Constraint)& aConst, Standard_Integer& extShape)
{
  extShape           = 0;
  const DataLabel& L = aConst->Label();
  if (!aConst->GetGeometry(1)->Label().IsDescendant(L))
    extShape = 1;
  else if (!aConst->GetGeometry(2)->Label().IsDescendant(L))
    extShape = 2;
}

//=================================================================================================

static void GetGoodShape(TopoShape& theShape)
{
  switch (theShape.ShapeType())
  {
    case TopAbs_EDGE:
    case TopAbs_VERTEX: {
      return;
    }
    default: {
      ShapeExplorer anExp(theShape, TopAbs_EDGE);
      if (anExp.More())
      {
        theShape = anExp.Current();
        return;
      }
      else
      {
        anExp.Init(theShape, TopAbs_VERTEX);
        if (anExp.More())
        {
          theShape = anExp.Current();
        }
      }
    }
  }
}

// Pour le cas ou S est un compound
static Standard_Boolean IsFace(const TopoShape& S)
{
  Standard_Boolean findface = Standard_False;
  ShapeExplorer  EXP(S, TopAbs_FACE);
  if (EXP.More())
    findface = Standard_True;
  return findface;
}

static TopoFace GetFace(const TopoShape& S)
{
  TopoFace     F;
  ShapeExplorer EXP(S, TopAbs_FACE);
  if (EXP.More())
    F = TopoDS::Face(EXP.Current());
  return F;
}

static TopoEdge GetEdge(const TopoShape& S)
{
  TopoEdge     E;
  ShapeExplorer EXP(S, TopAbs_EDGE);
  if (EXP.More())
    E = TopoDS::Edge(EXP.Current());
  return E;
}

//=================================================================================================

void ConstraintPresentationTools::ComputeTextAndValue(const Handle(TDataXtd_Constraint)& aConst,
                                                  Standard_Real&                     val,
                                                  UtfString&        txt,
                                                  const Standard_Boolean             anIsAngle)
{
  Standard_Real                outvalue;
  const Handle(TDataStd_Real)& VAL = aConst->GetValue();
  val                              = VAL->Get();
  if (anIsAngle)
  {
    outvalue = UnitsAPI::CurrentFromLS(Abs(val), "PLANE ANGLE");
  }
  else
  {
    outvalue = UnitsAPI::CurrentFromLS(val, "LENGTH");
  }
  char res[1000];
  sprintf(res, "%g", outvalue);
  txt = UtfString(res);

  if (VAL->IsCaptured())
  {
    Handle(TDF_Reference) ref;
    VAL->Label().FindAttribute(TDF_Reference::GetID(), ref);
    Handle(NameAttribute) name;
    const DataLabel&      L = ref->Get();
    if (ref->Get().FindAttribute(NameAttribute::GetID(), name))
    {
      UtfString fullname;
      Handle(NameAttribute)      Fathername;
      if (L.Father().FindAttribute(NameAttribute::GetID(), Fathername))
      {
        fullname = Fathername->Get() + UtfString(".") + name->Get();
      }
      else
        fullname = name->Get();
      txt = fullname + UtfString("=") + txt;
    }
  }
}

//=================================================================================================

void ConstraintPresentationTools::UpdateOnlyValue(const Handle(TDataXtd_Constraint)&   aConst,
                                              const Handle(VisualEntity)& anAIS)
{
  if (anAIS.IsNull())
    return;
  if (!aConst->IsDimension())
    return;
  Standard_Real              val;
  UtfString txt;
  ConstraintPresentationTools::ComputeTextAndValue(aConst,
                                               val,
                                               txt,
                                               aConst->GetType() == TDataXtd_ANGLE);
  Handle(PrsDim_Relation) rel = Handle(PrsDim_Relation)::DownCast(anAIS);
  if (!rel.IsNull())
    rel->SetText(txt);
}

//=======================================================================
// function : ComputeDistance
// purpose  : Build an AIS_LengthDimension.
//=======================================================================
void ConstraintPresentationTools::ComputeDistance(const Handle(TDataXtd_Constraint)& theConst,
                                              Handle(VisualEntity)&     theAIS)
{
  Standard_Integer aGeomNum = theConst->NbGeometries();

  // Dimension is build on one or two shapes.
  if (aGeomNum < 1 || aGeomNum > 2)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeDistance: 1 or 2 geometries are needed"
              << std::endl;
#endif
    NullifyAIS(theAIS);
    return;
  }

  TopoShape          aShape1, aShape2;
  Handle(Geom_Geometry) aGeom3;
  Standard_Boolean      isPlanar(theConst->IsPlanar());

  // Get shapes and geometry
  if (aGeomNum == 1)
  {
    GetOneShape(theConst, aShape1);

    if (aShape1.IsNull())
    {
#ifdef OCCT_DEBUG
      std::cout << "ConstraintPresentationTools::ComputeDistance : null shape" << std::endl;
#endif
      NullifyAIS(theAIS);
      return;
    }
  }
  else // aGeomNum == 2
  {
    GetTwoShapes(theConst, aShape1, aShape2);

    if (aShape1.IsNull() || aShape2.IsNull())
    {
#ifdef OCCT_DEBUG
      std::cout << "ConstraintPresentationTools::ComputeDistance : null shape" << std::endl;
#endif
      NullifyAIS(theAIS);
      return;
    }
  }

  // Get plane from constraint
  Handle(GeomPlane) aPlane;
  if (isPlanar)
  {
    GetGeom(theConst, aGeom3);

    GetGoodShape(aShape1);

    if (aGeomNum == 2)
    {
      GetGoodShape(aShape2);
    }

    aPlane = Handle(GeomPlane)::DownCast(aGeom3);
  }

  // Get custom value
  Standard_Real              aValue;
  UtfString aText;
  ComputeTextAndValue(theConst, aValue, aText, Standard_False);

  Standard_Boolean isFaces    = Standard_False;
  Standard_Boolean isEdges    = Standard_False;
  Standard_Boolean isEdgeFace = Standard_False;
  Standard_Boolean isVertices = Standard_False;
  Standard_Boolean isEdge     = Standard_False;

  Standard_Boolean               SaveDrw = Standard_False;
  Handle(StyleDrawer)           aDrawer;
  Handle(PrsDim_LengthDimension) aDim;

  if (!theAIS.IsNull())
  {
    aDim = Handle(PrsDim_LengthDimension)::DownCast(theAIS);
  }

  // Check shapes for AIS1 dimension
  if (aGeomNum == 1)
  {
    if (aShape1.ShapeType() != TopAbs_EDGE)
    {
#ifdef OCCT_DEBUG
      std::cout << "ConstraintPresentationTools::ComputeDistance : shape should be edge" << std::endl;
#endif
      NullifyAIS(theAIS);
      return;
    }

    isEdge = Standard_True;
  }
  else // nbgeom == 2
  {
    isFaces = IsFace(aShape1) && IsFace(aShape2);

    isEdges = (aShape1.ShapeType() == TopAbs_EDGE) && (aShape2.ShapeType() == TopAbs_EDGE);

    isEdgeFace = ((aShape1.ShapeType() == TopAbs_FACE) && (aShape2.ShapeType() == TopAbs_EDGE))
                 || ((aShape1.ShapeType() == TopAbs_EDGE) && (aShape2.ShapeType() == TopAbs_FACE));

    isVertices = (aShape1.ShapeType() == TopAbs_VERTEX) && (aShape2.ShapeType() == TopAbs_VERTEX);

    if (!isPlanar && !isFaces && !isEdges && !isVertices)
    {
      // Search suitable geometry for dimension
      if (aShape1.ShapeType() == aShape2.ShapeType())
      {
        TopoVertex aV1, aV2, aV3, aV4;
        if (aShape1.ShapeType() == TopAbs_WIRE)
        {
          TopExp1::Vertices(TopoDS::Wire(aShape1), aV1, aV2);
          TopExp1::Vertices(TopoDS::Wire(aShape2), aV3, aV4);
        }
        aShape1    = aV1;
        Point3d aP1 = BRepInspector::Pnt(aV1);
        Point3d aP2 = BRepInspector::Pnt(aV3);
        Point3d aP3 = BRepInspector::Pnt(aV4);
        if (aP1.Distance(aP2) < aP1.Distance(aP3))
        {
          aShape2 = aV3;
          Frame3d ax2(aP1, Dir3d(aP2.XYZ() - aP1.XYZ()));
          aPlane = new GeomPlane(aP1, ax2.XDirection());
        }
        else
        {
          aShape2 = aV4;
          Frame3d anAx2(aP1, Dir3d(aP3.XYZ() - aP1.XYZ()));
          aPlane = new GeomPlane(aP1, anAx2.XDirection());
        }
      }
      else if (!isEdgeFace)
      {
        NullifyAIS(theAIS);
        return;
      }
    }
  }

  // Check plane
  Standard_Boolean isCheckPlane = (aDim.IsNull() && !isFaces) || isPlanar;
  if ((isVertices || isEdges) && !isPlanar)
  {
    Point3d aP1, aP2, aP3;

    if (isVertices)
    {
      aP1 = BRepInspector::Pnt(TopoDS::Vertex(aShape1));
      aP2 = BRepInspector::Pnt(TopoDS::Vertex(aShape2));
      aP3 = Point3d(aP1.Y() - 1.0, aP2.X() + 1.0, 0.0);
    }

    if (isEdges)
    {
      TopoVertex aV1, aV2, aV3, aV4;
      TopExp1::Vertices(TopoDS::Edge(aShape1), aV1, aV2);
      TopExp1::Vertices(TopoDS::Edge(aShape2), aV3, aV4);
      aP1 = BRepInspector::Pnt(aV1);
      aP2 = BRepInspector::Pnt(aV2);
      aP3 = BRepInspector::Pnt(aV3);
    }

    GC_MakePlane aPlaneMaker(aP1, aP2, aP3);
    if (aPlaneMaker.IsDone() && !isPlanar)
    {
      aPlane = aPlaneMaker.Value();
    }
  }

  if (isCheckPlane && aPlane.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeDistance : null plane" << std::endl;
#endif
    NullifyAIS(theAIS);
    return;
  }

  //  Update of AIS1
  if (aDim.IsNull())
  {
    if (isEdge)
    {
      aDim = new PrsDim_LengthDimension(GetEdge(aShape1), aPlane->Pln());
    }
    else if (isFaces)
    {
      aDim = new PrsDim_LengthDimension(GetFace(aShape1), GetFace(aShape2));
    }
    else
    {
      aDim = new PrsDim_LengthDimension(aShape1, aShape2, aPlane->Pln());
    }

    if (SaveDrw)
    {
      aDim->SetAttributes(aDrawer);
    }
  }
  else
  {
    if (isEdge)
    {
      aDim->SetMeasuredGeometry(GetEdge(aShape1), aPlane->Pln());
    }
    else
    {
      aDim->SetMeasuredShapes(aShape1, aShape2);
    }

    aDim->SetCustomValue(aValue);
  }

  if (!aPlane.IsNull())
  {
    aDim->SetCustomPlane(aPlane->Pln());
  }

  theAIS = aDim;
}

//=================================================================================================

void ConstraintPresentationTools::ComputePerpendicular(const Handle(TDataXtd_Constraint)& aConst,
                                                   Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 2)
  {
#ifdef OCCT_DEBUG
    std::cout
      << "ConstraintPresentationTools::ComputePerpendicular: at leat two constraintes are needed"
      << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  TopoShape          shape1, shape2;
  Handle(Geom_Geometry) ageom3;
  Standard_Boolean      is_planar(aConst->IsPlanar());

  if (is_planar)
    GetShapesAndGeom(aConst, shape1, shape2, ageom3);
  else
    GetTwoShapes(aConst, shape1, shape2);
  if (shape1.IsNull() || shape2.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputePerpendicular : null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  GetGoodShape(shape1);
  GetGoodShape(shape2);
  //  Update de l'AIS1
  Handle(PrsDim_PerpendicularRelation) ais;
  if (anAIS.IsNull())
    ais = new PrsDim_PerpendicularRelation(shape1, shape2);
  else
  {
    ais = Handle(PrsDim_PerpendicularRelation)::DownCast(anAIS);
    if (ais.IsNull())
    {
      ais = new PrsDim_PerpendicularRelation(shape1, shape2);
    }
    else
    {
      ais->SetFirstShape(shape1);
      ais->SetSecondShape(shape2);
    }
  }

  if (is_planar)
  {
    Handle(GeomPlane) aplane = Handle(GeomPlane)::DownCast(ageom3);
    if (aplane.IsNull())
    {
#ifdef OCCT_DEBUG
      std::cout << "ConstraintPresentationTools::ComputePerpendicular: nul plane" << std::endl;
#endif
      NullifyAIS(anAIS);
      return;
    }
    ais->SetPlane(aplane);
  }
  anAIS = ais;
}

//=================================================================================================

void ConstraintPresentationTools::ComputeParallel(const Handle(TDataXtd_Constraint)& aConst,
                                              Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 2)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeParallel: at least 2 constraintes are needed"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  if (!aConst->IsPlanar())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeParallel: must be a planar constraint"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  TopoShape          shape1, shape2;
  Handle(Geom_Geometry) ageom3;

  GetShapesAndGeom(aConst, shape1, shape2, ageom3);
  if (shape1.IsNull() || shape2.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeParallel : null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  Handle(GeomPlane) aplane = Handle(GeomPlane)::DownCast(ageom3);
  if (aplane.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeParallel: nul plane" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  //  Update de l'AIS1
  GetGoodShape(shape1);
  GetGoodShape(shape2);
  Handle(PrsDim_ParallelRelation) ais;
  if (anAIS.IsNull())
    ais = new PrsDim_ParallelRelation(shape1, shape2, aplane);
  else
  {
    ais = Handle(PrsDim_ParallelRelation)::DownCast(anAIS);
    if (ais.IsNull())
    {
      ais = new PrsDim_ParallelRelation(shape1, shape2, aplane);
    }
    else
    {
      ais->SetFirstShape(shape1);
      ais->SetSecondShape(shape2);
      ais->SetPlane(aplane);
    }
  }
  anAIS = ais;
}

//=================================================================================================

void ConstraintPresentationTools::ComputeSymmetry(const Handle(TDataXtd_Constraint)& aConst,
                                              Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 3)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeSymmetry: at least 3 constraintes are needed"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  Standard_Boolean is_planar(aConst->IsPlanar());
  if (!is_planar)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeSymmetry: must be a planar constraint"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  TopoShape          shape1, shape2, shape3;
  Handle(Geom_Geometry) ageom3;
  GetShapesAndGeom(aConst, shape1, shape2, shape3, ageom3);

  if (shape1.IsNull() || shape2.IsNull() || shape3.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeSymmetry : null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  GetGoodShape(shape1);
  GetGoodShape(shape2);
  GetGoodShape(shape3);
  Handle(GeomPlane) aplane = Handle(GeomPlane)::DownCast(ageom3);
  if (aplane.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeSymmetry: null plane" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  //  Update de l'AIS1
  Handle(PrsDim_SymmetricRelation) ais;
  if (anAIS.IsNull())
    ais = new PrsDim_SymmetricRelation(shape3, shape1, shape2, aplane);
  else
  {
    ais = Handle(PrsDim_SymmetricRelation)::DownCast(anAIS);
    if (ais.IsNull())
    {
      ais = new PrsDim_SymmetricRelation(shape3, shape1, shape2, aplane);
    }
    else
    {
      ais->SetFirstShape(shape1);
      ais->SetSecondShape(shape2);
      ais->SetPlane(aplane);
      ais->SetTool(shape3);
    }
  }
  anAIS = ais;
}

//=================================================================================================

void ConstraintPresentationTools::ComputeMidPoint(const Handle(TDataXtd_Constraint)& aConst,
                                              Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 3)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeSymmetry: at least 3 constraints are needed"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  Standard_Boolean is_planar(aConst->IsPlanar());
  if (!is_planar)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeSymmetry: must be a planar constraint"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  TopoShape          shape1, shape2, shape3;
  Handle(Geom_Geometry) ageom3;
  GetShapesAndGeom(aConst, shape1, shape2, shape3, ageom3);

  if (shape1.IsNull() || shape2.IsNull() || shape3.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeSymmetry : null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  GetGoodShape(shape1);
  GetGoodShape(shape2);
  GetGoodShape(shape3);

  Handle(GeomPlane) aplane = Handle(GeomPlane)::DownCast(ageom3);
  if (aplane.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeSymmetry: null plane" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  //  Update de l'AIS1
  Handle(PrsDim_MidPointRelation) ais;
  if (anAIS.IsNull())
    ais = new PrsDim_MidPointRelation(shape3, shape1, shape2, aplane);
  else
  {
    ais = Handle(PrsDim_MidPointRelation)::DownCast(anAIS);
    if (ais.IsNull())
    {
      ais = new PrsDim_MidPointRelation(shape3, shape1, shape2, aplane);
    }
    else
    {
      ais->SetFirstShape(shape1);
      ais->SetSecondShape(shape2);
      ais->SetPlane(aplane);
      ais->SetTool(shape3);
    }
  }
  anAIS = ais;
}

//=================================================================================================

void ConstraintPresentationTools::ComputeTangent(const Handle(TDataXtd_Constraint)& aConst,
                                             Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 2)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeTangent: at leat two constraintes are needed"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  if (!aConst->IsPlanar())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeTangent: must be a planar constraint"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  TopoShape          shape1, shape2;
  Handle(Geom_Geometry) ageom3;

  GetShapesAndGeom(aConst, shape1, shape2, ageom3);
  if (shape1.IsNull() || shape2.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeTangent : null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  GetGoodShape(shape1);
  GetGoodShape(shape2);
  Handle(GeomPlane) aplane = Handle(GeomPlane)::DownCast(ageom3);
  if (aplane.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeTangent: nul plane" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  //  Update de l'AIS1
  Handle(PrsDim_TangentRelation) ais;
  if (anAIS.IsNull())
  {
    ais = new PrsDim_TangentRelation(shape1, shape2, aplane);
    ais->SetArrowSize(10000000); // jfa 9/10/2000
  }
  else
  {
    ais = Handle(PrsDim_TangentRelation)::DownCast(anAIS);
    if (ais.IsNull())
    {
      ais = new PrsDim_TangentRelation(shape1, shape2, aplane);
      ais->SetArrowSize(10000000); // jfa 9/10/2000
    }
    else
    {
      ais->SetFirstShape(shape1);
      ais->SetSecondShape(shape2);
      ais->SetPlane(aplane);
      ais->SetArrowSize(10000000); // jfa 9/10/2000
    }
  }
  anAIS = ais;
}

//=======================================================================
// function : ComputeAngleForOneFace
// purpose  : computes AngleDimension for one-conical-face case
//=======================================================================
void ConstraintPresentationTools::ComputeAngleForOneFace(const Handle(TDataXtd_Constraint)& aConst,
                                                     Handle(VisualEntity)&     anAIS)
{

  TopoShape          shape;
  Handle(Geom_Geometry) ageom3;

  GetOneShape(aConst, shape);
  if (shape.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeAngleForOneFace : null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  Standard_Real              val1;
  UtfString txt;
  ConstraintPresentationTools::ComputeTextAndValue(aConst, val1, txt, Standard_True);

  Handle(PrsDim_AngleDimension) ais;
  TopoFace                   face;
  if (!anAIS.IsNull())
  {
    ais = Handle(PrsDim_AngleDimension)::DownCast(anAIS);
    if (ais.IsNull())
    {
      face = TopoDS::Face(shape);
      ais  = new PrsDim_AngleDimension(face);
    }
    else
    {
      ais->SetMeasuredGeometry(TopoDS::Face(shape));
    }
  }
  else
  {
    face = TopoDS::Face(shape);
    ais  = new PrsDim_AngleDimension(face);
  }

  anAIS = ais;
}

//=================================================================================================

static Standard_Boolean CheckIsShapeCompound(TopoShape& shape, TopoFace& aFace)
{
  if (shape.ShapeType() == TopAbs_COMPOUND)
  {
    TopTools_IndexedMapOfShape aFaceMap;
    TopExp1::MapShapes(shape, TopAbs_FACE, aFaceMap);
    for (Standard_Integer i = 1; i <= aFaceMap.Extent(); i++)
    {
      aFace = TopoDS::Face(aFaceMap.FindKey(i));
      if (!aFace.IsNull())
      {
        shape = aFace;
        return (Standard_True);
      }
    }
  }
#ifdef OCCT_DEBUG
  std::cout << "TPrsStd::Compute angle : Shape is not Compound or is Null" << std::endl;
#endif
  return (Standard_False);
}

//=================================================================================================

void ConstraintPresentationTools::ComputeAngle(const Handle(TDataXtd_Constraint)& aConst,
                                           Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 2)
  {
    if (nbgeom == 1)
    {
      ComputeAngleForOneFace(aConst, anAIS);
      return;
    }
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeAngle: at least 2 constraints are needed"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  TopoShape          shape1, shape2;
  Handle(Geom_Geometry) ageom3;

  GetShapesAndGeom(aConst, shape1, shape2, ageom3);
  if (shape1.IsNull() || shape2.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeAngle : null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  Standard_Boolean isCurvilinear = Standard_False;
  if (ageom3.IsNull())
  {
    // on essaie de le calculer

    TopoFace aFace;
    if (shape1.ShapeType() == TopAbs_WIRE)
    {
      FaceMaker MkF(TopoDS::Wire(shape1), Standard_True);
      if (MkF.IsDone())
      {
        aFace  = MkF.Face();
        shape1 = aFace;
      }
    }
    else if (shape1.ShapeType() == TopAbs_FACE)
      aFace = TopoDS::Face(shape1);
    else if (!CheckIsShapeCompound(shape1, aFace))
    {
#ifdef OCCT_DEBUG
      std::cout << "Compute angle : Geom type = " << shape1.ShapeType() << " non traite"
                << std::endl;
#endif
      NullifyAIS(anAIS);
      return;
    }

    Axis3d anax1aFace1;
    gp_Pln aPlnaFace1;

    BRepAdaptor_Surface aSurfaFace(aFace);
    GeomAbs_SurfaceType aTypeaFace = aSurfaFace.GetType();
    if (aTypeaFace == GeomAbs_Plane)
    {
      aPlnaFace1  = aSurfaFace.Plane1();
      anax1aFace1 = aPlnaFace1.Axis(); // Normale au plan
    }
    else if (aTypeaFace == GeomAbs_Cylinder)
    {
      Cylinder1 aCylaFace = aSurfaFace.Cylinder();
      anax1aFace1           = aCylaFace.Axis();
    }
    else if (aTypeaFace == GeomAbs_Cone)
    {
      Cone1 aCone = aSurfaFace.Cone();
      anax1aFace1   = aCone.Axis();
    }
    else if (aTypeaFace == GeomAbs_Torus)
    {
      gp_Torus aTore = aSurfaFace.Torus();
      anax1aFace1    = aTore.Axis();
    }
    else
    {
#ifdef OCCT_DEBUG
      std::cout << "Compute angle" << aTypeaFace << " non traite" << std::endl;
#endif
      NullifyAIS(anAIS);
      return;
    }

    Axis3d anax1aFace2;
    gp_Pln aPlnaFace2;
    if (shape2.ShapeType() == TopAbs_WIRE)
    {
      FaceMaker MkF(TopoDS::Wire(shape2), Standard_True);
      if (MkF.IsDone())
      {
        aFace  = MkF.Face();
        shape2 = aFace;
      }
    }
    else if (shape2.ShapeType() == TopAbs_FACE)
      aFace = TopoDS::Face(shape2);
    else if (!CheckIsShapeCompound(shape2, aFace))
    {
#ifdef OCCT_DEBUG
      std::cout << "Compute angle : Geom type = " << shape2.ShapeType() << " non traite"
                << std::endl;
#endif
      NullifyAIS(anAIS);
      return;
    }

    aSurfaFace.Initialize(aFace);
    aTypeaFace = aSurfaFace.GetType();
    if (aTypeaFace == GeomAbs_Plane)
    {
      aPlnaFace2  = aSurfaFace.Plane1();
      anax1aFace2 = aPlnaFace2.Axis(); // Normale au plan
    }
    else if (aTypeaFace == GeomAbs_Cylinder)
    {
      Cylinder1 aCylaFace = aSurfaFace.Cylinder();
      anax1aFace2           = aCylaFace.Axis();
    }
    else if (aTypeaFace == GeomAbs_Cone)
    {
      Cone1 aCone = aSurfaFace.Cone();
      anax1aFace2   = aCone.Axis();
    }
    else if (aTypeaFace == GeomAbs_Torus)
    {
      gp_Torus aTore = aSurfaFace.Torus();
      anax1aFace2    = aTore.Axis();
    }
    else
    {
#ifdef OCCT_DEBUG
      std::cout << "Compute angle " << aTypeaFace << " non traite" << std::endl;
#endif
      NullifyAIS(anAIS);
      return;
    }

    if (aTypeaFace == GeomAbs_Plane)
    {
      if (!anax1aFace1.IsParallel(anax1aFace2, Precision::Angular()))
      {

        QuadQuadGeoIntersection IntersectPlane(aPlnaFace1,
                                          aPlnaFace2,
                                          Precision::Angular(),
                                          Precision::Angular());
        if (IntersectPlane.IsDone() && (IntersectPlane.TypeInter() != IntAna_Empty))
        {
          gp_Lin            aLine         = IntersectPlane.Line(1);
          Handle(GeomLine) computedgeom3 = new GeomLine(aLine);
          ageom3                          = computedgeom3;
        }
        else
        {
#ifdef OCCT_DEBUG
          std::cout << "Compute angle insertection of planes failed" << std::endl;
#endif
          NullifyAIS(anAIS);
          return;
        }
      }
      else
      {

#ifdef OCCT_DEBUG
        std::cout << "Compute angle faces are //" << std::endl;
#endif
        NullifyAIS(anAIS);
        return;
      }
    }
    else
    {
      // Curvilinear faces...
      isCurvilinear = Standard_True;
    }
  } // endif (ageom3.IsNull())

  Standard_Boolean isplan(Standard_False);

  if (!isCurvilinear)
  {
    if (ageom3->IsKind(STANDARD_TYPE(GeomPlane)))
      isplan = Standard_True;
    else if (ageom3->IsKind(STANDARD_TYPE(GeomLine)))
      isplan = Standard_False;
    else
    {
#ifdef OCCT_DEBUG
      std::cout << "ConstraintPresentationTools::ComputeAngle: unknown 3rd arg " << std::endl;
#endif
      NullifyAIS(anAIS);
      return;
    }
  }
  Standard_Real              val1;
  UtfString txt;
  ComputeTextAndValue(aConst, val1, txt, Standard_True);

  Standard_Boolean toCreate(Standard_True);
  Standard_Boolean isface(shape1.ShapeType() == TopAbs_FACE);

  Handle(PrsDim_AngleDimension) ais;
  if (!anAIS.IsNull())
  {
    ais = Handle(PrsDim_AngleDimension)::DownCast(anAIS);
    if (ais.IsNull())
    {
      toCreate = Standard_True;
    }
    else
      toCreate = Standard_False;
  }

  Standard_Integer ExtShape(0);
  if (toCreate)
  {
    // Creation de l'AIS1
    if (isplan)
    {
      if (!isface)
      {
        FindExternalShape(aConst, ExtShape);
        GetGoodShape(shape1);
        GetGoodShape(shape2);
        ais = new PrsDim_AngleDimension(TopoDS::Edge(shape1), TopoDS::Edge(shape2));
      }
    }
    else
    {
      if (isCurvilinear)
      {
        ais = new PrsDim_AngleDimension(TopoDS::Face(shape1), TopoDS::Face(shape2));
      }
      else if (isface)
      {
        ais = new PrsDim_AngleDimension(TopoDS::Face(shape1), TopoDS::Face(shape2));
      }
    }
  }
  else
  {
    // Update de l'AIS1
    if (isplan)
    {
      GetGoodShape(shape1);
      GetGoodShape(shape2);
    }
    ais->SetMeasuredGeometry(TopoDS::Face(shape1), TopoDS::Face(shape2));
    if (isplan)
      ais->SetCustomPlane(Handle(GeomPlane)::DownCast(ageom3)->Pln());
    else if (!isCurvilinear)
    {
      gp_Pln aPlane;
      aPlane.SetAxis(Handle(GeomLine)::DownCast(ageom3)->Position1());
      ais->SetCustomPlane(aPlane);
    }
  }
  anAIS = ais;
}

//=================================================================================================

void ConstraintPresentationTools::ComputeConcentric(const Handle(TDataXtd_Constraint)& aConst,
                                                Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 2)
  {
    throw Standard_ProgramError(
      "ConstraintPresentationTools::ComputeConcentric: at least 2 constraintes are needed");
  }
  if (!aConst->IsPlanar())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeConcentric: must be a planar constraint"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  TopoShape          shape1, shape2;
  Handle(Geom_Geometry) ageom3;

  GetShapesAndGeom(aConst, shape1, shape2, ageom3);
  if (shape1.IsNull() || shape2.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeConcentric : null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  GetGoodShape(shape1);
  GetGoodShape(shape2);

  // ota : to allow concentric constraint display between vertex and edge
  if (shape1.ShapeType() != TopAbs_EDGE && shape2.ShapeType() != TopAbs_EDGE)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeConcentric: concentric between two vertexes : "
                 "NOT DISPLAYED"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  Handle(GeomPlane) aplane = Handle(GeomPlane)::DownCast(ageom3);
  if (aplane.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeConcentric: nul plane" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  //  Update de l'AIS1
  Handle(PrsDim_ConcentricRelation) ais;
  if (!anAIS.IsNull())
  {
    ais = Handle(PrsDim_ConcentricRelation)::DownCast(anAIS);
    if (ais.IsNull())
    {
      ais = new PrsDim_ConcentricRelation(shape1, shape2, aplane);
    }
    else
    {
      ais->SetFirstShape(shape1);
      ais->SetSecondShape(shape2);
      ais->SetPlane(aplane);
    }
  }
  else
  {
    ais = new PrsDim_ConcentricRelation(shape1, shape2, aplane);
  }

  anAIS = ais;
}

//=================================================================================================

void ConstraintPresentationTools::ComputeRadius(const Handle(TDataXtd_Constraint)& aConst,
                                            Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 1)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeRadius: at least one constrainte is needed"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  TopoShape shape1;
  GetOneShape(aConst, shape1);
  if (shape1.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeRadius: null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  // POP on teste si ce n'est pas un compound
  if (shape1.ShapeType() == TopAbs_COMPOUND || shape1.ShapeType() == TopAbs_COMPSOLID
      || shape1.ShapeType() == TopAbs_SOLID || shape1.ShapeType() == TopAbs_SHELL)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeRadius: not good shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  if (IsFace(shape1))
    shape1 = GetFace(shape1);

  Standard_Real              val1;
  UtfString txt;
  ComputeTextAndValue(aConst, val1, txt, Standard_False);

  //  Update de l'AIS1
  Standard_Boolean isplanar(aConst->IsPlanar());
  if (isplanar)
    GetGoodShape(shape1);

  Handle(PrsDim_RadiusDimension) ais;
  if (!anAIS.IsNull())
  {
    ais = Handle(PrsDim_RadiusDimension)::DownCast(anAIS);
    if (ais.IsNull())
    {
      ais = new PrsDim_RadiusDimension(shape1);
    }
    else
    {
      ais->SetMeasuredGeometry(shape1);
    }
  }
  else
    ais = new PrsDim_RadiusDimension(shape1);

  if (isplanar)
  {
    Handle(Geom_Geometry) ageom2;
    GetGeom(aConst, ageom2);
    Handle(GeomPlane) aplane = Handle(GeomPlane)::DownCast(ageom2);
    if (aplane.IsNull())
    {
#ifdef OCCT_DEBUG
      std::cout << "ConstraintPresentationTools::ComputeRadius: nul plane" << std::endl;
#endif
      NullifyAIS(anAIS);
      return;
    }
    ais->SetCustomPlane(aplane->Pln());
  }
  anAIS = ais;
}

// ota -- begin --
//=================================================================================================

void ConstraintPresentationTools::ComputeMinRadius(const Handle(TDataXtd_Constraint)& aConst,
                                               Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 1)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeMinRadius: at least one constrainte is needed"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  TopoShape shape1;
  GetOneShape(aConst, shape1);
  if (shape1.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeMinradius: null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  // POP on teste si ce n'est pas un compound
  if (shape1.ShapeType() == TopAbs_COMPOUND || shape1.ShapeType() == TopAbs_COMPSOLID
      || shape1.ShapeType() == TopAbs_SOLID || shape1.ShapeType() == TopAbs_SHELL)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeMinRadius: not good shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  if (IsFace(shape1))
    shape1 = GetFace(shape1);

  Standard_Real              val1;
  UtfString txt;
  ComputeTextAndValue(aConst, val1, txt, Standard_False);

  //  Update de l'AIS1
  Standard_Boolean isplanar(aConst->IsPlanar());
  if (isplanar)
    GetGoodShape(shape1);

  Handle(PrsDim_MinRadiusDimension) ais;
  if (!anAIS.IsNull())
  {
    ais = Handle(PrsDim_MinRadiusDimension)::DownCast(anAIS);
    if (ais.IsNull())
    {
      ais = new PrsDim_MinRadiusDimension(shape1, val1, txt);
    }
    else
    {
      ais->SetValue(val1);
      ais->SetFirstShape(shape1);
      ais->SetText(txt);
    }
  }
  else
    ais = new PrsDim_MinRadiusDimension(shape1, val1, txt);

  if (isplanar)
  {
    Handle(Geom_Geometry) ageom2;
    GetGeom(aConst, ageom2);
    Handle(GeomPlane) aplane = Handle(GeomPlane)::DownCast(ageom2);
    if (aplane.IsNull())
    {
#ifdef OCCT_DEBUG
      std::cout << "ConstraintPresentationTools::ComputeMinRadius: nul plane" << std::endl;
#endif
      NullifyAIS(anAIS);
      return;
    }
    ais->SetPlane(aplane);
  }
  anAIS = ais;
}

//=================================================================================================

void ConstraintPresentationTools::ComputeMaxRadius(const Handle(TDataXtd_Constraint)& aConst,
                                               Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 1)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeMaxRadius: at least one constrainte is needed"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  TopoShape shape1;
  GetOneShape(aConst, shape1);
  if (shape1.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeMaxradius: null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  // POP on teste si ce n'est pas un compound
  if (shape1.ShapeType() == TopAbs_COMPOUND || shape1.ShapeType() == TopAbs_COMPSOLID
      || shape1.ShapeType() == TopAbs_SOLID || shape1.ShapeType() == TopAbs_SHELL)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeMaxRadius: not good shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  if (IsFace(shape1))
    shape1 = GetFace(shape1);

  Standard_Real              val1;
  UtfString txt;
  ComputeTextAndValue(aConst, val1, txt, Standard_False);

  //  Update de l'AIS1
  Standard_Boolean isplanar(aConst->IsPlanar());
  if (isplanar)
    GetGoodShape(shape1);

  Handle(PrsDim_MaxRadiusDimension) ais;
  if (!anAIS.IsNull())
  {
    ais = Handle(PrsDim_MaxRadiusDimension)::DownCast(anAIS);
    if (ais.IsNull())
    {
      ais = new PrsDim_MaxRadiusDimension(shape1, val1, txt);
    }
    else
    {
      ais->SetValue(val1);
      ais->SetFirstShape(shape1);
      ais->SetText(txt);
    }
  }
  else
    ais = new PrsDim_MaxRadiusDimension(shape1, val1, txt);

  if (isplanar)
  {
    Handle(Geom_Geometry) ageom2;
    GetGeom(aConst, ageom2);
    Handle(GeomPlane) aplane = Handle(GeomPlane)::DownCast(ageom2);
    if (aplane.IsNull())
    {
#ifdef OCCT_DEBUG
      std::cout << "ConstraintPresentationTools::ComputeMaxRadius: nul plane" << std::endl;
#endif
      NullifyAIS(anAIS);
      return;
    }
    ais->SetPlane(aplane);
  }
  anAIS = ais;
}

//=================================================================================================

void ConstraintPresentationTools::ComputeEqualDistance(const Handle(TDataXtd_Constraint)& aConst,
                                                   Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 4)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeEqual: at least four geometries are needed"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  TopoShape          aShape1, aShape2, aShape3, aShape4;
  Handle(Geom_Geometry) aGeom;
  GetShapesAndGeom(aConst, aShape1, aShape2, aShape3, aShape4, aGeom);
  if (aShape1.IsNull() || aShape2.IsNull() || aShape3.IsNull() || aShape4.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeEqualDistance : null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  GetGoodShape(aShape1);
  GetGoodShape(aShape2);
  GetGoodShape(aShape3);
  GetGoodShape(aShape4);

  if (!CheckShapesPair(aShape1, aShape2) || !CheckShapesPair(aShape3, aShape4))
  {
#ifdef OCCT_DEBUG
    std::cout
      << "ConstraintPresentationTools::ComputeEqualDistance : at least one pair of shapes is incorrect"
      << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  // Get the plane
  Standard_Boolean   IsPlanar(aConst->IsPlanar());
  Handle(GeomPlane) aPlane;
  if (IsPlanar)
    aPlane = Handle(GeomPlane)::DownCast(aGeom);

  if (!IsPlanar || aPlane.IsNull())
  {
    // create the plane
#ifdef OCCT_DEBUG
    std::cout << "The constraint plane is not assigned " << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  // Update AIS1
  Handle(PrsDim_EqualDistanceRelation) ais;
  if (!anAIS.IsNull())
  {
    {
      ais = Handle(PrsDim_EqualDistanceRelation)::DownCast(anAIS);

      if (ais.IsNull())
        ais = new PrsDim_EqualDistanceRelation(aShape1, aShape2, aShape3, aShape4, aPlane);

      else
      {
        ais->SetFirstShape(aShape1);
        ais->SetSecondShape(aShape2);
        ais->SetShape3(aShape3);
        ais->SetShape4(aShape4);
        ais->SetPlane(aPlane);
      }
    }
  }
  else
    ais = new PrsDim_EqualDistanceRelation(aShape1, aShape2, aShape3, aShape4, aPlane);

  anAIS = ais;

  return;
}

//======================================================================
// function : CheckShapesPair
// purpose  : checks the types of two shapes.
//            If the types aShape1 and aShape2 are EDGE - EDGE,
//                                              or EDGE - VERTEX,
//                                              or VERTEX - VERTEX,
//                                              or CIRCLE - CIRCLE,
//                                              or CIRCLE - VERTEX,
//            then function returns TRUE, otherwise FALSE.
//======================================================================
static Standard_Boolean CheckShapesPair(const TopoShape& aShape1, const TopoShape& aShape2)
{
  // Check whether the shapes form a correct pair.
  if (aShape1.ShapeType() == TopAbs_EDGE && aShape2.ShapeType() == TopAbs_EDGE)
  {
    BRepAdaptor_Curve aCurve1(TopoDS::Edge(aShape1));
    BRepAdaptor_Curve aCurve2(TopoDS::Edge(aShape2));
    if (aCurve1.GetType() == GeomAbs_Line && aCurve2.GetType() == GeomAbs_Line)
    { // Are lines parallel ?
      Dir3d aDir1 = aCurve1.Line().Direction();
      Dir3d aDir2 = aCurve2.Line().Direction();
      if (!(aDir1.IsParallel(aDir2, Precision::Confusion())))
      {
#ifdef OCCT_DEBUG
        std::cout << " Lines are not parallel" << std::endl;
#endif
        return Standard_False;
      }
    }
    else if (aCurve1.GetType() == GeomAbs_Circle && aCurve2.GetType() == GeomAbs_Circle)
    {
      Point3d aCntr1 = aCurve1.Circle().Location(); // get the circle center
      Point3d aCntr2 = aCurve2.Circle().Location(); // get the circle center
      if (!aCntr1.IsEqual(aCntr2, Precision::Confusion()))
      {
#ifdef OCCT_DEBUG
        std::cout << " Circles are not concentric" << std::endl;
#endif
        return Standard_False;
      }
    }
    else
    {
#ifdef OCCT_DEBUG
      std::cout << "Incorrect pair of curves " << std::endl;
#endif
      return Standard_False;
    }
  }
  else if (aShape1.ShapeType() != TopAbs_VERTEX || aShape2.ShapeType() != TopAbs_VERTEX)
  {
    Point3d            aPnt;
    BRepAdaptor_Curve aCurve;
    if (aShape1.ShapeType() == TopAbs_VERTEX)
    {
      aPnt = BRepInspector::Pnt(TopoDS::Vertex(aShape1));
      aCurve.Initialize(TopoDS::Edge(aShape2));
    }
    else
    {
      aPnt = BRepInspector::Pnt(TopoDS::Vertex(aShape2));
      aCurve.Initialize(TopoDS::Edge(aShape1));
    }
    if (aCurve.GetType() == GeomAbs_Circle)
    {
      Point3d aCntr = aCurve.Circle().Location();
      if (!aCntr.IsEqual(aPnt, Precision::Confusion()))
      {
#ifdef OCCT_DEBUG
        std::cout << " The point doesn't coincide with the circle center" << std::endl;
#endif
        return Standard_False;
      }
    }
  }
  return Standard_True;
}

//=================================================================================================

void ConstraintPresentationTools::ComputeEqualRadius(const Handle(TDataXtd_Constraint)& aConst,
                                                 Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 2)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeEqualRadius: at least two geometries are needed"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  TopoShape          shape1, shape2;
  Handle(Geom_Geometry) ageom3;

  GetShapesAndGeom(aConst, shape1, shape2, ageom3);
  if (shape1.IsNull() || shape2.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeEqualRadius : null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  //  Update AIS1
  Standard_Boolean IsPlanar(aConst->IsPlanar());

  GetGoodShape(shape1);
  GetGoodShape(shape2);
  const TopoEdge  edge1 = TopoDS::Edge(shape1);
  const TopoEdge  edge2 = TopoDS::Edge(shape2);
  Handle(GeomPlane) aplane;

  if (IsPlanar)
    aplane = Handle(GeomPlane)::DownCast(ageom3);

  if (!IsPlanar || aplane.IsNull())
  {
    // check are the planes of edge1 and edge2 coincident
    BRepAdaptor_Curve  aCurve(edge1);
    Handle(GeomCurve3d) aProjCurve = aCurve.Curve().Curve();
    gp_Circ            aCircle    = (Handle(GeomCircle)::DownCast(aProjCurve))->Circ();
    Ax3             anAx31(aCircle.Position1()); // get the circle axis
    // get the circle plane
    Handle(GeomPlane) aPlane1 = new GeomPlane(anAx31);

    aCurve.Initialize(edge2);
    aProjCurve = aCurve.Curve().Curve();
    aCircle    = (Handle(GeomCircle)::DownCast(aProjCurve))->Circ();
    Ax3 anAx32(aCircle.Position1()); // get the circle axis
    // get the circle plane
    Handle(GeomPlane) aPlane2 = new GeomPlane(anAx32);

    Standard_Real A, B, C, D1, D2;
    aPlane1->Coefficients(A, B, C, D1); // Get normalized coefficients
    aPlane2->Coefficients(A, B, C, D2); // Get normalized coefficients
    const Dir3d& aDir1 = anAx31.Direction();
    const Dir3d& aDir2 = anAx32.Direction();

    if (Abs(D1 - D2) < Precision::Confusion() && aDir1.IsParallel(aDir2, Precision::Confusion()))
      aplane = aPlane2;
    else
    {
#ifdef OCCT_DEBUG
      std::cout << "ConstraintPresentationTools::ComputeRadiusRelation: nul plane" << std::endl;
#endif
      NullifyAIS(anAIS);
      return;
    }
  }
  Handle(PrsDim_EqualRadiusRelation) ais;
  if (!anAIS.IsNull())
  {
    ais = Handle(PrsDim_EqualRadiusRelation)::DownCast(anAIS);

    if (ais.IsNull())
    {
      ais = new PrsDim_EqualRadiusRelation(edge1, edge2, aplane);
    }
    else
    {
      ais->SetFirstShape(shape1);
      ais->SetSecondShape(shape2);
      ais->SetPlane(aplane);
    }
  }
  else
  {
    ais = new PrsDim_EqualRadiusRelation(edge1, edge2, aplane);
  }

  anAIS = ais;
}

// ota -- end --

//=================================================================================================

void ConstraintPresentationTools::ComputeDiameter(const Handle(TDataXtd_Constraint)& aConst,
                                              Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 1)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeDiameter: at least one constrainte is needed"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  TopoShape shape1;

  GetOneShape(aConst, shape1);
  if (shape1.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeDiameter : null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  Standard_Real              val1;
  UtfString txt;
  ComputeTextAndValue(aConst, val1, txt, Standard_False);

  //  Update de l'AIS1
  Standard_Boolean IsPlanar(aConst->IsPlanar());
  if (IsPlanar)
    GetGoodShape(shape1);
  Handle(PrsDim_DiameterDimension) ais;
  if (!anAIS.IsNull())
  {
    ais = Handle(PrsDim_DiameterDimension)::DownCast(anAIS);
    if (ais.IsNull())
    {
      ais = new PrsDim_DiameterDimension(shape1);
    }
    else
    {
      ais->SetMeasuredGeometry(shape1);
    }
  }
  else
    ais = new PrsDim_DiameterDimension(shape1);

  if (IsPlanar)
  {
    Handle(Geom_Geometry) ageom2;
    GetGeom(aConst, ageom2);
    Handle(GeomPlane) aplane = Handle(GeomPlane)::DownCast(ageom2);
    if (aplane.IsNull())
    {
#ifdef OCCT_DEBUG
      std::cout << "ConstraintPresentationTools::ComputeDiameter: nul plane" << std::endl;
#endif
      NullifyAIS(anAIS);
      return;
    }
    // ais->SetCustomPlane(aplane);
  }
  anAIS = ais;
}

//=================================================================================================

void ConstraintPresentationTools::ComputeFix(const Handle(TDataXtd_Constraint)& aConst,
                                         Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 1)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeFix: at least one constrainte is needed"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  if (!aConst->IsPlanar())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeFix: must be a planar constraint" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  TopoShape          shape1;
  Handle(Geom_Geometry) ageom2;

  GetOneShape(aConst, shape1);
  if (shape1.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeFix : null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  GetGoodShape(shape1);
  GetGeom(aConst, ageom2);
  Handle(GeomPlane) aplane = Handle(GeomPlane)::DownCast(ageom2);
  if (aplane.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeFix: nul plane" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  //  Update de l'AIS1
  Handle(PrsDim_FixRelation) ais;
  if (!anAIS.IsNull())
  {
    ais = Handle(PrsDim_FixRelation)::DownCast(anAIS);
    if (ais.IsNull())
    {
      ais = new PrsDim_FixRelation(shape1, aplane);
    }
    else
    {
      ais->SetFirstShape(shape1);
      ais->SetPlane(aplane);
    }
  }
  else
    ais = new PrsDim_FixRelation(shape1, aplane);

  anAIS = ais;
}

//=================================================================================================

void ConstraintPresentationTools::ComputeOffset(const Handle(TDataXtd_Constraint)& aConst,
                                            Handle(VisualEntity)&     anAIS)
{
  // Get plane for planar constraint
  Standard_Boolean   is_planar(aConst->IsPlanar());
  Handle(GeomPlane) aplane;
  if (is_planar)
  {
    Handle(Geom_Geometry) ageom;
    GetGeom(aConst, ageom);
    aplane = Handle(GeomPlane)::DownCast(ageom);
    if (aplane.IsNull())
    {
#ifdef OCCT_DEBUG
      std::cout << "ConstraintPresentationTools::ComputeOffset: null plane" << std::endl;
#endif
      NullifyAIS(anAIS);
      return;
    }
  }

  // Get shapes
  TopoShape     S1, S2;
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom == 1)
  {
    Handle(ShapeAttribute) ageom1 = aConst->GetGeometry(1);
    // c'est une shape qui contient les faces generees par les faces d'origines
    Iterator1 It(ageom1);
    if (It.More())
    {
      S1 = It.OldShape();
      S2 = It.NewShape();
    }
  }
  else if (nbgeom == 2)
  {
    // Get geometry of the constraint
    GetTwoShapes(aConst, S1, S2);
  }

  if (S1.IsNull() || S2.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeOffset: null shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  Standard_Real                  val1;
  UtfString     txt;
  Handle(PrsDim_LengthDimension) ais;
  // Handle(StyleDrawer) aDrawer;

  if (nbgeom == 1)
  {
    ComputeTextAndValue(aConst, val1, txt, Standard_False);
    if (!anAIS.IsNull())
    {
      ais = Handle(PrsDim_LengthDimension)::DownCast(anAIS);
    }

    if (S1.ShapeType() == TopAbs_FACE && S2.ShapeType() == TopAbs_FACE)
    {
      if (ais.IsNull())
      {
        ais = new PrsDim_LengthDimension(TopoDS::Face(S1), TopoDS::Face(S2));
      }
      else
      {
        ais->SetMeasuredShapes(S1, S2);
        ais->SetCustomValue(val1);
      }

      if (is_planar)
      {
        ais->SetCustomPlane(aplane->Pln());
      }

      anAIS = ais;
      return;
    }
    else if (S1.ShapeType() == TopAbs_EDGE && S2.ShapeType() == TopAbs_EDGE)
    {
      // Find a plane for the dimension
      TopoEdge       OE = TopoDS::Edge(S1);
      BRepAdaptor_Curve CURVE(OE);
      if (CURVE.GetType() == GeomAbs_Line)
      {
        // Works only with line !!
        Handle(Geom_Geometry) aGeomGeometry = CURVE.Curve().Curve()->Transformed(CURVE.Trsf());
        gp_Lin                OLin          = Handle(GeomLine)::DownCast(aGeomGeometry)->Lin();
        TopoEdge           NE            = TopoDS::Edge(S2);
        CURVE.Initialize(NE);
        aGeomGeometry = CURVE.Curve().Curve()->Transformed(CURVE.Trsf());
        gp_Lin NLin   = Handle(GeomLine)::DownCast(aGeomGeometry)->Lin();
        Dir3d TDir(NLin.Location().XYZ() - OLin.Location().XYZ());
        aplane = new GeomPlane(NLin.Location(), NLin.Direction() ^ TDir);

        if (ais.IsNull())
        {
          ais = new PrsDim_LengthDimension(S1, S2, aplane->Pln());
        }
        else
        {
          ais->SetMeasuredShapes(S1, S2);
          ais->SetCustomValue(val1);

          ais->SetCustomPlane(aplane->Pln());
        }
        anAIS = ais;
        return;
      }
      else if (CURVE.GetType() == GeomAbs_Circle)
      {
        Handle(Geom_Geometry) aGeomGeometry = CURVE.Curve().Curve()->Transformed(CURVE.Trsf());
        Axis3d                ax = Handle(GeomCircle)::DownCast(aGeomGeometry)->Circ().Axis();
        aplane                   = new GeomPlane(ax.Location(), ax.Direction());
        is_planar                = Standard_True;
      }
    }
  }

  if (!is_planar)
  {
    if (S1.ShapeType() == TopAbs_COMPOUND && S2.ShapeType() == TopAbs_COMPOUND)
    {
      // Resultat d'un offset - on reconstruit un wire pour determiner un plan
      TopoWire  w1;
      ShapeBuilder B;
      B.MakeWire(w1);
      ShapeExplorer exp(S1, TopAbs_EDGE);
      for (; exp.More(); exp.Next())
        B.Add(w1, exp.Current());

      FaceMaker MkF(w1, Standard_True);
      if (MkF.IsDone())
      {
        Handle(GeomSurface) aGeomSurface = BRepInspector::Surface(MkF.Face());
        aplane                            = Handle(GeomPlane)::DownCast(aGeomSurface);
        is_planar                         = Standard_True;
      }
    }
  }

  if (is_planar)
  {
    ComputeTextAndValue(aConst, val1, txt, Standard_False);
    ShapeExplorer EXP1(S1, TopAbs_VERTEX);
    S1       = EXP1.Current();
    Point3d P = BRepInspector::Pnt(TopoDS::Vertex(S1));

    TopoVertex nearest;
    Standard_Real dist(RealLast());

    for (ShapeExplorer EXP2(S2, TopAbs_VERTEX); EXP2.More(); EXP2.Next())
    {
      const TopoVertex& current = TopoDS::Vertex(EXP2.Current());
      Point3d               curpnt  = BRepInspector::Pnt(current);
      Standard_Real        curdist = P.SquareDistance(curpnt);
      if (curdist < dist)
      {
        nearest = current;
        dist    = curdist;
      }
    }
    S2  = nearest;
    ais = Handle(PrsDim_LengthDimension)::DownCast(anAIS);
    if (ais.IsNull())
    {
      ais = new PrsDim_LengthDimension(S1, S2, aplane->Pln());
    }
    else
    {
      ais->SetMeasuredShapes(S1, S2);
      ais->SetCustomValue(val1);
      ais->SetCustomPlane(aplane->Pln());
    }
    anAIS = ais;
    return;
  }

#ifdef OCCT_DEBUG
  std::cout << "ConstraintPresentationTools::ComputeOffset: Case not implemented" << std::endl;
#endif
  NullifyAIS(anAIS);
}

//=================================================================================================

void ConstraintPresentationTools::ComputePlacement(const Handle(TDataXtd_Constraint)& aConst,
                                               Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 2)
    throw Standard_ProgramError(
      "ConstraintPresentationTools::ComputePlacement: at leat two constraints are needed");

  TopoShape shape1, shape2;
  GetTwoShapes(aConst, shape1, shape2);
  if (shape1.IsNull() || shape2.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputePlacement: nul shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  Standard_Real              val1 = 0.0;
  UtfString txt  = " ";
  if (aConst->IsDimension())
  {
    ComputeTextAndValue(aConst, val1, txt, Standard_False);
  }
  //  Update de l'AIS1
  Handle(PrsDim_OffsetDimension) ais;
  if (anAIS.IsNull())
  {
    ais = new PrsDim_OffsetDimension(GetFace(shape1), GetFace(shape2), val1, txt);
    ais->SetArrowSize(val1 / 20.);
  }
  else
  {
    ais = Handle(PrsDim_OffsetDimension)::DownCast(anAIS);
    if (ais.IsNull())
    {
      ais = new PrsDim_OffsetDimension(GetFace(shape1), GetFace(shape2), val1, txt);
      ais->SetArrowSize(val1 / 20.);
    }
    else
    {
      ais->SetFirstShape(GetFace(shape1));
      ais->SetSecondShape(GetFace(shape2));
      ais->SetValue(val1);
      ais->SetText(txt);
      ais->SetArrowSize(val1 / 20.);
    }
  }
  if (GetFace(shape1).IsNull() || GetFace(shape2).IsNull())
    ais.Nullify();
  anAIS = ais;
}

//=================================================================================================

void ConstraintPresentationTools::ComputeOthers(const Handle(TDataXtd_Constraint)& /*aConst*/,
                                            Handle(VisualEntity)& /*anAIS*/)
{
}

//=================================================================================================

void ConstraintPresentationTools::GetOneShape(const Handle(TDataXtd_Constraint)& aConst,
                                          TopoShape&                      aShape)
{
  const Handle(ShapeAttribute)& ageom1 = aConst->GetGeometry(1);
  if (!ageom1.IsNull())
    aShape = Tool11::CurrentShape(ageom1);
}

//=================================================================================================

void ConstraintPresentationTools::GetTwoShapes(const Handle(TDataXtd_Constraint)& aConst,
                                           TopoShape&                      aShape1,
                                           TopoShape&                      aShape2)
{
  const Handle(ShapeAttribute)& ageom1 = aConst->GetGeometry(1);
  if (!ageom1.IsNull())
    aShape1 = Tool11::CurrentShape(aConst->GetGeometry(1));
  const Handle(ShapeAttribute)& ageom2 = aConst->GetGeometry(2);
  if (!ageom2.IsNull())
    aShape2 = Tool11::CurrentShape(aConst->GetGeometry(2));
}

//=================================================================================================

void ConstraintPresentationTools::GetShapesAndGeom(const Handle(TDataXtd_Constraint)& aConst,
                                               TopoShape&                      aShape1,
                                               TopoShape&                      aShape2,
                                               Handle(Geom_Geometry)&             aGeom)
{
  GetTwoShapes(aConst, aShape1, aShape2);
  GetGeom(aConst, aGeom);
}

//=================================================================================================

void ConstraintPresentationTools::GetShapesAndGeom(const Handle(TDataXtd_Constraint)& aConst,
                                               TopoShape&                      aShape1,
                                               TopoShape&                      aShape2,
                                               TopoShape&                      aShape3,
                                               Handle(Geom_Geometry)&             aGeom)
{
  GetTwoShapes(aConst, aShape1, aShape2);
  const Handle(ShapeAttribute)& ageom3 = aConst->GetGeometry(3); // ota: GetGeometry(2) was
  if (!ageom3.IsNull())
    aShape3 = Tool11::CurrentShape(aConst->GetGeometry(3));
  GetGeom(aConst, aGeom);
}

//=================================================================================================

void ConstraintPresentationTools::GetShapesAndGeom(const Handle(TDataXtd_Constraint)& aConst,
                                               TopoShape&                      aShape1,
                                               TopoShape&                      aShape2,
                                               TopoShape&                      aShape3,
                                               TopoShape&                      aShape4,
                                               Handle(Geom_Geometry)&             aGeom)
{
  GetTwoShapes(aConst, aShape1, aShape2);
  const Handle(ShapeAttribute)& ageom3 = aConst->GetGeometry(3);
  if (!ageom3.IsNull())
    aShape3 = Tool11::CurrentShape(aConst->GetGeometry(3));
  const Handle(ShapeAttribute)& ageom4 = aConst->GetGeometry(4);
  if (!ageom4.IsNull())
    aShape4 = Tool11::CurrentShape(aConst->GetGeometry(4));
  GetGeom(aConst, aGeom);
}

//=================================================================================================

void ConstraintPresentationTools::ComputeCoincident(const Handle(TDataXtd_Constraint)& aConst,
                                                Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 2)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeCoincident: at leat two constraintes are needed"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  if (!aConst->IsPlanar())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeCoincident: must be a planar constraint"
              << std::endl;
#endif
    anAIS.Nullify();
    return;
  }

  TopoShape          shape1, shape2;
  Handle(Geom_Geometry) ageom;
  GetShapesAndGeom(aConst, shape1, shape2, ageom);
  Handle(GeomPlane) aplane = Handle(GeomPlane)::DownCast(ageom);
  if (shape1.IsNull() || shape2.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeCoincident: nul shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  GetGoodShape(shape1);
  GetGoodShape(shape2);
  if (aplane.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeCoincident: nul plane" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  //  Update de l'AIS1
  Handle(PrsDim_IdenticRelation) ais;
  if (anAIS.IsNull())
    ais = new PrsDim_IdenticRelation(shape1, shape2, aplane);
  else
  {
    ais = Handle(PrsDim_IdenticRelation)::DownCast(anAIS);
    if (ais.IsNull())
    {
      ais = new PrsDim_IdenticRelation(shape1, shape2, aplane);
    }
    else
    {
      ais->SetFirstShape(shape1);
      ais->SetSecondShape(shape2);
      ais->SetPlane(aplane);
    }
  }
  anAIS = ais;
}

//=================================================================================================

void ConstraintPresentationTools::ComputeRound(const Handle(TDataXtd_Constraint)& aConst,
                                           Handle(VisualEntity)&     anAIS)
{
  Standard_Integer nbgeom = aConst->NbGeometries();
  if (nbgeom < 1)
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputeRound: at leat one geometry is needed"
              << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }
  TopoShape shape1;
  GetOneShape(aConst, shape1);
  if (shape1.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::ComputePlacement: nul shape" << std::endl;
#endif
    NullifyAIS(anAIS);
    return;
  }

  Standard_Real              val1;
  UtfString txt;
  ComputeTextAndValue(aConst, val1, txt, Standard_False);

  //  Update de l'AIS1
  Handle(PrsDim_RadiusDimension) ais;

  {
    try
    {
      OCC_CATCH_SIGNALS
      if (anAIS.IsNull())
        ais = new PrsDim_RadiusDimension(shape1);
      else
      {
        ais = Handle(PrsDim_RadiusDimension)::DownCast(anAIS);
        if (ais.IsNull())
        {
          ais = new PrsDim_RadiusDimension(shape1);
        }
        else
        {
          ais->SetMeasuredGeometry(shape1);
        }
      }
    }
    catch (ExceptionBase const&)
    {
      ais.Nullify();
    }
  }
  anAIS = ais;
}

//=================================================================================================

void ConstraintPresentationTools::GetGeom(const Handle(TDataXtd_Constraint)& aConst,
                                      Handle(Geom_Geometry)&             aGeom)
{
  Handle(ShapeAttribute) atgeom = aConst->GetPlane();
  if (atgeom.IsNull())
  {
#ifdef OCCT_DEBUG
    std::cout << "ConstraintPresentationTools::GetGeom : aConst->GetPlane().IsNull()" << std::endl;
#endif
    return;
  }
  gp_Pln aplane;
  gp_Lin anaxis;
  Point3d apoint;

  DataLabel label = atgeom->Label();

  Handle(ShapeAttribute) NS;
  if (label.FindAttribute(ShapeAttribute::GetID(), NS))
  {
    TopoShape s = Tool11::GetShape(NS);
    if (s.IsNull())
      return;
  }

  if (TDataXtd_Geometry::Plane1(label, aplane))
    aGeom = new GeomPlane(aplane);
  else if (TDataXtd_Geometry::Line(label, anaxis))
    aGeom = new GeomLine(anaxis);
  else if (TDataXtd_Geometry::Point(label, apoint))
    aGeom = new Geom_CartesianPoint(apoint);
#ifdef OCCT_DEBUG
  else
  {
    std::cout << "ConstraintPresentationTools::GetGeom no geom on label " << std::endl;
  }
#endif
}
