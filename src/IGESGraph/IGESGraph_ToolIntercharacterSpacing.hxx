...
public:
  ...
  Standard_EXPORT void ReadOwnParams(const Handle(IGESGraph_IntercharacterSpacing)& ent,
                                     const Handle(IGESData_IGESReaderData)&         IR,
                                     IGESData_ParamReader&                          PR) const;

  //! Writes own parameters to IGESWriter
  Standard_EXPORT void WriteOwnParams(const Handle(IGESGraph_IntercharacterSpacing)& ent,
                                      IGESData_IGESWriter&                           IW) const;

  //! Lists the Entities shared by a IntercharacterSpacing <ent>, from
  //! its specific (own) parameters
  Standard_EXPORT void OwnShared(const Handle(IGESGraph_IntercharacterSpacing)& ent,
  ...
  Standard_EXPORT void OwnCheck(const Handle(IGESGraph_IntercharacterSpacing)& ent,
                                const Interface_ShareTool&                     shares,
                                Handle(Interface_Check)&                       ach) const;

  //! Copies Specific Parameters
  Standard_EXPORT void OwnCopy(const Handle(IGESGraph_IntercharacterSpacing)& entfrom,
                               const Handle(IGESGraph_IntercharacterSpacing)& entto,
                               Interface_CopyTool&                            TC) const;

  //! Dump of Specific Parameters
  Standard_EXPORT void OwnDump(const Handle(IGESGraph_IntercharacterSpacing)& ent,
