...
void MAT2d_Tool2d::CreateBisector(const Handle(MAT_Bisector)& abisector)
{
  ...
  Standard_Boolean ontheline        = AreNeighbours(edge1number, edge2number, NumberOfItems());
  Standard_Boolean InitialNeighbour = ontheline;

  if (theCircuit->ConnexionOn(edge2number))
    ontheline = Standard_False;
                             ...
                             const Standard_Integer StartOrEnd) const
{
  ...
  if (EdgeType != STANDARD_TYPE(Geom2d_CartesianPoint))
  {
    if (!InitialNeighbour)
    {
      Curve    = Handle(Geom2d_TrimmedCurve)::DownCast(theCircuit->Value(IndexEdge))->BasisCurve();
      ...
      if (!Line1.IsNull())
      {
        Line = new Geom2d_TrimmedCurve(Line1, 0., Precision1::Infinite());
        SetTrim(B1, Line);
      }
      if (!Line2.IsNull())
      {
        Line = new Geom2d_TrimmedCurve(Line2, 0., Precision1::Infinite());
        SetTrim(B1, Line);
      }
    }
  }
...
Standard_Boolean MAT2d_Tool2d::TrimBisector(const Handle(MAT_Bisector)& abisector)
{
  ...
  if (bisector->BasisCurve()->IsPeriodic() && param == Precision1::Infinite())
  {
...
Standard_Boolean MAT2d_Tool2d::IsSameDistance(const Handle(MAT_Bisector)& BisectorOne,
                                              ...
                                              Standard_Real&              Distance) const
{
  ...
  Distance              = Dist(1);
  if (theJoinType == GeomAbs_Intersection && Precision1::IsInfinite(Distance))
  {
    for (Standard_Integer i = 2; i <= 4; i++)
      if (!Precision1::IsInfinite(Dist(i)))
      {
        Distance = Dist(i);
        break;
      }
  ...
  {
    if (theJoinType == GeomAbs_Intersection && Precision1::IsInfinite(Dist(i)))
      continue;
                                              ...
                                              Standard_Integer&           IntPnt)
{
  ...
  DistanceMini   = Precision1::Infinite();
  Param1         = Precision1::Infinite();
  Param2         = Precision1::Infinite();
  ...
  if (Intersect.NbSegments() >= 1)
  {
    ...
    for (Standard_Integer i = 1; i <= Intersect.NbSegments(); i++)
    {
      ...
      {
        Parama =
          Handle(Bisector_Curve)::DownCast(Bisector1->BasisCurve())->Parameter(PointOnSegment);
        Paramb =
          Handle(Bisector_Curve)::DownCast(Bisector2->BasisCurve())->Parameter(PointOnSegment);
  ...
  if (BisectorOne->SecondParameter() < Precision1::Infinite()
      && BisectorOne->SecondParameter() < Param1 * (1. - Tolerance))
    return Precision1::Infinite();

  if (BisectorTwo->FirstParameter() < Precision1::Infinite()
                                     ...
                                     const Standard_Real         Param2) const
{
  ...
  if (Param1 != Precision1::Infinite() && Param2 != Precision1::Infinite())
  {
...
#ifndef OCCT_DEBUG
void MAT2d_Tool2d::Dump(const Standard_Integer, const Standard_Integer) const
{
  throw Standard_NotImplemented();
#else
void MAT2d_Tool2d::Dump(const Standard_Integer bisector, const Standard_Integer) const
{
  if (bisector == -1)
    return;
...
void MAT2d_Tool2d::BisecFusion(const Standard_Integer I1, const Standard_Integer I2)
{
...
static void SetTrim(Bisector_Bisec& Bis, const Handle(GeomCurve2d)& Line1)
{
  ...
  Standard_Real UTrim              = Precision1::Infinite();
