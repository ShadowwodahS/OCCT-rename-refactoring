...
// purpose  : Compute the chamfer in the particular case Plane1/Cylinder
//            or Cylinder/Plane1
                                       ...
                                       const Standard_Boolean   plandab)
{
  ...
  //    {VecTranslCyl.Rotate(Axis3d(OrSpine,XDir),alpha);}
  //  else
  //    {VecTranslCyl.Rotate(Axis3d(OrSpine,XDir.Reversed()),alpha);}

  //  POnCyl.SetXYZ( OrCyl.XYZ().Added(VecTranslCyl.XYZ()) );

  Vector3d VecCylTransl = Vector3d(OrCyl, OrSpine);

  if ((XDir.Crossed(Dir3d(VecCylTransl))).Dot(NorF) > 0.)
  {
    VecCylTransl.Rotate(Axis3d(OrCyl, XDir), alpha);
  }
  else
  {
    VecCylTransl.Rotate(Axis3d(OrCyl, XDir.Reversed()), alpha);
  }
  ...
  POnCyl = ElSLib1::CylinderValue(UOnCyl, VOnCyl, AxCyl, Cyl.Radius());
  ...
  Ax3 AxCh(POnPln, XDir.Crossed(YDir), XDir);
  ...
  gp_Lin2d LOnChamfer(gp_Pnt2d(U, VOnChamfer), gp1::DX2d());
  ...
  LOnChamfer = gp_Lin2d(gp_Pnt2d(U, VOnChamfer), gp1::DX2d());
