                                      ...
                                      const Standard_Boolean         DisOnP)
{
  ...
  if ((plandab && DisOnP) || (!plandab && !DisOnP))
  {
    ...
    if (dedans)
    {
      ...
      if (Abs(ChamfRad) < Precision1::Confusion())
    ...
    if (ouvert)
    {
      if (Abs(angCon) - Abs(SemiAngl) > -Precision1::Confusion())
      {
#ifdef OCCT_DEBUG
  ...
  else
  {
    ...
    if (ouvert)
    {
      ...
      if ((M_PI / 2. - SemiAngl) < Precision1::Confusion())
      {
#ifdef OCCT_DEBUG
        std::cout << "wrong choice of angle for the chamfer" << std::endl;
#endif
    ...
    else
    {
      ...
      if (Abs(SemiAngl) < Precision1::Confusion())
      {
      ...
      if (SemiAngl > Precision1::Confusion())
  ...
  if (iscylinder)
  {
    Handle(Geom_CylindricalSurface) gcyl = new Geom_CylindricalSurface(ChamfAx3, ChamfRad);
    ...
    if (plandab)
      ...
      v = -sqrt(dis * dis + move * move);
    p2dch.SetCoord(0., v);
    ElSLib1::CylinderD1(0., v, ChamfAx3, ChamfRad, Pt, deru, derv);
    ...
    if (angCon > Precision1::Confusion())
      DirCon.Reverse();
  ...
  {
    Handle(Geom_ConicalSurface) gcon = new Geom_ConicalSurface(ChamfAx3, SemiAngl, ChamfRad);
