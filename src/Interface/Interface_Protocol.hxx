...
DEFINE_STANDARD_HANDLE(Interface_Protocol, RefObject)
...
class Interface_Protocol : public RefObject
{
...
public:
  ...
  Standard_EXPORT virtual Standard_Boolean IsDynamicType(
  ...
  Standard_EXPORT virtual Standard_Integer NbTypes(const Handle(RefObject)& obj) const;
  ...
  Standard_EXPORT virtual Standard_Boolean IsSuitableModel(
    const Handle(Interface_InterfaceModel)& model) const = 0;

  //! Creates a new Unknown Entity for the considered Norm
  Standard_EXPORT virtual Handle(RefObject) UnknownEntity() const = 0;
  ...
  DEFINE_STANDARD_RTTIEXT(Interface_Protocol, RefObject)
