...
class ViewsVisibleTool
{
public:
  DEFINE_STANDARD_ALLOC

  //! Returns a ToolViewsVisible, ready to work
  Standard_EXPORT ViewsVisibleTool();

  //! Reads own parameters from file. <PR> gives access to them,
  //! <IR> detains parameter types and values
  Standard_EXPORT void ReadOwnParams(const Handle(IGESDraw_ViewsVisible)&   ent,
                                     const Handle(IGESData_IGESReaderData)& IR,
                                     IGESData_ParamReader&                  PR) const;

  //! Writes own parameters to IGESWriter
  Standard_EXPORT void WriteOwnParams(const Handle(IGESDraw_ViewsVisible)& ent,
                                      IGESData_IGESWriter&                 IW) const;

  //! Lists the Entities shared by a ViewsVisible <ent>, from
  //! its specific (own) parameters shared not implied (the Views)
  Standard_EXPORT void OwnShared(const Handle(IGESDraw_ViewsVisible)& ent,
                                 Interface_EntityIterator&            iter) const;

  //! Lists the Entities shared by a ViewsVisible <ent>, from
  //! its specific (own) implied parameters : the Displayed Entities
  Standard_EXPORT void OwnImplied(const Handle(IGESDraw_ViewsVisible)& ent,
                                  Interface_EntityIterator&            iter) const;

  //! Returns specific DirChecker
  Standard_EXPORT DirectoryChecker DirChecker(const Handle(IGESDraw_ViewsVisible)& ent) const;

  //! Performs Specific Semantic Check
  Standard_EXPORT void OwnCheck(const Handle(IGESDraw_ViewsVisible)& ent,
                                const Interface_ShareTool&           shares,
                                Handle(Interface_Check)&             ach) const;

  //! Copies Specific Parameters shared not implied, i.e. all but
  //! the Displayed Entities
  Standard_EXPORT void OwnCopy(const Handle(IGESDraw_ViewsVisible)& entfrom,
  ...
  Standard_EXPORT void OwnRenew(const Handle(IGESDraw_ViewsVisible)& entfrom,
                                const Handle(IGESDraw_ViewsVisible)& entto,
  ...
  Standard_EXPORT void OwnDump(const Handle(IGESDraw_ViewsVisible)& ent,
