                       ...
                       const Standard_Integer            theFaceId)
{
      ...
      && (theParams.FontHinting & Font_Hinting_Normal) != 0)
  {
    throw Standard_ProgramError(
      "Font_FTFont, Light and Normal hinting styles are mutually exclusive");
  }
#ifdef HAVE_FREETYPE
  setLoadFlag(FT_LOAD_TARGET_LIGHT, (theParams.FontHinting & Font_Hinting_Light) != 0);
  setLoadFlag(FT_LOAD_NO_HINTING,
              (theParams.FontHinting & Font_Hinting_Normal) == 0
  ...
  else if (FT_Set_Char_Size(myFTFace,
                            0L,
...
bool Font_FTFont::loadGlyph(const Standard_Utf32Char theUChar)
{
  ...
  if (myToUseUnicodeSubsetFallback && !HasSymbol(theUChar))
  {
    // try using fallback
    const Font_UnicodeSubset aSubset = CharSubset(theUChar);
    if (findAndInitFallback(aSubset) && myFallbackFaces[aSubset]->HasSymbol(theUChar))
    {
...
#ifdef HAVE_FREETYPE
  if (theUChar != 0 && myToUseUnicodeSubsetFallback && !HasSymbol(theUChar))
  {
    // try using fallback
    const Font_UnicodeSubset aSubset = CharSubset(theUChar);
    if (findAndInitFallback(aSubset) && myFallbackFaces[aSubset]->HasSymbol(theUChar))
...
float Font_FTFont::AdvanceX(Standard_Utf32Char theUCharNext) const
{
  ...
  return myWidthScaling
         * fromFTPoints<float>(myActiveFTFace->glyph->advance.x + aKern.x
                               + myActiveFTFace->glyph->lsb_delta
                               - myActiveFTFace->glyph->rsb_delta);
#else
  (void)theUCharNext;
  return 0.0f;
#endif
