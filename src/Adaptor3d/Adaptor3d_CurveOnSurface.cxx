...
IMPLEMENT_STANDARD_RTTIEXT(Adaptor3d_CurveOnSurface, Curve5)
...
static Frame3d to3d(const gp_Pln& Pl, const Ax22d& A)
{
...
{ // Warning: Hunt is used to find number of knot which equals coordinate component,
  ...
  constexpr Standard_Real Tol = Precision1::PConfusion() / 10;
...
Adaptor3d_CurveOnSurface::Adaptor3d_CurveOnSurface(const Handle(SurfaceAdaptor)& S)
    : myType(GeomAbs_OtherCurve),
      myIntCont(GeomAbs_CN)
{
  Load(S);
...
Handle(Curve5) Adaptor3d_CurveOnSurface::ShallowCopy() const
{
...
void Adaptor3d_CurveOnSurface::Load(const Handle(SurfaceAdaptor)& S)
{
  mySurface = S;
  if (!myCurve.IsNull())
    EvalKPart();
...
Handle(Curve5) Adaptor3d_CurveOnSurface::Trim(const Standard_Real First,
...
const Handle(SurfaceAdaptor)& Adaptor3d_CurveOnSurface::GetSurface() const
{
  return mySurface;
}
...
Handle(SurfaceAdaptor)& Adaptor3d_CurveOnSurface::ChangeSurface()
{
  return mySurface;
}
    ...
    if (CTy == GeomAbs_Line)
    {
      ...
      { // Iso U.
        ...
        else if (STy == GeomAbs_Cylinder)
        {
          ...
          Cylinder1 Cyl = mySurface->Cylinder();
          gp_Pnt2d    P   = myCurve->Line().Location();
          myLin           = ElSLib1::CylinderUIso(Cyl.Position1(), Cyl.Radius(), P.X());
        ...
        else if (STy == GeomAbs_Cone)
        {
          ...
          Cone1  Cone = mySurface->Cone();
          gp_Pnt2d P    = myCurve->Line().Location();
          myLin = ElSLib1::ConeUIso(Cone.Position1(), Cone.RefRadius(), Cone.SemiAngle(), P.X());
          Vector3d Tr(myLin.Direction());
          Tr.Multiply(P.Y());
          myLin.Translate(Tr);
          if (D.IsOpposite(gp1::DY2d(), Precision1::Angular()))
...
{
  Handle(Curve5) AHC = S->BasisCurve();
