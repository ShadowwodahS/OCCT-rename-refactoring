...
public:
  ...
  Standard_EXPORT void Perform(const TopoWire&     theSpine,
  ...
  const TopoShape& Shape() const { return myResult; }
...
protected:
  ...
  Standard_EXPORT void RemoveExcessSolids(const ShapeList& theLSplits,
                                          const TopoShape&         theShape,
                                          ShapeList&       theArgsList,
                                          BOPAlgo_MakerVolume&        theMV);

  Standard_EXPORT void ExtractOuterSolid(TopoShape& theShape, ShapeList& theArgsList);

  Standard_EXPORT void GetSpineAndProfile(const TopoWire& theSpine,
                                          const TopoWire& theProfile);

  Standard_EXPORT void UnifyShape();

  Standard_EXPORT Standard_Boolean PerformBoolean(const ShapeList& theArgsList,
                                                  TopoShape&               theResult) const;

  Standard_EXPORT Standard_Boolean
    CheckSingularityAndAdd(const TopoFace&    theF,
                           const Standard_Real   theFuzzyToler,
                           ShapeList& theListOfFaces,
                           ShapeList& theListOfSplits) const;

  Standard_EXPORT Standard_Boolean IsLid(const TopoFace&                theF,
...
private:
  ...
  TopoWire      mySpine;
  TopoWire      myProfile;
  TopoShape     myPipeShell;
  TopoCompound  myTopBottom; // Lids can be split on several faces
  TopoShape     myResult;
