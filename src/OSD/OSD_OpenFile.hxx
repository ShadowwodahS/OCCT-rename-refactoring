                           ...
                           const std::ios_base::openmode     theMode)
{
  ...
  if (theFileBuf.is_open())
    return false;
  // __gnu_cxx::stdio_filebuf is a std::filebuf providing extra constructor taking FILE* or file
  // descriptor; It does not modify virtual methods or add any fields - so we can safely use swap
  // (or move operator) here. MinGW does not provide open() methods taking wchar_t* or file
  // descriptor - thus, creating __gnu_cxx::stdio_filebuf is the only way for opening such files
  // since _wfopen()/_wsopen_s() from C world are available.
                           ...
                           const std::ios_base::openmode     theMode)
{
  ...
  // which considers implementation details of std::filebuf within
  // std::fstream/std::ifstream/std::ofstream. Should be removed when MinGW will be improved to
  // support wchar_t file paths natively within C++ streams.
