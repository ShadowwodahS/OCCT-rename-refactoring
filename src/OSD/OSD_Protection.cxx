...
static Standard_Integer __fastcall _get_mask(OSD_SingleProtection p)
{
  ...
  if (p == OSD_W || p == OSD_RW || p == OSD_WX || p == OSD_RWX || p == OSD_WXD || p == OSD_RWXD
      || p == OSD_WD || p == OSD_RWD)
    retVal |= FLAG_WRITE;

  if (p == OSD_X || p == OSD_RX || p == OSD_WX || p == OSD_RWX || p == OSD_XD || p == OSD_RXD
      || p == OSD_WXD || p == OSD_RWXD)
    retVal |= FLAG_EXECUTE;

  if (p == OSD_D || p == OSD_RD || p == OSD_WD || p == OSD_RWD || p == OSD_XD || p == OSD_RXD
      || p == OSD_WXD || p == OSD_RWXD)
    retVal |= FLAG_DELETE;
