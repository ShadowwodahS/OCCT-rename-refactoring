...
//!
//! Parallel1 class provides simple interface for parallel processing of
...
//! more efficient to use it directly instead of using Parallel1.
...
private:
  ...
  class IteratorInterface1
  {
  public:
    virtual ~IteratorInterface1() {}

    //! Returns true if iterators wrapped by this and theOther are equal
    virtual bool IsEqual(const IteratorInterface1& theOther) const = 0;
    ...
    virtual IteratorInterface1* Clone() const = 0;
  };
  ...
  template <class Type>
  class IteratorWrapper : public IteratorInterface1
  {
  public:
    IteratorWrapper() {}
    ...
    virtual bool IsEqual(const IteratorInterface1& theOther) const Standard_OVERRIDE
    {
    ...
    virtual IteratorInterface1* Clone() const Standard_OVERRIDE
    {
...
protected:
  // Note: UniversalIterator1 and FunctorInterface1 are made protected to be
  ...
  //! IteratorInterface1, which contains actual type-specific iterator.
  class UniversalIterator1
  ...
  public:
    ...
    using value_type        = IteratorInterface1*;
    ...
    UniversalIterator1(IteratorInterface1* theOther)
        : myPtr(theOther)
    {
    }

    UniversalIterator1(const UniversalIterator1& theOther)
        : myPtr(theOther.myPtr->Clone())
    {
    }

    UniversalIterator1& operator=(const UniversalIterator1& theOther)
    {
      myPtr.reset(theOther.myPtr->Clone());
      return *this;
    }

    bool operator!=(const UniversalIterator1& theOther) const
    {
    ...
    UniversalIterator1& operator++()
    {
      myPtr->Increment();
      return *this;
    }

    UniversalIterator1 operator++(int)
    {
      UniversalIterator1 aValue(*this);
      myPtr->Increment();
      return aValue;
    }
  ...
  private:
    std::unique_ptr<IteratorInterface1> myPtr;
  };

  //! Interface class representing functor object.
  //! Intended to add polymorphic behaviour to For and ForEach functionality
  //! enabling execution of arbitrary function in parallel mode.
  class FunctorInterface1
  {
  public:
    virtual ~FunctorInterface1() {}

    virtual void operator()(IteratorInterface1* theIterator) const = 0;

    // type cast to actual iterator
    template <typename Iterator>
    static const Iterator& DownCast(IteratorInterface1* theIterator)
    {
      return dynamic_cast<Parallel1::IteratorWrapper<Iterator>*>(theIterator)->Value();
    }
  };
...
private:
  ...
  class FunctorWrapperIter : public FunctorInterface1
  {
  public:
    ...
    virtual void operator()(IteratorInterface1* theIterator) const Standard_OVERRIDE
    {
  ...
  class FunctorWrapperInt : public FunctorInterface1
  {
  public:
...
private:
  ...
  Standard_EXPORT static void forEachOcct(UniversalIterator1&      theBegin,
  ...
  Standard_EXPORT static void forEachExternal(UniversalIterator1&      theBegin,
                                              UniversalIterator1&      theEnd,
                                              const FunctorInterface1& theFunctor,
                                              Standard_Integer        theNbItems);
...
public: //! @name public methods
                      ...
                      Standard_Integer       theNbItems                   = -1)
  {
    ...
    {
      UniversalIterator1 aBegin(new IteratorWrapper<InputIterator>(theBegin));
      UniversalIterator1 aEnd(new IteratorWrapper<InputIterator>(theEnd));
                  ...
                  const Standard_Boolean isForceSingleThreadExecution = Standard_False)
  {
    ...
    {
      UniversalIterator1          aBegin(new IteratorWrapper<Standard_Integer>(theBegin));
      UniversalIterator1          aEnd(new IteratorWrapper<Standard_Integer>(theEnd));
