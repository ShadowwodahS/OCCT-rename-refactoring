...
void StdPersistent_DataXtd_PatternStd::Import(const Handle(TDataXtd_PatternStd)& theAttribute) const
{
  ...
  if (mySignature < 5)
  {
    theAttribute->Axis1(Handle(ShapeAttribute)::DownCast(myAxis1->GetAttribute()));

    theAttribute->Value1(Handle(TDataStd_Real)::DownCast(myValue1->GetAttribute()));

    theAttribute->NbInstances1(Handle(IntAttribute)::DownCast(myNb1->GetAttribute()));

    if (mySignature > 2)
    {
      theAttribute->Axis2(Handle(ShapeAttribute)::DownCast(myAxis2->GetAttribute()));

      theAttribute->Value2(Handle(TDataStd_Real)::DownCast(myValue2->GetAttribute()));

      theAttribute->NbInstances2(Handle(IntAttribute)::DownCast(myNb2->GetAttribute()));
    }
  }
  else
  {
    theAttribute->Mirror(Handle(ShapeAttribute)::DownCast(myMirror->GetAttribute()));
  }
}
