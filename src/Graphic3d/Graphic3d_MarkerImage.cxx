                                 ...
                                 Standard_Integer&         theNumOfBytes)
{
  const Standard_Integer aType =
    Standard_Integer(theMarkerType > Aspect_TOM_O ? Aspect_TOM_O : theMarkerType);
  ...
  const Standard_Boolean theIsTopDown) const
{
  ...
  const Standard_Integer aNumOfBytesInRow =
    (Standard_Integer)(myImage->Width() / 8) + (myImage->Width() % 8 ? 1 : 0);
  ...
  for (Standard_Integer aRow = 0; aRow < aHeight; aRow++)
  {
    ...
    for (Standard_Integer aColumn = 0; aColumn < aWidth; aColumn++)
    {
      ...
      aBitMap->SetValue(
        anIndex,
        (Standard_Byte)(aBitMap->Value(anIndex) + (aBitOn ? (0x80 >> (aColumn % 8)) : 0)));
    }
  }
      ...
      for (Standard_Size aRowIter = 0; aRowIter < myImage->Height(); aRowIter++)
      {
        ...
        for (Standard_Size aColumnIter = 0; aColumnIter < myImage->Width(); aColumnIter++)
        {
          const Quantity_ColorRGBA aColor =
            myImage->PixelColor((Standard_Integer)aColumnIter, (Standard_Integer)aRowIter);
