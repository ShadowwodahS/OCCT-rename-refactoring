...
{
  int        aBits = theBits;
  const bool toUseDerivates =
    theUsesDerivates || (theBits & Graphic3d_ShaderFlags_StippleLine) != 0
  ...
  if (theNbSamples > 1)
  {
    ...
    aSrcFrag =
      ...
      + EOL "void main()" EOL "{" EOL "  ivec2 aSize  = textureSize (uColorSampler);" EOL
            ...
            "  gl_FragDepth = texelFetch (uDepthSampler, anUV, THE_NUM_SAMPLES / 2 - 1).r;" EOL EOL
            "  vec4 aColor = vec4 (0.0);" EOL
            "  for (int aSample = 0; aSample < THE_NUM_SAMPLES; ++aSample)" EOL "  {" EOL
            "    vec4 aVal = texelFetch (uColorSampler, anUV, aSample);" EOL
            "    aColor += aVal;" EOL "  }" EOL "  aColor /= float(THE_NUM_SAMPLES);" EOL
            "#ifdef THE_SHIFT_sRGB" EOL "  aColor.rgb = pow (aColor.rgb, vec3 (1.0 / 2.2));" EOL
            "#endif" EOL "  occSetFragColor (aColor);" EOL "}";
  }
  else
  {
    aUniforms.Append(
    ...
    aSrcFrag = AsciiString1() + (theIsFallback_sRGB ? EOL "#define THE_SHIFT_sRGB" : "")
               + EOL "void main()" EOL "{" EOL
                     ...
                     "#ifdef THE_SHIFT_sRGB" EOL
                     "  aColor.rgb = pow (aColor.rgb, vec3 (1.0 / 2.2));" EOL "#endif" EOL
  ...
  switch (myGapi)
  {
    ...
    case Aspect_GraphicsLibrary_OpenGLES: {
      ...
      else
      {
        ...
        aSrcFrag = EOL "void main()" EOL "{" EOL
                       "  occSetFragColor (occTexture2D (uColorSampler, TexCoord));" EOL "}";
      }
      break;
    }
  }
  ...
  if (!theMsaa)
  {
    ...
    aSrcFrag =
      EOL "void main()" EOL "{" EOL "  vec4 aAccum   = occTexture2D (uAccumTexture,  TexCoord);" EOL
          ...
          "  occSetFragColor (vec4 (aAccum.rgb / max (aWeight, 0.00001), aAccum.a));" EOL "}";
  }
  else
  {
    aUniforms.Append(
    ...
    aSrcFrag =
      EOL "void main()" EOL "{" EOL
          ...
          "  vec4 aAccum   = texelFetch (uAccumTexture,  aTexel, gl_SampleID);" EOL
          "  float aWeight = texelFetch (uWeightTexture, aTexel, gl_SampleID).r;" EOL
          "  occSetFragColor (vec4 (aAccum.rgb / max (aWeight, 0.00001), aAccum.a));" EOL "}";
  }
  ...
  Standard_Boolean theMsaa) const
{
  ...
  aSrcFrag =
    ...
    "  vec4  aFrontColor = texelFetch (uDepthPeelingFrontColor, aFragCoord, THE_SAMPLE_ID);" EOL
    "  vec4  aBackColor  = texelFetch (uDepthPeelingBackColor,  aFragCoord, THE_SAMPLE_ID);" EOL
    "  float anAlphaMult = 1.0 - aFrontColor.a;" EOL
    "  occFragColor = vec4 (aFrontColor.rgb + anAlphaMult * aBackColor.rgb, aFrontColor.a + "
    "aBackColor.a);" EOL "}";
...
AsciiString1 Graphic3d_ShaderManager::pointSpriteAlphaSrc(Standard_Integer theBits) const
{
  ...
  return isAlpha && myUseRedAlpha
           ? EOL "float getAlpha(void) { return "
                 "occTexture2D(occSamplerPointSprite, " THE_VEC2_glPointCoord ").r; }"
           : EOL "float getAlpha(void) { return "
  ...
  Standard_Integer               theBits) const
{
  ...
  if ((theBits & Graphic3d_ShaderFlags_PointSpriteA) == Graphic3d_ShaderFlags_PointSpriteA)
  {
    aSrcFragGetColor = pointSpriteAlphaSrc(theBits)
                       ...
                       + ";" EOL "  aColor.a = getAlpha();" EOL
                         "  if (aColor.a <= 0.1) discard;" EOL "  return aColor;" EOL "}";
  }
  ...
  Standard_Integer                            theBits)
{
  ...
  if ((theBits & Graphic3d_ShaderFlags_MeshEdges) != 0)
  {
    ...
    aSrcMainGeom =
      ...
      + EOL "vec3 ViewPortTransform (vec4 theVec)" EOL "{" EOL
            ...
            "  aWinCoord.xy = aWinCoord.xy * occViewport.zw + occViewport.xy;" EOL
            "  return aWinCoord;" EOL "}"
      + aSrcMainGeom
      + EOL "  vec3 aSideA = ViewPortTransform (gl_in[2].gl_Position) - ViewPortTransform "
            "(gl_in[1].gl_Position);" EOL
            "  vec3 aSideB = ViewPortTransform (gl_in[2].gl_Position) - ViewPortTransform "
            "(gl_in[0].gl_Position);" EOL
            "  vec3 aSideC = ViewPortTransform (gl_in[1].gl_Position) - ViewPortTransform "
            "(gl_in[0].gl_Position);" EOL
  ...
  for (Standard_Integer aVertIter = 0; aVertIter < 3; ++aVertIter)
  {
    ...
    if ((theBits & Graphic3d_ShaderFlags_MeshEdges) != 0)
    {
      ...
      {
        case 0:
          aSrcMainGeom += EOL "  EdgeDistance = vec3 (aHeightABC[0], 0.0, aQuadModeHeightC);";
          break;
        case 1:
          aSrcMainGeom += EOL "  EdgeDistance = vec3 (0.0, aHeightABC[1], aQuadModeHeightC);";
          break;
        case 2:
  ...
  Standard_Boolean theIsOutline) const
{
  ...
  if ((theBits & Graphic3d_ShaderFlags_IsPoint) != 0)
  {
    ...
    if ((theBits & Graphic3d_ShaderFlags_PointSprite) != 0)
    {
      ...
      aSrcFragMainGetColor =
        EOL "  vec4 aColor = getColor();" EOL "  aColor.a = getAlpha();" EOL
            "  if (aColor.a <= 0.1) discard;" EOL "  occSetFragColor (aColor);";
    }
    else
    {
      ...
      aSrcFragMainGetColor =
        EOL "  vec4 aColor = getColor();" EOL "  if (aColor.a <= 0.1) discard;" EOL
  ...
  else
  {
    if ((theBits & Graphic3d_ShaderFlags_HasTextures) != 0)
    {
      ...
      if ((theBits & Graphic3d_ShaderFlags_HasTextures) == Graphic3d_ShaderFlags_TextureEnv)
      {
        ...
        aSrcVertExtraMain +=
          EOL "  vec4 aPosition = occWorldViewMatrix * occModelWorldMatrix * occVertex;" EOL
              "  vec3 aNormal   = transformNormal (occNormal);" EOL
              "  vec3 aReflect  = reflect (normalize (aPosition.xyz), aNormal);" EOL
    ...
    if ((aBits & Graphic3d_ShaderFlags_StippleLine) != 0)
    {
      ...
      aSrcFragMainGetColor =
        ...
        + EOL "  vec2 anAxis = vec2 (0.0, 1.0);" EOL
              ...
              "  float aRotatePoint = dot (gl_FragCoord.xy, anAxis);"
        + (hasGlslBitwiseOps() ? EOL
             "  uint aBit = uint (floor (aRotatePoint / occStippleFactor + 0.5)) & 15U;" EOL
             "  if ((uint (occStipplePattern) & (1U << aBit)) == 0U) discard;"
                               : EOL
             "  int aBit = int (mod (floor (aRotatePoint / occStippleFactor + 0.5), 16.0));" EOL
             "  if (!occStipplePattern[aBit]) discard;")
        + EOL "  vec4 aColor = getFinalColor();" EOL "  if (aColor.a <= 0.1) discard;" EOL
              "  occSetFragColor (aColor);";
    }
    else
    {
  ...
  aSrcFrag = aSrcFragGetColor + aSrcGetAlpha
             ...
             + aSrcFragExtraMain + aSrcFragMainGetColor + EOL "}";
  ...
  if (!theLights.IsNull())
  {
          ...
          case Graphic3d_TypeOfLightSource_Directional: {
            if (theNbShadowMaps > 0 && aLightIter.Value()->ToCastShadows())
            {
              aLightsLoop = aLightsLoop + EOL "    occDirectionalLight (" + anIndex
                            + ", theNormal, theView, theIsFront," EOL
                              "                         occLightShadow (occShadowMapSamplers["
                            + anIndex + "], " + anIndex + ", theNormal));";
            }
            else
            {
              aLightsLoop = aLightsLoop + EOL "    occDirectionalLight (" + anIndex
                            + ", theNormal, theView, theIsFront, 1.0);";
            }
            ++anIndex;
            break;
          }
          ...
          case Graphic3d_TypeOfLightSource_Spot: {
            if (theNbShadowMaps > 0 && aLightIter.Value()->ToCastShadows())
            {
              aLightsLoop = aLightsLoop + EOL "    occSpotLight (" + anIndex
                            + ", theNormal, theView, aPoint, theIsFront," EOL
                              "                  occLightShadow (occShadowMapSamplers["
                            + anIndex + "], " + anIndex + ", theNormal));";
            }
            else
            {
              aLightsLoop = aLightsLoop + EOL "    occSpotLight (" + anIndex
                            + ", theNormal, theView, aPoint, theIsFront, 1.0);";
            }
            ++anIndex;
            break;
          }
    ...
    else
    {
      ...
      aLightsLoop        = aLightsLoop
                    + EOL "    for (int anIndex = 0; anIndex < occLightSourcesCount; ++anIndex)" EOL
                          "    {" EOL "      int aType = occLight_Type (anIndex);";
      if (theLights->NbEnabledLightsOfType(Graphic3d_TypeOfLightSource_Directional) > 0)
      {
        isFirstInLoop = false;
        aLightsLoop +=
          EOL "      if (aType == OccLightType_Direct)" EOL "      {" EOL
              "        occDirectionalLight (anIndex, theNormal, theView, theIsFront, 1.0);" EOL
              "      }";
      }
      if (theLights->NbEnabledLightsOfType(Graphic3d_TypeOfLightSource_Positional) > 0)
      {
        ...
        isFirstInLoop = false;
        aLightsLoop += EOL
          "      if (aType == OccLightType_Point)" EOL "      {" EOL
          "        occPointLight (anIndex, theNormal, theView, aPoint, theIsFront);" EOL "      }";
      }
      if (theLights->NbEnabledLightsOfType(Graphic3d_TypeOfLightSource_Spot) > 0)
      {
        ...
        isFirstInLoop = false;
        aLightsLoop +=
          EOL "      if (aType == OccLightType_Spot)" EOL "      {" EOL
              "        occSpotLight (anIndex, theNormal, theView, aPoint, theIsFront, 1.0);" EOL
              "      }";
      }
      aLightsLoop += EOL "    }";
    }
    ...
    if (theLights->NbEnabledLightsOfType(Graphic3d_TypeOfLightSource_Directional) == 1
        && theNbLights == 1 && !theIsPBR && theNbShadowMaps == 0)
    {
    ...
    else if (theLights->NbEnabledLightsOfType(Graphic3d_TypeOfLightSource_Directional) > 0)
    {
      ...
      aLightsFunc +=
        theIsPBR ? Shaders_PBRDirectionalLight_glsl : Shaders_PhongDirectionalLight_glsl;
    }
    if (theLights->NbEnabledLightsOfType(Graphic3d_TypeOfLightSource_Positional) > 0)
    {
  ...
  if (!theIsPBR)
  {
    return AsciiString1() + THE_FUNC_lightDef + Shaders_PointLightAttenuation_glsl
           ...
           + EOL EOL "vec4 computeLighting (in vec3 theNormal," EOL
                     ...
                     "  Ambient  = occLightAmbient.rgb;" EOL "  Diffuse  = vec3 (0.0);" EOL
                     "  Specular = vec3 (0.0);" EOL "  vec3 aPoint = thePoint.xyz / thePoint.w;"
           + aLightsLoop
           + EOL "  vec3 aMatAmbient  = occMaterial_Ambient(theIsFront);" EOL
                 "  vec4 aMatDiffuse  = occMaterial_Diffuse(theIsFront);" EOL
           ...
           + (theHasTexColor ? EOL
                ...
                "#endif"
                             : "")
           + EOL
           "  occMaterialOcclusion(aColor.rgb, TexCoord.st / TexCoord.w);" EOL
           "  vec3 aMatEmission = occMaterialEmission(theIsFront, TexCoord.st / TexCoord.w);" EOL
           "  aColor.rgb += aMatEmission.rgb;" EOL "  return aColor;" EOL "}";
  }
  else
  {
    return AsciiString1() + THE_FUNC_PBR_lightDef + Shaders_PointLightAttenuation_glsl
           ...
           "  NormalizedRoughness = occMaterialRoughness(theIsFront, TexCoord.st / TexCoord.w);" EOL
           "  Roughness = occRoughness (NormalizedRoughness);" EOL
           "  IOR       = occPBRMaterial_IOR (theIsFront);" EOL
           "  vec3 aPoint = thePoint.xyz / thePoint.w;"
           + aLightsLoop
           + EOL
           "  vec3 aColor = DirectLighting;" EOL
           "  vec3 anIndirectLightingSpec = occPBRFresnel (BaseColor.rgb, Metallic, IOR);" EOL
           "  vec2 aCoeff = occTexture2D (occEnvLUT, vec2(abs(dot(theView, theNormal)), "
           "NormalizedRoughness)).xy;" EOL "  anIndirectLightingSpec *= aCoeff.x;" EOL
           "  anIndirectLightingSpec += aCoeff.y;" EOL
           "  anIndirectLightingSpec *= occTextureCubeLod (occSpecIBLMap, -reflect (theView, "
           "theNormal), NormalizedRoughness * float (occNbSpecIBLLevels - 1)).rgb;" EOL
           "  vec3 aRefractionCoeff = 1.0 - occPBRFresnel (BaseColor.rgb, Metallic, "
           "NormalizedRoughness, IOR, abs(dot(theView, theNormal)));" EOL
           "  aRefractionCoeff *= (1.0 - Metallic);" EOL
           "  vec3 anIndirectLightingDiff = aRefractionCoeff * BaseColor.rgb * BaseColor.a;" EOL
           "  anIndirectLightingDiff *= occDiffIBLMap (theNormal).rgb;" EOL
           "  aColor += occLightAmbient.rgb * (anIndirectLightingDiff + "
           "anIndirectLightingSpec);" EOL "  aColor += Emission;" EOL
           "  occMaterialOcclusion(aColor, TexCoord.st / TexCoord.w);" EOL
           "  return vec4 (aColor, mix(1.0, BaseColor.a, aRefractionCoeff.x));" EOL "}";
  }
}
  ...
  Standard_Integer                  theBits) const
{
  ...
  else
  {
    if ((theBits & Graphic3d_ShaderFlags_TextureRGB) != 0)
    {
      ...
      aSrcFragGetColor = EOL
        "vec4 getColor(void)" EOL "{" EOL
        "  vec4 aColor = gl_FrontFacing ? FrontColor : BackColor;" EOL
        "  return occTexture2D(occSamplerBaseColor, TexCoord.st / TexCoord.w) * aColor;" EOL "}";
    }
  }
  ...
  aSrcVert =
    ...
    + EOL "void main()" EOL "{" EOL "  vec4 aPositionWorld = occModelWorldMatrix * occVertex;" EOL
          ...
          "    aView = (occWorldViewMatrixInverse * vec4(0.0, 0.0, 1.0, 0.0)).xyz;" EOL "  }" EOL
          "  else" EOL "  {" EOL
  ...
  const Standard_Integer            theNbShadowMaps) const
{
  ...
  else
  {
        ...
        && myHasFlatShading)
    {
      ...
      aSrcFragExtraMain += EOL
        "#if defined(THE_HAS_TEXTURE_NORMAL)" EOL "  vec2 aTexCoord = TexCoord.st / TexCoord.w;" EOL
        "  vec4 aMapNormalValue = occTextureNormal(aTexCoord);" EOL
        "  if (aMapNormalValue.w > 0.5)" EOL "  {" EOL
        "    mat2 aDeltaUVMatrix = mat2 (dFdx(aTexCoord), dFdy(aTexCoord));" EOL
        "    mat2x3 aDeltaVectorMatrix = mat2x3 (dFdx (PositionWorld.xyz), dFdy "
        "(PositionWorld.xyz));" EOL
        "    Normal = TangentSpaceNormal (aDeltaUVMatrix, aDeltaVectorMatrix, aMapNormalValue.xyz, "
        "Normal, !gl_FrontFacing);" EOL "  }" EOL "#endif";
    }
  }
  ...
  if (theNbShadowMaps > 0)
  {
    ...
    aSrcVertExtraMain += EOL
      ...
      "    PosLightSpace[aShadowIter] = occShadowMapMatrices[aShadowIter] * PositionWorld;" EOL
      "  }";
  }
  ...
  aSrcVert = AsciiString1() + aSrcVertExtraFunc
             ...
             "  if (occProjectionMatrix[3][3] == 1.0)" EOL "  {" EOL
             "    View = (occWorldViewMatrixInverse * vec4(0.0, 0.0, 1.0, 0.0)).xyz;" EOL "  }" EOL
             "  else" EOL "  {" EOL
             "    vec3 anEye = (occWorldViewMatrixInverse * vec4(0.0, 0.0, 0.0, 1.0)).xyz;" EOL
  ...
  switch (theStereoMode)
  {
    case Graphic3d_StereoMode_Anaglyph: {
      ...
      const AsciiString1 aNormalize =
        mySRgbState ? EOL "#define sRgb2linear(theColor) theColor" EOL
                    ...
                    : EOL
          "#define sRgb2linear(theColor) pow(theColor, vec4(2.2, 2.2, 2.2, 1.0))" EOL
          "#define linear2sRgb(theColor) pow(theColor, 1.0 / vec4(2.2, 2.2, 2.2, 1.0))";
      aSrcFrag = aNormalize
                 + EOL "void main()" EOL "{" EOL
                       "  vec4 aColorL = occTexture2D (uLeftSampler,  TexCoord);" EOL
                       "  vec4 aColorR = occTexture2D (uRightSampler, TexCoord);" EOL
                       "  aColorL = sRgb2linear (aColorL);" EOL
                       "  aColorR = sRgb2linear (aColorR);" EOL
                       "  vec4 aColor = uMultR * aColorR + uMultL * aColorL;" EOL
    ...
    case Graphic3d_StereoMode_RowInterlaced: {
      ...
      aSrcFrag = EOL "void main()" EOL "{" EOL "  vec2 aTexCoordL = TexCoord - uTexOffset;" EOL
                     ...
                     "  vec4 aColorR = occTexture2D (uRightSampler, aTexCoordR);" EOL
                     "  if (int (mod (gl_FragCoord.y - 1023.5, 2.0)) != 1)" EOL "  {" EOL
                     "    occSetFragColor (aColorL);" EOL "  }" EOL "  else" EOL "  {" EOL
    ...
    case Graphic3d_StereoMode_ColumnInterlaced: {
      ...
      aSrcFrag = EOL "void main()" EOL "{" EOL "  vec2 aTexCoordL = TexCoord - uTexOffset;" EOL
                     ...
                     "  vec4 aColorL = occTexture2D (uLeftSampler,  aTexCoordL);" EOL
                     "  vec4 aColorR = occTexture2D (uRightSampler, aTexCoordR);" EOL
                     "  if (int (mod (gl_FragCoord.x - 1023.5, 2.0)) == 1)" EOL "  {" EOL
                     "    occSetFragColor (aColorL);" EOL "  }" EOL "  else" EOL "  {" EOL
                     "    occSetFragColor (aColorR);" EOL "  }" EOL "}";
      break;
    }
    case Graphic3d_StereoMode_ChessBoard: {
      ...
      aSrcFrag = EOL "void main()" EOL "{" EOL "  vec2 aTexCoordL = TexCoord - uTexOffset;" EOL
                     ...
                     "  vec4 aColorL = occTexture2D (uLeftSampler,  aTexCoordL);" EOL
                     "  vec4 aColorR = occTexture2D (uRightSampler, aTexCoordR);" EOL
                     "  bool isEvenX = int(mod(floor(gl_FragCoord.x - 1023.5), 2.0)) != 1;" EOL
                     "  bool isEvenY = int(mod(floor(gl_FragCoord.y - 1023.5), 2.0)) == 1;" EOL
                     "  if ((isEvenX && isEvenY) || (!isEvenX && !isEvenY))" EOL "  {" EOL
                     "    occSetFragColor (aColorL);" EOL "  }" EOL "  else" EOL "  {" EOL
                     "    occSetFragColor (aColorR);" EOL "  }" EOL "}";
      break;
    }
    case Graphic3d_StereoMode_SideBySide: {
      ...
      aSrcFrag =
        EOL "void main()" EOL "{" EOL "  vec2 aTexCoord = vec2 (TexCoord.x * 2.0, TexCoord.y);" EOL
            "  if (TexCoord.x > 0.5)" EOL "  {" EOL "    aTexCoord.x -= 1.0;" EOL "  }" EOL
            "  vec4 aColorL = occTexture2D (uLeftSampler,  aTexCoord);" EOL
            "  vec4 aColorR = occTexture2D (uRightSampler, aTexCoord);" EOL
            "  if (TexCoord.x <= 0.5)" EOL "  {" EOL "    occSetFragColor (aColorL);" EOL "  }" EOL
    ...
    case Graphic3d_StereoMode_OverUnder: {
      ...
      aSrcFrag =
        EOL "void main()" EOL "{" EOL "  vec2 aTexCoord = vec2 (TexCoord.x, TexCoord.y * 2.0);" EOL
            "  if (TexCoord.y > 0.5)" EOL "  {" EOL "    aTexCoord.y -= 1.0;" EOL "  }" EOL
            "  vec4 aColorL = occTexture2D (uLeftSampler,  aTexCoord);" EOL
            "  vec4 aColorR = occTexture2D (uRightSampler, aTexCoord);" EOL
            "  if (TexCoord.y <= 0.5)" EOL "  {" EOL "    occSetFragColor (aColorL);" EOL "  }" EOL
            "  else" EOL "  {" EOL "    occSetFragColor (aColorR);" EOL "  }" EOL "}";
      break;
    }
    ...
    default: {
      aSrcFrag =
        EOL "void main()" EOL "{" EOL "  vec4 aColorL = occTexture2D (uLeftSampler,  TexCoord);" EOL
            ...
            "  aColorL.g = 0.0;" EOL "  aColorR.r = 0.0;" EOL
            "  occSetFragColor (aColorL + aColorR);" EOL "}";
      break;
    }
  }
  ...
  Standard_Integer theIndex) const
{
  ...
  static const char* THE_BAKE_NAMES[3] = {"pbr_env_baking_diffuse",
                                          "pbr_env_baking_specular",
                                          "pbr_env_baking_difffallback"};
