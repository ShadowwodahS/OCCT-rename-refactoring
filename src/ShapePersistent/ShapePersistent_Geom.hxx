// Copyright (c) 2015 OPEN CASCADE SAS
//
// This file is part of Open CASCADE Technology software library.
//
// This library is free software; you can redistribute it and/or modify it under
// the terms of the GNU Lesser General Public License version 2.1 as published
// by the Free Software Foundation, with special exception defined in the file
// OCCT_LGPL_EXCEPTION.txt. Consult the file LICENSE_LGPL_21.txt included in OCCT
// distribution for complete text of the license and disclaimer of any warranty.
//
// Alternatively, this file may be used under the terms of Open CASCADE
// commercial license or contractual agreement.

#ifndef _ShapePersistent_Geom_HeaderFile
#define _ShapePersistent_Geom_HeaderFile

#include <Standard_NotImplemented.hxx>
#include <Standard_NullObject.hxx>

#include <StdObjMgt_SharedObject.hxx>
#include <StdObjMgt_TransientPersistentMap.hxx>

#include <StdObject_gp_Curves.hxx>
#include <StdObject_gp_Surfaces.hxx>
#include <StdObject_gp_Trsfs.hxx>

#include <Geom_CartesianPoint.hxx>
#include <Geom_Direction.hxx>
#include <Geom_VectorWithMagnitude.hxx>
#include <Geom_Axis1Placement.hxx>
#include <Geom_Axis2Placement.hxx>
#include <Geom_Transformation.hxx>
#include <Geom_Surface.hxx>

class Geom1 : public SharedObject
{
public:
  class Geometry1 : public StdObjMgt_Persistent
  {
  public:
    //! Read persistent data from a file.
    Standard_EXPORT virtual void Read(ReadData& theReadData);
    //! Write persistent data to a file.
    Standard_EXPORT virtual void Write(WriteData& theWriteData) const;
    //! Gets persistent objects
    Standard_EXPORT virtual void PChildren(SequenceOfPersistent& theChildren) const;

    //! Returns persistent type name
    virtual Standard_CString PName() const { return "PGeom_Geometry"; }
  };

protected:
  template <class Transient>
  struct geometryBase : public DelayedBase<Geometry1, Transient>
  {
    //! Write persistent data to a file.
    virtual void Write(WriteData&) const
    {
      Standard_NotImplemented::Raise("Geom1::geometryBase::Write - not implemented");
    }

    //! Gets persistent child objects
    virtual void PChildren(StdObjMgt_Persistent::SequenceOfPersistent&) const {}

    //! Returns persistent type name
    virtual Standard_CString PName() const
    {
      Standard_NotImplemented::Raise("Geom1::geometryBase::PName - not implemented");
      return "";
    }
  };

  template <class Base, class PData>
  class subBase : public Base
  {
  public:
    //! Read persistent data from a file.
    virtual void Read(ReadData& theReadData) { PData().Read(theReadData); }

    //! Write persistent data to a file.
    virtual void Write(WriteData& theWriteData) const { PData().Write(theWriteData); }

    //! Gets persistent child objects
    virtual void PChildren(StdObjMgt_Persistent::SequenceOfPersistent&) const
    {
      Standard_NotImplemented::Raise("Geom1::subBase::PChildren - not implemented");
    }

    //! Returns persistent type name
    virtual Standard_CString PName() const
    {
      Standard_NotImplemented::Raise("Geom1::subBase::PName - not implemented");
      return "";
    }
  };

  template <class Base, class GpData>
  struct subBase_gp : public Base
  {
  public:
    //! Read persistent data from a file.
    virtual void Read(ReadData&) {}

    //! Write persistent data to a file.
    virtual void Write(WriteData&) const {}

    //! Gets persistent child objects
    virtual void PChildren(StdObjMgt_Persistent::SequenceOfPersistent&) const {}

    //! Returns persistent type name
    virtual Standard_CString PName() const
    {
      Standard_NotImplemented::Raise("Geom1::subBase_gp::PName - not implemented");
      return "";
    }
  };

  template <class Base>
  struct subBase_empty : Base
  {
    //! Returns persistent type name
    virtual Standard_CString PName() const
    {
      Standard_NotImplemented::Raise(
        "Geom1::subBase_empty::PName - not implemented");
      return "";
    }
  };

  template <class Base, class Target, class Data = void>
  class instance : public Base
  {
  public:
    //! Read persistent data from a file.
    virtual void Read(ReadData& theReadData)
    {
      Data aData;
      theReadData >> aData;
      this->myTransient = new Target(aData);
    }

    //! Gets persistent child objects
    virtual void PChildren(StdObjMgt_Persistent::SequenceOfPersistent&) const {}

    //! Write persistent data to a file.
    virtual void Write(WriteData&) const
    {
      Standard_NotImplemented::Raise("Geom1::instance::Write - not implemented");
    }

    //! Returns persistent type name
    virtual Standard_CString PName() const
    {
      Standard_NotImplemented::Raise("Geom1::instance::PName - not implemented");
      return "";
    }
  };

private:
  typedef geometryBase<Geometry3> basic;

public:
  typedef subBase_empty<basic>                         Point;
  typedef instance<Point, Geom_CartesianPoint, Point3d> CartesianPoint;

  typedef subBase_gp<basic, Vector3d>                          Vector;
  typedef instance<Vector, Geom_Direction, Dir3d>           Direction;
  typedef instance<Vector, Geom_VectorWithMagnitude, Vector3d> VectorWithMagnitude;

  typedef subBase_gp<basic, Axis3d>                            AxisPlacement;
  typedef instance<AxisPlacement, Geom_Axis1Placement, Axis3d> Axis1Placement;
  typedef instance<AxisPlacement, Geom_Axis2Placement>         Axis2Placement;

  typedef instance<SharedBase<Transformation1>, Transformation1, Transform3d> Transformation;

  typedef geometryBase<GeomCurve3d>   Curve;
  typedef geometryBase<GeomSurface> Surface;

public:
  //! Create a persistent object for a curve
  Standard_EXPORT static Handle(Curve) Translate(const Handle(GeomCurve3d)&         theCurve,
                                                 StdObjMgt_TransientPersistentMap& theMap);
  //! Create a persistent object for a curve
  Standard_EXPORT static Handle(Surface) Translate(const Handle(GeomSurface)&       theSurf,
                                                   StdObjMgt_TransientPersistentMap& theMap);
};

//=======================================================================
// Point
//=======================================================================

template <>
inline Standard_CString Geom1::subBase_empty<Geom1::basic>::PName()
  const
{
  return "PGeom_Point";
}

//=======================================================================
// CartesianPoint
//=======================================================================

template <>
inline Standard_CString Geom1::
  instance<Geom1::Point, Geom_CartesianPoint, Point3d>::PName() const
{
  return "PGeom_CartesianPoint";
}

template <>
inline void Geom1::instance<Geom1::Point,
                                           Geom_CartesianPoint,
                                           Point3d>::Write(WriteData& theWriteData) const
{
  Handle(Geom_CartesianPoint) aMyGeom = Handle(Geom_CartesianPoint)::DownCast(myTransient);
  theWriteData << aMyGeom->Pnt();
}

//=======================================================================
// Vector
//=======================================================================

template <>
inline Standard_CString Geom1::subBase_gp<Geom1::basic,
                                                         Vector3d>::PName() const
{
  return "PGeom_Vector";
}

//=======================================================================
// Direction
//=======================================================================

template <>
inline Standard_CString Geom1::
  instance<Geom1::Direction, Geom_Direction, Dir3d>::PName() const
{
  return "PGeom_Direction";
}

template <>
inline void Geom1::instance<Geom1::Direction,
                                           Geom_Direction,
                                           Dir3d>::Write(WriteData& theWriteData) const
{
  Handle(Geom_Direction) aMyGeom = Handle(Geom_Direction)::DownCast(myTransient);
  theWriteData << aMyGeom->Dir();
}

//=======================================================================
// VectorWithMagnitude
//=======================================================================

template <>
inline Standard_CString Geom1::instance<Geom1::VectorWithMagnitude,
                                                       Geom_VectorWithMagnitude,
                                                       Vector3d>::PName() const
{
  return "PGeom_VectorWithMagnitude";
}

template <>
inline void Geom1::instance<Geom1::VectorWithMagnitude,
                                           Geom_VectorWithMagnitude,
                                           Vector3d>::Write(WriteData& theWriteData) const
{
  Handle(Geom_VectorWithMagnitude) aMyGeom =
    Handle(Geom_VectorWithMagnitude)::DownCast(myTransient);
  theWriteData << aMyGeom->Vec();
}

//=======================================================================
// AxisPlacement
//=======================================================================

template <>
inline Standard_CString Geom1::subBase_gp<Geom1::basic,
                                                         Axis3d>::PName() const
{
  return "PGeom_AxisPlacement";
}

//=======================================================================
// Axis1Placement
//=======================================================================

template <>
inline Standard_CString Geom1::
  instance<Geom1::Axis1Placement, Geom_Axis1Placement, Axis3d>::PName() const
{
  return "PGeom_Axis1Placement";
}

template <>
inline void Geom1::instance<Geom1::Axis1Placement,
                                           Geom_Axis1Placement,
                                           Axis3d>::Write(WriteData& theWriteData) const
{
  Handle(Geom_Axis1Placement) aMyGeom = Handle(Geom_Axis1Placement)::DownCast(myTransient);
  write(theWriteData, aMyGeom->Ax1());
}

//=======================================================================
// Axis2Placement
//=======================================================================

template <>
inline Standard_CString Geom1::instance<Geom1::AxisPlacement,
                                                       Geom_Axis2Placement>::PName() const
{
  return "PGeom_Axis2Placement";
}

template <>
inline void Geom1::instance<Geom1::AxisPlacement,
                                           Geom_Axis2Placement>::Read(ReadData&
                                                                        theReadData)
{
  Axis3d anAxis;
  Dir3d anXDirection;

  theReadData >> anAxis >> anXDirection;

  myTransient = new Geom_Axis2Placement(anAxis.Location(), anAxis.Direction(), anXDirection);
}

template <>
inline void Geom1::instance<Geom1::AxisPlacement,
                                           Geom_Axis2Placement>::Write(WriteData&
                                                                         theWriteData) const
{
  Handle(Geom_Axis2Placement) aMyGeom      = Handle(Geom_Axis2Placement)::DownCast(myTransient);
  const Axis3d&               anAxis       = aMyGeom->Axis();
  const Dir3d&               anXDirection = aMyGeom->Direction();
  write(theWriteData, anAxis) << anXDirection;
}

//=======================================================================
// Transformation
//=======================================================================

template <>
inline Standard_CString Geom1::
  instance<Geom1::Transformation, Transformation1, Transform3d>::PName() const
{
  return "PGeom_Transformation";
}

template <>
inline void Geom1::instance<Geom1::Transformation,
                                           Transformation1,
                                           Transform3d>::Write(WriteData& theWriteData) const
{
  theWriteData << myTransient->Trsf();
}

//=======================================================================
// Geometry1
//=======================================================================

template <>
inline Standard_CString Geom1::geometryBase<Geometry3>::PName() const
{
  return "PGeom_Geometry";
}

//=======================================================================
// Curve
//=======================================================================

template <>
inline Standard_CString Geom1::geometryBase<GeomCurve3d>::PName() const
{
  return "PGeom_Curve";
}

//=======================================================================
// Surface
//=======================================================================

template <>
inline Standard_CString Geom1::geometryBase<GeomSurface>::PName() const
{
  return "PGeom_Surface";
}

#endif
